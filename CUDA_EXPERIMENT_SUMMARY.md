...
| Exact production full-stream histogram counts | Correctness only: direct multi-span, cache, literal, copy, prefix-boundary, repeat, invalid-command, and transactional-decline tests pass; isolated CPU/CUDA streams are byte-identical for methods 0–6. The implementation consumes the real linked blocks without a CPU flattening pass. No timing has been collected. | Integrated behind `WEBP_CUDA_HISTOGRAM=1`; entropy estimation, local distance-remapped histograms, merging, and Huffman construction remain on CPU. Keep off by default until a portable E2E result establishes a useful crossover. |
...
| Production cross-color → main-hash resident handoff | Correctness-only implementation. A dedicated device copy survives the intervening transform-map encode; the main hash request reuses it only when host pointer, pixel count, and row width match. No timing has been collected. | Integrated behind `WEBP_CUDA_RESIDENT_LOSSLESS=1`; the portable forced suite records and enables it. Keep off by default until measured. |
| Parallel production predictor policy | Correctness-only implementation. Every tile evaluates all 14 legal predictor modes with a deterministic integer concentration score, emits exact residuals, and hands them directly to resident cross-color when enabled. Odd dimensions and methods 1–6 decode identically; compressed bytes/size may differ by design. No timing has been collected. | Integrated behind `WEBP_CUDA_PREDICTOR=1`; keep off by default until E2E time and size measurements establish whether the parallel policy beats the CPU selector. |
