// Copyright 2026
//
// Private runtime gate for the independent v5 recorder-free remote
...
int VP8LBackrefCostWorkspaceRemoteV5ExperimentEnabled(void) {
  return EnvironmentIsOne("WEBP_BACKREF_COST_WORKSPACE_REMOTE_V5_EXPERIMENT");
}

void* VP8LBackrefCostWorkspaceRemoteV5ExperimentMalloc(size_t size) {
  if (EnvironmentIsOne(
          "WEBP_BACKREF_COST_WORKSPACE_REMOTE_V5_EXPERIMENT_INJECT_FAILURE")) {
    return NULL;
  }
