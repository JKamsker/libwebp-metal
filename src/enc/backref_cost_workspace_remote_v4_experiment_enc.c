// Copyright 2026
//
// Private runtime gate for the independent v4 recorder-free remote
...
int VP8LBackrefCostWorkspaceRemoteV4ExperimentEnabled(void) {
  return EnvironmentIsOne("WEBP_BACKREF_COST_WORKSPACE_REMOTE_V4_EXPERIMENT");
}

void* VP8LBackrefCostWorkspaceRemoteV4ExperimentMalloc(size_t size) {
  if (EnvironmentIsOne(
          "WEBP_BACKREF_COST_WORKSPACE_REMOTE_V4_EXPERIMENT_INJECT_FAILURE")) {
    return NULL;
  }
