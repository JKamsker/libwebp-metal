// Copyright 2026
//
// Private runtime gate for the independent v3 recorder-free remote
...
int VP8LBackrefCostWorkspaceRemoteV3ExperimentEnabled(void) {
  return EnvironmentIsOne("WEBP_BACKREF_COST_WORKSPACE_REMOTE_V3_EXPERIMENT");
}

void* VP8LBackrefCostWorkspaceRemoteV3ExperimentMalloc(size_t size) {
  if (EnvironmentIsOne(
          "WEBP_BACKREF_COST_WORKSPACE_REMOTE_V3_EXPERIMENT_INJECT_FAILURE")) {
    return NULL;
  }
