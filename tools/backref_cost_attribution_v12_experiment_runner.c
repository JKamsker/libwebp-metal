// Copyright 2026
//
// Private diagnostic runner for the backref-cost attribution v12 pipeline.
...
static int CounterCheck(const char* const input_path, int method,
  ...
  WebPPicture picture;
  WebPMemoryWriter writer;
  VP8LBackrefCostAttributionV12Counters zero, observed, reset;
  uint64_t digest;

  if (!VariantMatchesEnvironment(variant)) return 40;
  VP8LBackrefCostAttributionV12ResetCounters();
  zero = VP8LBackrefCostAttributionV12GetCounters();
  ...
  WebPPictureFree(&picture);
  observed = VP8LBackrefCostAttributionV12GetCounters();
  ...
  VP8LBackrefCostAttributionV12ResetCounters();
  reset = VP8LBackrefCostAttributionV12GetCounters();
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-attribution-v12-counter-check-v1\","
...
static int TimerCheck(void) {
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-attribution-v12-timer-check-v1\","
...
static int TimerAccountingCheck(const char* const variant) {
  ...
  const char* const timers = getenv("WEBP_BACKREF_COST_ATTRIBUTION_V12_TIMERS");
  WebPConfig config;
  WebPPicture picture;
  ...
  VP8LBackrefCostAttributionV12RecordDP(strcmp(variant, "candidate") == 0);
...
static int Profile(const char* const input_path, const char* const case_id,
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    digest = Fnv1a64(writer.mem, writer.size);
    printf(
        "{\"schema\":\"libwebp-backref-cost-attribution-v12-output-v1\","
...
int main(int argc, char** argv) {
  if (argc == 2 && strcmp(argv[1], "guard") == 0) {
    ...
    printf("{\"enabled\":%s}\n",
           VP8LBackrefCostAttributionV12ExperimentEnabled() ? "true" : "false");
    return 0;
  }
  ...
  fputs("invalid backref-cost attribution v12 runner arguments\n", stderr);
  return 2;
}
