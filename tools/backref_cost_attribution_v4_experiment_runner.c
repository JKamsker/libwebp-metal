// Copyright 2026
//
// Private diagnostic runner for the backref-cost attribution v4 pipeline.
...
static int CounterCheck(const char* const input_path, int method,
  ...
  WebPPicture picture;
  WebPMemoryWriter writer;
  VP8LBackrefCostAttributionV4Counters zero, observed, reset;
  uint64_t digest;

  if (!VariantMatchesEnvironment(variant)) return 40;
  VP8LBackrefCostAttributionV4ResetCounters();
  zero = VP8LBackrefCostAttributionV4GetCounters();
  ...
  WebPPictureFree(&picture);
  observed = VP8LBackrefCostAttributionV4GetCounters();
  ...
  VP8LBackrefCostAttributionV4ResetCounters();
  reset = VP8LBackrefCostAttributionV4GetCounters();
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-attribution-v4-counter-check-v1\","
...
static int TimerCheck(void) {
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-attribution-v4-timer-check-v1\","
...
static int TimerAccountingCheck(const char* const variant) {
  ...
  const char* const timers = getenv("WEBP_BACKREF_COST_ATTRIBUTION_V4_TIMERS");
  WebPConfig config;
  WebPPicture picture;
  ...
  VP8LBackrefCostAttributionV4RecordDP(strcmp(variant, "candidate") == 0);
...
static int Profile(const char* const input_path, const char* const case_id,
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    digest = Fnv1a64(writer.mem, writer.size);
    printf(
        "{\"schema\":\"libwebp-backref-cost-attribution-v4-output-v1\","
...
int main(int argc, char** argv) {
  if (argc == 2 && strcmp(argv[1], "guard") == 0) {
    ...
    printf("{\"enabled\":%s}\n",
           VP8LBackrefCostAttributionV4ExperimentEnabled() ? "true" : "false");
    return 0;
  }
  ...
  fputs("invalid backref-cost attribution v4 runner arguments\n", stderr);
  return 2;
}
