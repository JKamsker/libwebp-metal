// Copyright 2026
//
// Recorder-free process runner for the backref cost/traceback workspace A/B.
...
int main(int argc, char** argv) {
  ...
  const char* const experiment =
      getenv("WEBP_BACKREF_COST_TRACEBACK_EXPERIMENT");
  ...
  if (session == NULL || strcmp(session, "exclusive") != 0) {
    fprintf(stderr,
            "backref cost/traceback runner refuses timing without "
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    printf(
        "{\"schema\":\"libwebp-backref-cost-traceback-sample-v1\","
