...
// recorder-free CostManager interval-search v1 A/B.
...
static int VariantMatchesEnvironment(const char* const variant) {
  const char* const value =
      getenv("WEBP_BACKREF_COST_INTERVAL_SEARCH_V1_EXPERIMENT");
...
static int Diagnostic(const char* const input_path, const char* const case_id,
  ...
  VP8LBackrefCostIntervalSearchV1StatsReset();
  if (!Encode(&config, &picture, &writer)) {
    WebPPictureFree(&picture);
    return 22;
  }
  VP8LBackrefCostIntervalSearchV1StatsGet(&stats);
  digest = Fnv1a64(writer.mem, writer.size);
  printf(
      "{\"schema\":\"libwebp-backref-cost-interval-search-v1-work-v1\","
      ...
      "\"position_forward_steps\":%llu,\"tail_fast_paths\":%llu,"
      ...
      (unsigned long long)stats.tail_fast_paths,
...
static int Sample(const char* const input_path, const char* const case_id,
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    digest = Fnv1a64(writer.mem, writer.size);
    printf(
        "{\"schema\":\"libwebp-backref-cost-interval-search-v1-sample-v1\","
...
int main(int argc, char** argv) {
  ...
  fputs("invalid interval-search v1 runner arguments\n", stderr);
  return 2;
}
