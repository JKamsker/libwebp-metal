// Copyright 2026
//
// Recorder-free process runner for the cache-size single-pass slab A/B.
...
int main(int argc, char** argv) {
  ...
  if (session == NULL || strcmp(session, "exclusive") != 0) {
    fprintf(stderr,
            "cache-size single-pass slab runner refuses timing without "
  ...
  if (!ReadPPM(input_path, &rgb, &width, &height)) return 5;
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    printf("{\"schema\":\"libwebp-cache-size-single-pass-slab-sample-v1\","
