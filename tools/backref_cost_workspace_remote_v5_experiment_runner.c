// Copyright 2026
//
// Recorder-free process runner for the independent v5 remote backref-cost
// workspace A/B experiment.
...
int main(int argc, char** argv) {
  ...
  const char* const experiment =
      getenv("WEBP_BACKREF_COST_WORKSPACE_REMOTE_V5_EXPERIMENT");
  const char *input_path, *case_id, *lifecycle, *variant;
  int method, warmups, measured, iteration;
  uint8_t* rgb = NULL;
  ...
  if (session == NULL || strcmp(session, "exclusive") != 0) {
    fprintf(stderr,
            "backref-cost workspace remote v5 runner refuses timing without "
  ...
  if ((strcmp(lifecycle, "cold") != 0 && strcmp(lifecycle, "warm") != 0) ||
      (strcmp(variant, "baseline") != 0 && strcmp(variant, "candidate") != 0) ||
  ...
  if ((strcmp(variant, "candidate") == 0 &&
       (experiment == NULL || strcmp(experiment, "1") != 0)) ||
      (strcmp(variant, "baseline") == 0 && experiment != NULL)) {
    fprintf(stderr, "variant/runtime gate mismatch\n");
    return 5;
  }
  if (!ReadPPM(input_path, &rgb, &width, &height)) return 6;
  if (!WebPConfigInit(&config) || !WebPPictureInit(&picture)) return 7;
  config.lossless = 1;
  config.exact = 1;
  config.quality = 75.f;
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    printf(
        "{\"schema\":\"libwebp-backref-cost-workspace-remote-v5-sample-v1\","
