// Copyright 2026
//
// Private diagnostic runner for the backref-cost attribution v1 pipeline.
...
static int Profile(const char* const input_path, const char* const case_id,
  ...
  const char* const timers =
      getenv("WEBP_BACKREF_COST_ATTRIBUTION_V1_TIMERS");
  WebPConfig config;
  WebPPicture picture;
  int iteration;
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    printf("{\"schema\":\"libwebp-backref-cost-attribution-v1-output-v1\","
...
int main(int argc, char** argv) {
  if (argc == 2 && strcmp(argv[1], "guard") == 0) {
    ...
    printf("{\"enabled\":%s}\n",
           VP8LBackrefCostAttributionV1ExperimentEnabled() ? "true" :
                                                             "false");
    return 0;
  }
  ...
  fputs("invalid backref-cost attribution v1 runner arguments\n", stderr);
  return 2;
}
