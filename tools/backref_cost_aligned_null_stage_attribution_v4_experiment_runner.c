// Copyright 2026
//
// Private diagnostic runner for the backref-cost specialization aligned null stage attribution v4 pipeline.
...
static int DirectChildAdmission(void) {
  const char* const enabled = getenv("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_DIRECT_CHILD");
  const char* const ordinal = getenv("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_ORDINAL");
  const char* const token = getenv("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_CREATION_TOKEN");
  const char* const purpose = getenv("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_PURPOSE");
  const char* const lifecycle = getenv("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_LIFECYCLE");
  const char* const variant = getenv(
      "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_VARIANT");
  const char* const case_id = getenv("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_CASE_ID");
  const char* const method = getenv("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_METHOD");
  ...
  fprintf(stderr,
          "{\"schema\":\"libwebp-backref-cost-aligned-null-stage-attribution-v4-child-start-v1\","
...
static int WorkCheck(const char* const input_path, const char* const case_id,
  ...
  WebPPicture picture;
  WebPMemoryWriter writer;
  VP8LBackrefCostAlignedNullStageAttributionV4Stats observed;
  uint64_t digest;
  uint64_t selected_dp_calls;

  if (!VariantMatchesEnvironment(variant)) return 40;
  VP8LBackrefCostAlignedNullStageAttributionV4StatsReset();
  ...
  WebPPictureFree(&picture);
  VP8LBackrefCostAlignedNullStageAttributionV4StatsGet(&observed);
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-aligned-null-stage-attribution-v4-work-v1\","
...
static int TimerCheck(void) {
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-aligned-null-stage-attribution-v4-timer-check-v1\","
...
static int Profile(const char* const input_path, const char* const case_id,
  ...
  const char* const timers = getenv("WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_TIMERS");
  WebPConfig config;
  WebPPicture picture;
  int iteration;
  ...
  if (!VariantMatchesEnvironment(variant) ||
      !((warmups == 1 && measured == 4) ||
        (warmups == 0 && measured == 1 &&
         getenv("WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_SAMPLE_SET") !=
             NULL &&
         strcmp(getenv(
                    "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_SAMPLE_SET"),
                "timer-validation") == 0))) {
    return 31;
  }
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    digest = Fnv1a64(writer.mem, writer.size);
    printf(
        "{\"schema\":\"libwebp-backref-cost-aligned-null-stage-attribution-v4-output-v1\","
        "\"case_id\":\"%s\",\"method\":%d,\"variant\":\"%s\","
...
static int TimerAccountingCheck(const char* const input_path,
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-aligned-null-stage-attribution-v4-timer-accounting-output-v1\","
      "\"case_id\":\"%s\",\"method\":%d,\"variant\":\"%s\","
      "\"selector_evaluations\":%llu,\"baseline_dp_calls\":%llu,"
      "\"layout_clone_dp_calls\":%llu,"
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && strcmp(argv[1], "guard") == 0) {
    ...
    printf("{\"variant\":%d}\n",
           (int)VP8LBackrefCostAlignedNullStageAttributionV4GetVariant());
    return 0;
  }
...
#endif
  fputs("invalid backref-cost specialization aligned null stage attribution v4 runner arguments\n", stderr);
  return 2;
}
