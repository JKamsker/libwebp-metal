// Copyright 2026
//
// Private diagnostic runner for the backref-cost specialization factorization v2 pipeline.
...
static int WorkCheck(const char* const input_path, const char* const case_id,
  ...
  WebPPicture picture;
  WebPMemoryWriter writer;
  VP8LBackrefCostSpecializationFactorizationV2Stats observed;
  uint64_t digest;
  uint64_t selected_dp_calls;

  if (!VariantMatchesEnvironment(variant)) return 40;
  VP8LBackrefCostSpecializationFactorizationV2StatsReset();
  ...
  WebPPictureFree(&picture);
  VP8LBackrefCostSpecializationFactorizationV2StatsGet(&observed);
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-specialization-factorization-v2-work-v1\","
...
static int TimerCheck(void) {
  ...
  printf(
      "{\"schema\":\"libwebp-backref-cost-specialization-factorization-v2-timer-check-v1\","
...
static int Profile(const char* const input_path, const char* const case_id,
  ...
  const char* const timers = getenv("WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_TIMERS");
  WebPConfig config;
  WebPPicture picture;
  int iteration;
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    digest = Fnv1a64(writer.mem, writer.size);
    printf(
        "{\"schema\":\"libwebp-backref-cost-specialization-factorization-v2-output-v1\","
        "\"case_id\":\"%s\",\"method\":%d,\"variant\":\"%s\","
...
int main(int argc, char** argv) {
  if (argc == 2 && strcmp(argv[1], "guard") == 0) {
    ...
    printf("{\"variant\":%d}\n",
           (int)VP8LBackrefCostSpecializationFactorizationV2GetVariant());
    return 0;
  }
  ...
  fputs("invalid backref-cost specialization factorization v2 runner arguments\n", stderr);
  return 2;
}
