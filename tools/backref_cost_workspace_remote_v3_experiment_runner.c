// Copyright 2026
//
// Recorder-free process runner for the independent v3 remote backref-cost
...
int main(int argc, char** argv) {
  ...
  const char* const experiment =
      getenv("WEBP_BACKREF_COST_WORKSPACE_REMOTE_V3_EXPERIMENT");
  ...
  if (session == NULL || strcmp(session, "exclusive") != 0) {
    fprintf(stderr,
            "backref-cost workspace remote v3 runner refuses timing without "
  ...
  for (iteration = 0; iteration < warmups + measured; ++iteration) {
    ...
    printf(
        "{\"schema\":\"libwebp-backref-cost-workspace-remote-v3-sample-v1\","
