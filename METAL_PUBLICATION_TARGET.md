...
| Correctness | Automated forced-Metal checks now cover odd and minimal dimensions, padded strides, RGB/BGR/RGBA/BGRA/RGBX/BGRX, cancellation, fallback, byte-identity promises, decode fidelity, and a bounded UBSan mutation harness. Evidence remains from one Apple GPU family. | Cross-device repetition plus ASan on a compatible Apple toolchain and deeper failure-injection, concurrency, and memory-pressure evidence. |
