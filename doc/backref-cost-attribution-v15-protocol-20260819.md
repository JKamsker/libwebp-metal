# Frozen backref-cost attribution V15 protocol

Phase 1 starts from exact main commit
`e085ff0cd7e0e9f4e9bdeaa939346725ae3d4d22`. Merged V14 is default-off,
nonauthoritative groundwork only: it has no Phase 1A/1B freeze and exactly zero
remote observations. V15 independently re-audits V11–V14, creates fresh gates,
schemas, paths, process identities, and authority, and ports only the compact
evidence boundary supported by local fixtures and the V11/V13 raw truncations.
V15 fixes V14's invalid unary-plus archive-inventory digest comparison and
requires the complete framed response, including cleanup-last, to fit before
the first response byte is written. It does not extend another timeout or
select an unowned alternate data plane.
...
## Audited evidence and V15 boundary
...
ancestor's embedded source. V7 correctly authorized no timing. V15 does not
...
left no reusable remote child. V15 does not contact the V8 safe root or any
prior-cycle path.
...
authorized no timing. V15 does not contact, inspect, clean, or reuse the V11
...
silence was still valid. V15 does not contact, inspect, clean, or reuse the V12
...
V15 reads them locally only and never contacts either remote path.
...
compact archive, and a 530,845-byte request/response-direction fixture; V15
...
response before emission. V15 does not contact, inspect, clean, or reuse any
V14 remote path or authority; there is none.

V15 has independent default-off identities:

- `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V15_EXPERIMENT`;
- `WEBP_USE_BACKREF_COST_ATTRIBUTION_V15_EXPERIMENT`;
- exact runtime opt-in `WEBP_BACKREF_COST_ATTRIBUTION_V15_EXPERIMENT=1`.

All predecessor build and compile gates are mutually exclusive. Ordinary
objects, targets, symbols, and strings omit V15. Public API/ABI, installed
...
Inside each `VP8LBackwardReferencesTraceBackwards` call, V15 evaluates the
...
under the opposite preprocessor branch and cannot execute in V15. Private
...
session revalidates and removes only its own exact V15 child, proves absence,
...
default-off V15 gate, manifest, schema, exact rehearsal/Phase-2 command, corpus,
...
evidence below `evidence/backref-cost-attribution-v15-design/rehearsal/`, one
hash index for every raw file, and
`evidence/backref-cost-attribution-v15-design/phase1b-authorization.json`.
...
V15 freezes one `/bin/ps -axo pid=,ppid=,comm=` snapshot per guard and parses
...
space-bearing, and path-like values are valid; V15 never calls `realpath`,
...
derived only as the final slash component. V15 defines no resolved-path API.
...
Remote state is one canonical exact child under the distinct V15 safe root.
