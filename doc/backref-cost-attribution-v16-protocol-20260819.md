# Frozen backref-cost attribution V16 protocol

Phase 1 starts from exact main commit
`8ac198e565ab36eedd6e124202b78c09e5180ec1`. Merged V15 is default-off,
nonauthoritative forensic groundwork. Its Phase 1B explicitly grants no Phase
2 authority. V16 independently re-audits the V14/V15 compact-response design
and V15 refusal, creates fresh gates, schemas, paths, process identities,
wire version, representative payload identity, and authority, and ports only
the compact evidence boundary supported by local fixtures and the V11/V13 raw
truncations. V16 retains V15's corrected full-inventory comparison and
complete-response preflight, including cleanup-last before the first response
byte. It does not extend another timeout or select an unowned alternate data
plane.
...
## Audited evidence and V16 boundary
...
ancestor's embedded source. V7 correctly authorized no timing. V16 does not
...
left no reusable remote child. V16 does not contact the V8 safe root or any
prior-cycle path.
...
authorized no timing. V16 does not contact, inspect, clean, or reuse the V11
...
silence was still valid. V16 does not contact, inspect, clean, or reuse the V12
...
V16 reads them locally only and never contacts either remote path.

Merged V14 freezes no authority. Its preserved local summary claims 16
correctness encodes, sanitizer/ABI/guard checks, a deterministic 5,646-byte
compact archive, and a 530,845-byte request/response-direction fixture; V16
independently re-executes those gates. Static audit confirms that V14 retains
raw stage/runner/command/correctness/resource/counter records and complete
artifact indexes while removing only the representative request copy, repeated
raw process tables, full `nm`/build text, and duplicated command output. The
boundary is therefore complete and auditable after two V15 repairs: the local
inventory comparison is corrected, and the remote preflights the complete
response before emission. V16 does not contact, inspect, clean, or reuse any
V14 remote path or authority; there is none.

Merged V15 independently re-executed the compact local gates and froze those
two repairs, but its one permitted no-workload/no-lease rehearsal established
no SSH connection and emitted zero framed stdout bytes. The preserved raw
stderr is exactly 65 bytes and exactly
`ssh: connect to host 100.85.234.14 port 22: Operation timed out\r\n`.
Source admission, process isolation, operator zero-workload, lease
not-attempted, archive, integrity, cleanup, and exact-child absence receipts
are all missing. V15 therefore froze `timed_execution_authorized=false` and
`phase_2_authority=NONE`. V16 reads those merged bytes locally only, never
reruns V15, and never contacts or cleans a V15 or earlier path. Any post-V15
ICMP/TCP reachability checks are transient read-only context only: they are not
V16 authority, evidence, admission, or justification for a retry.

V16 has independent default-off identities:

- `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT`;
- `WEBP_USE_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT`;
- exact runtime opt-in `WEBP_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT=1`.

All predecessor build and compile gates are mutually exclusive. Ordinary
objects, targets, symbols, and strings omit V16. Public API/ABI, installed
...
Inside each `VP8LBackwardReferencesTraceBackwards` call, V16 evaluates the
...
under the opposite preprocessor branch and cannot execute in V16. Private
...
and the complete framed stdout stream is strictly below 1 MiB. The 16 correctness
...
archive remains complete. Before response emission, the remote encodes every
control, archive chunk, integrity footer, and cleanup-last receipt and rejects
an archive over 768 KiB, a control over 8 KiB, or a total response greater than
or equal to 1 MiB. The online parser independently validates the declaration
before state transition and enforces the same exclusive response cap before
extraction. It verifies frame
...
verification.
An integrity footer follows bulk. Before writing any response bytes, the
session revalidates and removes only its own exact V16 child, proves absence,
encodes the mandatory cleanup footer as the last frame, and preflights the
complete response. Normal success is
...
default-off V16 gate, manifest, schema, exact rehearsal/Phase-2 command, corpus,
order, count, timeout, fixture, and local validation. After Phase 1A is
committed, Phase 1 may run at most one synchronous, untimed
zero-encoder/profiler/lease rehearsal against exact 1A
...
evidence below `evidence/backref-cost-attribution-v16-design/rehearsal/`, one
...
env WEBP_BENCHMARK_SESSION=exclusive \
  ...
  python3 scripts/execute_backref_cost_attribution_v16.py run \
  "$PWD/evidence/backref-cost-attribution-v16-execution-20260819"
```
...
V16 freezes one `/bin/ps -axo pid=,ppid=,comm=` snapshot per guard and parses
...
space-bearing, and path-like values are valid; V16 never calls `realpath`,
...
derived only as the final slash component. V16 defines no resolved-path API.
...
Remote state is one canonical exact child under the distinct V16 safe root.
