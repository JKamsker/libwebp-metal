# Frozen Backref Cost Aligned Null Stage Attribution V2 protocol

## Authority and premise

Phase 1 starts from exact merged main
`fb297dd1097f6fc5a2f2d4d453da3c24b235d7d1`. It designs, validates,
...
evaluator reconstruction are audited without importing rows into V2. Eleven of
...
complete attribution ledger. V2 never pools, relabels, or reuses a V4 sample.

## Candidate and isolation

V2 is independently compile- and runtime-gated by
`WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_EXPERIMENT` and
`WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_VARIANT`. Both default
...
`__TEXT,__br_null_v2`, each aligned to 16,384 bytes, in order Push-B, Push-L,
...
branch, load, store, and body-size equality. No V2 H body, append-hint
...
370, 387, 405, 442, or 457. V2 uses
...
seed `0x4e554c32`. There is no adaptive stopping.
...
limited to the exact canonical V2 transaction child proved by its receipt,
...
`env -u WEBP_BENCHMARK_SESSION EXPECTED_FREEZE_COMMIT="$(git rev-parse HEAD)" python3 scripts/execute_backref_cost_aligned_null_stage_attribution_v2.py rehearse "$PWD/evidence/backref-cost-aligned-null-stage-attribution-v2-design/rehearsal"`
