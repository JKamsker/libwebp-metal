# Frozen Backref Cost Aligned Null Stage Attribution V3 protocol
...
evaluator reconstruction are audited without importing rows into V3. Eleven of
...
complete attribution ledger. V3 never pools, relabels, or reuses a V4 sample.

The transport premise is independently frozen before remote contact.
Successful Alignment V3/V4 rehearsals transferred 23,193,668 bytes in 354
...
V3 is independently compile- and runtime-gated by
`WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V3_EXPERIMENT` and
`WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V3_VARIANT`. Both default
...
`__TEXT,__br_null_v3`, each aligned to 16,384 bytes, in order Push-B, Push-L,
...
branch, load, store, and body-size equality. No V3 H body, append-hint
...
370, 387, 405, 442, or 457. V3 uses
...
limited to the exact canonical V3 transaction child proved by its receipt,
...
`env -u WEBP_BENCHMARK_SESSION EXPECTED_FREEZE_COMMIT="$(git rev-parse HEAD)" python3 scripts/execute_backref_cost_aligned_null_stage_attribution_v3.py rehearse "$PWD/evidence/backref-cost-aligned-null-stage-attribution-v3-design/rehearsal"`
