# Frozen backref-cost specialization alignment v2 protocol

## Scope and authority

This default-off diagnostic starts exactly at merged main
`b96909b28b3d64f000d0f98ffa76137305010808`. Phase 1 designs, implements,
...
`evidence/backref-cost-specialization-alignment-v2-design/phase1-prior-evidence-audit.json`
...
`__TEXT,__br_align_v2`. Each entry has 16,384-byte alignment, matching the
...
The compile gate is
`WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_EXPERIMENT`; ordinary
builds omit the source and runner. Runtime selector
`WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_VARIANT` accepts exact `B`, `L`,
...
SHA-256 blocks over
`libwebp-backref-cost-specialization-alignment-v2-rehearsal\0`; manifest,
...
`~/.libwebp-metal-backref-cost-specialization-alignment-v2`, then must prove
...
`env -u WEBP_BENCHMARK_SESSION EXPECTED_FREEZE_COMMIT="$(git rev-parse HEAD)" python3 scripts/execute_backref_cost_specialization_alignment_v2.py rehearse "$PWD/evidence/backref-cost-specialization-alignment-v2-design/rehearsal"`
