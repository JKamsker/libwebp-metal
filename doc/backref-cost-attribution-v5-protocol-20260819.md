# Frozen backref-cost attribution V5 protocol

Phase 1 starts from exact main commit
`a192d9b8562b497b03bf8cc83c2de5c7a6ea8cae`. It carries forward the merged
V4 single-selected-DP and timer semantics unchanged, and repairs only the
...
## Audited evidence and V5 boundary
...
V5 has independent default-off identities:

- `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V5_EXPERIMENT`;
- `WEBP_USE_BACKREF_COST_ATTRIBUTION_V5_EXPERIMENT`;
- exact runtime opt-in `WEBP_BACKREF_COST_ATTRIBUTION_V5_EXPERIMENT=1`.

All predecessor build and compile gates are mutually exclusive. Ordinary
objects, targets, symbols, and strings omit V5. Public API/ABI, installed
...
Inside each `VP8LBackwardReferencesTraceBackwards` call, V5 evaluates the
...
under the opposite preprocessor branch and cannot execute in V5. Private
...
The synchronous remote rehearsal keeps `WEBP_BENCHMARK_SESSION` absent, attempts no lease,
...
Remote state is one canonical `run.XXXXXX` exact child under the V5 safe root.
