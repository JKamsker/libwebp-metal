# Frozen backref-cost specialization factorization v2 protocol

## Scope and provenance

This is a fresh successor to merged default-off Factorization V1 at exact main
`ccdd4900633064f203010cbd437d2682e1cba51e`. V1 is groundwork plus a valid
refusal: its Phase 1A representative digest belonged to a predecessor domain,
so its sole rehearsal correctly yielded authority `NONE`. V16 remains complete
prior evidence. No V16 or V1 timing authority, samples, or workloads are inputs
to V2, and neither predecessor is rerun.
...
`WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_VARIANT`. An absent value
...
untimed local transport rehearsal runs with `WEBP_BENCHMARK_SESSION` absent.
The explicit no-host-contact constraint forbids SSH, host resolution, or any
remote service. The rehearsal constructs the full plan and exercises source,
local toolchain/resource/process, two-build, timer, framed transport, archive,
and cleanup admission, but performs zero encoder workloads, zero timing
samples, and zero benchmark-lease attempts. There is no retry. Because this
cannot establish the frozen remote device/resource/toolchain identity, Phase
1B must record authority `NONE` even when every local gate passes.
...
`~/.libwebp-metal-backref-cost-specialization-factorization-v2`, with a receipt
