...
**Status:** research evidence only; **not production-ready or upstream-ready**
...
end-to-end encoding. A standalone predictor-residual dispatch regressed all six
cold median strata. The only implementation promoted from the experiment series
was an exact 2x2 lossy import kernel: relative to the historical per-pixel Metal
...
fallback tests. The evidence remains limited by one device, one day, small
sample counts, correlated warm samples, synthetic workloads, and unresolved
provenance and licensing for three real-image inputs. The report therefore
supports publication as a bounded research result, not deployment or upstream
submission.
...
order/run variation rather than wins.
...
### Experiment 5: predictor residual
...
The standalone stage is rejected for promotion. Its doubly gated source remains
useful as an exact oracle for all 14 predictors and as a transactional-fallback
test. The experiment does not rule out a fused or resident predictor selector
and residual pipeline; it only rejects this cold standalone boundary.
...
| Transform ablations | rejected; small sign-changing effects, with 512 threads a consistent regression |
| Hash ablations | rejected; sub-percent, sign-changing effects |
...
| Standalone predictor residual | rejected for promotion; all six cold median strata regressed |
...
Experiment 1 passed repeated CPU and Metal stream equality within backend and
CPU/Metal decoded-pixel equality for all 18 case/method cells. Experiment 2
...
matched CPU, single Metal, and batched Metal streams. Experiment 4 passed all
85 mechanical checks, including checksums and exact variant outputs. Experiment
5 passed internal CPU-reference, exact-stream, repeated-run, decoded-pixel, and
injected-fallback checks for methods 0--6. These are strong focused tests, not a
substitute for fuzzing, sanitizers, adversarial dimensions, cancellation, or a
device matrix.
...
- Experiment 5 uses centisecond shell timing and cold processes. It rejects the
  measured standalone boundary but cannot characterize a fused warm pipeline.
...
Regenerate the omitted experiment-1 corpus and compare all manifest hashes:

```sh
python3 scripts/encoder_stage_profile.py prepare \
  --dataset-dir /tmp/libwebp-metal-stage-dataset
shasum -a 256 /tmp/libwebp-metal-stage-dataset/manifest.json
```

The manifest embeds absolute paths, so a regenerated manifest at another path
will not have the same whole-file digest. Compare its schema, generator policy,
case dimensions, and nine per-image SHA-256 fields to the committed manifest.
...
1. **Corpus provenance and licensing:** the source, author, license, and
   redistribution terms for `mitski.png`, `corgi.jpeg`, and `siamese.jpg` are
   unresolved. This blocks a fully reproducible publication package for
   experiments 4/5 and the historical baseline.
...
4. **Build/distribution gap:** Autotools omits Metal; installed-library ABI and
   ...
   modes.
5. **Correctness depth:** no sanitizer/fuzzer campaign, broad licensed corpus,
...
These blockers do not prevent publishing a candid research report with the
limits above. They do prevent describing the package as fully reproducible,
production-quality, supported, or ready for upstream submission.
...
- Rerun frozen protocols on at least two days and a supported Apple GPU-family
  ...
  thermal state.
- Replace or provenance-clear the three real images, version the corpus, and
  retain lawful redistribution terms and exact hashes.
...
correctness gates. Threshold changes, generalized batching, other ablations,
and a standalone predictor-residual stage are rejected or retained only as
research paths.
