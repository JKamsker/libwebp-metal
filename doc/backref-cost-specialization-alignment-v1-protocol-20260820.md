# Frozen backref-cost specialization alignment v1 protocol

## Scope and authority

This default-off diagnostic starts exactly at merged main
`adf680e69b9eeeef636e62e0e0eb0c8fdace4013`. Phase 1 designs, implements,
...
`evidence/backref-cost-specialization-alignment-v1-design/phase1-prior-evidence-audit.json`
...
`__TEXT,__br_align_v1`. Each entry has 16,384-byte alignment, matching the
...
The compile gate is
`WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_EXPERIMENT`; ordinary
builds omit the source and runner. Runtime selector
`WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_VARIANT` accepts exact `B`, `L`,
...
at most 2.0 and every required stage-share range to be at most 0.10 absolute.
...
SHA-256 blocks over
`libwebp-backref-cost-specialization-alignment-v1-rehearsal\0`; manifest,
...
Compact framed transport caps are fixed at archive ≤786,432 bytes, total
response <1,048,576 bytes, each control ≤8,192 bytes, and chunks exactly
262,144 bytes. Local archive and streaming fixtures must prove all four caps,
frame ordering, integrity, extraction, cleanup-last, and refusal behavior.
...
`~/.libwebp-metal-backref-cost-specialization-alignment-v1`, then must prove
absence. Prior-cycle paths are forbidden.
...
`env -u WEBP_BENCHMARK_SESSION EXPECTED_FREEZE_COMMIT="$(git rev-parse HEAD)" python3 scripts/execute_backref_cost_specialization_alignment_v1.py rehearse "$PWD/evidence/backref-cost-specialization-alignment-v1-design/rehearsal"`
