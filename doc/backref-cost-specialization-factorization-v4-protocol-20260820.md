# Frozen backref-cost specialization factorization v4 protocol

## Scope and provenance

This is a fresh successor to merged default-off Factorization V3 at exact main
`f289499b37875d8166792ed104f2b26e23cf4ffd`. V3 froze payload SHA-256
`8fb97e16b8be09e7dc0c20b0b26387a0f8301d0aa738fa7ade4a6a97d15afeb8`
and canonical toolchain identity SHA-256
`735eb9be26c9b7118819b502bf0e24b6d87e8da945d81858c22bebdf889ce077`.
Its B/L work and normalized codegen equality passed, while its sole SSH ended
at one `ConnectTimeout` before connection or frame admission. Authority was
therefore `NONE` and it produced no performance samples. V4 does not rerun V3,
reuse any V2/V3 sample, use any prior-cycle safe root, or contact any
prior-cycle remote path. Its independent provenance gate rejects both V2 and
V3 payload identities and the V3 toolchain envelope identity.
...
`WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V4_VARIANT`. An absent value
...
diagnostic commands, timed child commands, and codegen commands. V4 pins absolute
...
independent fixture. Negative tests reject V2's stale xcrun digest and V3's
prior canonical toolchain envelope identity. No
...
The SSH command freezes `ConnectionAttempts=1` and `ConnectTimeout=30`. The
30-second connect admission exists only for overlay first-session admission;
it is not a retry or transaction extension. `ServerAliveInterval=15`,
`ServerAliveCountMax=3`, the 1,800-second transaction deadline, and the
stream/footer progress deadlines remain separate. No ping, `nc`, second SSH,
background keepalive, SCP, or post-transaction connection is permitted.
...
`~/.libwebp-metal-backref-cost-specialization-factorization-v4`, with a receipt
