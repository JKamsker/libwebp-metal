# Frozen backref-cost workspace remote v2 protocol

## Authority and phase boundary

This protocol is the sole phase-2 input. Its immutable design parent is
`66c5bca06e4371923ebd038e0776524b7c78bb9e`, based on main
`d12036f790cce1dd4bef9fe3c963b0ef294d4c4a`. Only samples created from the
...
performance data for v2.

The executor must not edit criteria, counts, order, power state, services, or
remote state outside the generated v2 run child. There is no local timing
...
```sh
WEBP_EXPECTED_BACKREF_COST_WORKSPACE_REMOTE_V2_COMMIT=<FROZEN_PROTOCOL_COMMIT> \
WEBP_BENCHMARK_SESSION=exclusive \
python3 scripts/execute_backref_cost_workspace_remote_v2.py run \
  /absolute/local/path/evidence/backref-cost-workspace-remote-v2
```
...
`$HOME/.libwebp-metal-backref-cost-workspace-remote-v2`, rejects a symlinked or
...
remotely, runs `git bundle verify`, clones only beneath the accepted run child,
checks out the expected commit, and verifies clean exact `HEAD`. Returned
...
`Runner.Worker`, and no competing v2 runner, benchmark, or CUDA process. A
listener is allowed but untouched. No power setting is changed.

The nonblocking user-scoped lease is the new path
`~/.libwebp-metal-backref-cost-workspace-remote-v2.lock`. It is acquired only
...
default-build-with-v2-environment bitstreams, exact decoded source pixels,
...
Before this freeze, that exact command passed against canonical HOME
`/Users/pandocs-agent-001`: it accepted generated child `run.1Rx3Mg`, rejected
the malformed, sibling, symlink, and traversal fixtures, removed only the
validated child, and proved absence. The process guard found no
`Runner.Worker`; the listener remained present and untouched. The exact
untimed/unleased record is frozen at
`evidence/backref-cost-workspace-remote-v2-design/path-round-trip.json`.
