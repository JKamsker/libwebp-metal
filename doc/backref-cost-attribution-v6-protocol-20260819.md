# Frozen backref-cost attribution V6 protocol

Phase 1 starts from exact main commit
`c9d21ac9c512c94f036fc2db7eb118da83b4f7f9`. It carries forward the merged
V5 single-selected-DP/timer semantics and two-plane evidence transport
unchanged, and repairs only the circular Phase-1 rehearsal identity contract.
...
## Audited evidence and V6 boundary
...
V6 has independent default-off identities:

- `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V6_EXPERIMENT`;
- `WEBP_USE_BACKREF_COST_ATTRIBUTION_V6_EXPERIMENT`;
- exact runtime opt-in `WEBP_BACKREF_COST_ATTRIBUTION_V6_EXPERIMENT=1`.

All predecessor build and compile gates are mutually exclusive. Ordinary
objects, targets, symbols, and strings omit V6. Public API/ABI, installed
...
Inside each `VP8LBackwardReferencesTraceBackwards` call, V6 evaluates the
...
under the opposite preprocessor branch and cannot execute in V6. Private
...
bound. V6 freezes merged V5's conservative 512 KiB chunks, 20-second per-chunk
bound, 60-second total bound, and 180-second archive-build bound before its
single rehearsal. The Phase-1B envelope records V6's observed archive bytes,
hash, elapsed time, throughput, and chunk count; those observations cannot
change any frozen timeout.
...
Evidence return is two-plane. The control plane independently transfers and
locally hashes/verifies source/admission identity, compact operator status, the
archive-build receipt, a lease receipt exactly when acquisition was attempted,
and—last—the exact-child cleanup/absence receipt. Lease state is derived only
from the independently returned operator/lease receipts. These receipts remain
available when optional bulk transfer fails. The data plane separately returns
one `tar+gzip` archive whose SHA-256, byte size, compression format, and full
safe-member inventory are declared before transfer. Size, hash, inventory,
member security, and canonical containment all verify before staged extraction;
rejection is all-or-nothing and quarantined.

## Two-commit Phase-1 identity and freeze

Phase 1A is the immutable source/protocol commit. It contains every code path,
default-off V6 gate, manifest, schema, exact rehearsal/Phase-2 command, corpus,
...
evidence below `evidence/backref-cost-attribution-v6-design/rehearsal/`, one
...
env WEBP_BENCHMARK_SESSION=exclusive \
  ...
  python3 scripts/execute_backref_cost_attribution_v6.py run \
  "$PWD/evidence/backref-cost-attribution-v6-execution-20260819"
```
...
Remote state is one canonical `run.XXXXXX` exact child under the V6 safe root.
