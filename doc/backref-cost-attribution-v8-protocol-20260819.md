# Frozen backref-cost attribution V8 protocol

Phase 1 starts from exact main commit
`fea767272f7d9d13e5e5f19cb910e6152418dcdd`. It carries forward the merged
V7 single-selected-DP/timer semantics, single-session framed transport,
integrated cleanup footer, two-plane evidence, and non-circular two-commit
Phase-1 authority unchanged. V8 repairs only V7's process-isolation false
positive: V7 searched arbitrary command arguments and matched the transport
ancestor because its `python3 -c` argument embedded source containing
`_experiment_runner`.
...
## Audited evidence and V8 boundary
...
ancestor's embedded source. V7 correctly authorized no timing. V8 does not
contact or inspect that V7 child or any prior-cycle path.

V8 has independent default-off identities:

- `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V8_EXPERIMENT`;
- `WEBP_USE_BACKREF_COST_ATTRIBUTION_V8_EXPERIMENT`;
- exact runtime opt-in `WEBP_BACKREF_COST_ATTRIBUTION_V8_EXPERIMENT=1`.

All predecessor build and compile gates are mutually exclusive. Ordinary
objects, targets, symbols, and strings omit V8. Public API/ABI, installed
...
Inside each `VP8LBackwardReferencesTraceBackwards` call, V8 evaluates the
...
under the opposite preprocessor branch and cannot execute in V8. Private
...
session revalidates and removes only its own exact V8 child, proves absence,
...
default-off V8 gate, manifest, schema, exact rehearsal/Phase-2 command, corpus,
...
evidence below `evidence/backref-cost-attribution-v8-design/rehearsal/`, one
hash index for every raw file, and
`evidence/backref-cost-attribution-v8-design/phase1b-authorization.json`.
...
env WEBP_BENCHMARK_SESSION=exclusive \
  ...
  python3 scripts/execute_backref_cost_attribution_v8.py run \
  "$PWD/evidence/backref-cost-attribution-v8-execution-20260819"
```
...
V8 freezes one `/bin/ps -axo pid=,ppid=,comm=` snapshot per guard and parses
only PID, PPID, and executable `comm`. It never requests or inspects command
arguments, source, or payload text. Conflicts are exact executable-basename
members of the documented frozen deny set; prefixes, suffixes, and lookalikes
do not match. The transaction's own self-to-PID-1 chain must be complete,
acyclic, and composed only of exact frozen Python, SSH/sshd-session, shell, and
launchd executable paths. This ancestor exception applies only to the verified
chain and does not allow unrelated executables. Duplicate, missing, malformed,
noncanonical, or otherwise ambiguous identity data fails closed.

Remote state is one canonical exact child under the distinct V8 safe root.
The same session owns child creation, validation, operator execution, evidence
streaming, final removal, and absence proof. It never contacts the V7 root or
