# Frozen backref-cost specialization alignment v4 protocol

## Scope and authority

This default-off diagnostic starts exactly at merged main
`91ab79b70bb1f99888dce63e8b1ac512741bafb3`. Phase 1 designs, implements,
...
static/rehearsal gate, the sole rehearsal succeeds, and the envelope grants
`ALIGNMENT_V4_CONDITIONAL_DIAGNOSTIC_AUTHORITY`. Phase 3 is outside this Phase
1 handoff.

Factorization V4 is merged evidence, not an experiment to rerun. The audit in
`evidence/backref-cost-specialization-alignment-v4-design/phase1-prior-evidence-audit.json`
...
`__TEXT,__br_align_v4`. Each entry has 16,384-byte alignment, matching the
...
The compile gate is
`WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_EXPERIMENT`; ordinary
builds omit the source and runner. Runtime selector
`WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_VARIANT` accepts exact `B`, `L`,
...
## Two-level authority and in-pipeline admission

The sole Phase 1 rehearsal has zero encoder workloads, zero timer-runner
children, zero performance samples, and zero lease attempts. It proves the
transport, source, payload, toolchain, device, resource, process-isolation,
archive, integrity, cleanup/absence, codegen, and frozen local/static semantic
prerequisites. It does not claim that future correctness encodes, timer rows,
or child ownership already happened.

If those rehearsal facts pass, Phase 1B may grant only
`ALIGNMENT_V4_CONDITIONAL_DIAGNOSTIC_AUTHORITY`. This permits one frozen Phase
2 SSH transaction and one exclusive lease. It permits no accepted performance
row until the same transaction passes these runtime gates in order:

1. Gate A runs exactly 24 correctness encodes in frozen order:
   photo-large then graphic-large; methods 4 then 6; B, L, H; two repeats.
   Every six-product case/method set must have one bitstream hash, each repeat
   must be deterministic, decoded RGB must equal input, and the input corpus
   hash must remain unchanged. The frozen default-omission, isolation,
   fallback/fault, integer/tie, and transactional behavior must remain
   hash-identical.
2. Gate B first runs one clock-only child, then one explicitly untimed
   timer-accounting encode for each B/L/H on photo-large method 4. Its files
   live only under `timer-validation/` and use timer-validation schemas, never
   the performance filenames/schemas. The clock must be monotonic. Exactly one
   expected selected DP counter may be nonzero; selector, activation, DP total,
   setup, and steady call counts must agree; setup+steady must not exceed DP
   total; DP total+traceback+materialization must not exceed backward refs;
   reset scope is one encode and per-pixel clock calls are zero.
3. Gate C applies online to all 76 actual runner children: 24 correctness, one
   clock, three timer-accounting, 12 work, and 36 profile children. Each is an
   exact-argv/env/cwd `Popen` with `shell=False`, `close_fds=True`, and no new
   session. The child emits its ordinal, PID, observed PPID, operator creation
   identity, child monotonic start token, variant, case, method, lifecycle, and
   purpose, then blocks before work. Only the retained direct parent validates
   that receipt and returns the creation token. At most one child is live; the
   predecessor must be exited and reaped; ordinals and receipts are one-to-one;
   wrappers, shells, detached sessions, grandchildren, PID/start reuse,
   identity mismatch, duplicates, missing reap, and discovery-derived
   ownership fail closed. The invariant remains active through every warmup
   and retained profile child.

Any admission or later ownership failure releases the lease, returns indexed
refusal evidence, preserves exact-child cleanup/absence, accepts zero
performance rows, forbids partial promotion, and forbids retry. Raw partial
rows, if a later child fails, are explicitly marked unaccepted and cannot be
used as evidence.
...
SHA-256 blocks over
`libwebp-backref-cost-specialization-alignment-v4-rehearsal\0`; manifest,
...
`RequestTTY=no`. Its command embeds only the bounded, hash-bound V4 bootstrap;
...
`~/.libwebp-metal-backref-cost-specialization-alignment-v4`, then must prove
...
`env -u WEBP_BENCHMARK_SESSION EXPECTED_FREEZE_COMMIT="$(git rev-parse HEAD)" python3 scripts/execute_backref_cost_specialization_alignment_v4.py rehearse "$PWD/evidence/backref-cost-specialization-alignment-v4-design/rehearsal"`

That command must not run correctness, clock, timer-accounting, work, warmup,
retained, or profiler children. It may construct and freeze the 36-command plan
but attempts no lease. There is no second contact under any outcome.
