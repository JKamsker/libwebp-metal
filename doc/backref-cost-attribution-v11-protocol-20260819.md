# Frozen backref-cost attribution V11 protocol

Phase 1 starts from exact main commit
`631ac52803ff7d74f8461917503d54b46a8fc5e4`. It carries forward the merged
V10 single-selected-DP/timer semantics, single-session framed transport,
integrated cleanup footer, two-plane evidence, and non-circular two-commit
Phase-1 authority unchanged. V11 repairs only V10's frozen process-identity
model: V10 parsed complete opaque `comm` correctly but required each verified
ancestor to match an environment-specific positive executable allowlist and
returned no offending identity in its refusal frame.
...
## Audited evidence and V11 boundary
...
ancestor's embedded source. V7 correctly authorized no timing. V11 does not
...
left no reusable remote child. V11 does not contact the V8 safe root or any
prior-cycle path.
...
Phase 2. V11 reuses no V10 authority, evidence, remote child, or timing.

V11 has independent default-off identities:

- `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V11_EXPERIMENT`;
- `WEBP_USE_BACKREF_COST_ATTRIBUTION_V11_EXPERIMENT`;
- exact runtime opt-in `WEBP_BACKREF_COST_ATTRIBUTION_V11_EXPERIMENT=1`.

All predecessor build and compile gates are mutually exclusive. Ordinary
objects, targets, symbols, and strings omit V11. Public API/ABI, installed
...
Inside each `VP8LBackwardReferencesTraceBackwards` call, V11 evaluates the
...
under the opposite preprocessor branch and cannot execute in V11. Private
...
session revalidates and removes only its own exact V11 child, proves absence,
...
default-off V11 gate, manifest, schema, exact rehearsal/Phase-2 command, corpus,
...
evidence below `evidence/backref-cost-attribution-v11-design/rehearsal/`, one
hash index for every raw file, and
`evidence/backref-cost-attribution-v11-design/phase1b-authorization.json`.
...
env WEBP_BENCHMARK_SESSION=exclusive \
  ...
  python3 scripts/execute_backref_cost_attribution_v11.py run \
  "$PWD/evidence/backref-cost-attribution-v11-execution-20260819"
```
...
V11 freezes one `/bin/ps -axo pid=,ppid=,comm=` snapshot per guard and parses
...
space-bearing, and path-like values are valid; V11 never calls `realpath`,
...
derived only as the final slash component. V11 defines no resolved-path API.
...
Remote state is one canonical exact child under the distinct V11 safe root.
The same session owns child creation, validation, operator execution, evidence
streaming, final removal, and absence proof. It never contacts a V1-V10 root or
