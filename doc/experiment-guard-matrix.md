...
| 35 backref-cost attribution v15 | `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V15_EXPERIMENT` | `WEBP_USE_BACKREF_COST_ATTRIBUTION_V15_EXPERIMENT` | `WEBP_BACKREF_COST_ATTRIBUTION_V15_EXPERIMENT=1` | diagnostic-only v14 compact evidence with whole-response preflight and corrected inventory admission, a 768 KiB archive cap, and a strict 1 MiB total framed-response cap; no argv scanning, profiler, attach rights, or promotion authority |
| 36 backref-cost attribution v16 | `WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT` | `WEBP_USE_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT` | `WEBP_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT=1` | diagnostic-only v15 compact protocol with fresh identity and authority, whole-response preflight, a 768 KiB archive cap, and a strict 1 MiB total framed-response cap; no prior-cycle authority, argv scanning, profiler, attach rights, or promotion authority |
| 37 backref-cost specialization factorization v1 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_VARIANT=B/L/H` | fresh diagnostic-only three-way B original / L matched-layout baseline clone / H append-hint factorization; default build omission, exact selector-once boundary, recorder-free timed identity, separate untimed work recorder, compact capped transport, and no prior timing reuse |
| 38 backref-cost specialization factorization v2 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_VARIANT=B/L/H` | payload-identity-repaired diagnostic-only B/L/H factorization; manifest/generator/fixture agreement precedes freeze, timed binary remains recorder-free, counters are untimed, and Phase 1 rehearsal has zero host contact/workload/lease attempts |
| 39 backref-cost specialization factorization v3 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_VARIANT=B/L/H` | payload-and-toolchain-identity-repaired diagnostic-only B/L/H factorization; pinned Xcode executables and SDK metadata replace xcrun, triple agreements precede freeze, timed binary remains recorder-free, counters are untimed, and Phase 1 rehearsal has zero workload/lease attempts |
| 40 backref-cost specialization factorization v4 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V4_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V4_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V4_VARIANT=B/L/H` | independently frozen V4-domain diagnostic-only B/L/H factorization; rejects stale V2/V3 payload and V3 envelope identities, pins absolute Xcode tools without xcrun, uses a recorder-free timed runner plus separate untimed recorder, and permits one zero-workload/zero-lease rehearsal only |
| 41 backref-cost specialization alignment v1 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_VARIANT=B/L/H` | fresh 16 KiB page-offset-normalized B/L/H diagnostic; six distinct no-deduplicate DP/Push entries occupy one executable section at equal modulo/order/spacing, B→L is a mandatory fixed null gate, timed binary is recorder-free, and Phase 1 permits one zero-workload/zero-lease rehearsal only |
| 42 backref-cost specialization alignment v2 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_VARIANT=B/L/H` | fresh V2 identity preserving Alignment V1's 16 KiB B/L/H causal design and fixed null gate while replacing the oversized full source-map control receipt with a compact count/digest commitment and independently verified indexed archive detail; default-off, recorder-separated, and one zero-workload/zero-lease Phase 1 rehearsal only |
| 43 backref-cost specialization alignment v3 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V3_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V3_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V3_VARIANT=B/L/H` | fresh V3 identity preserving V2's 16 KiB six-body design, compact source receipt, corpus/counts/thresholds, and adding a single-SSH READY/declaration/fixed-chunk/ACK request channel; default-off, recorder-separated, and one zero-workload/zero-lease Phase 1 rehearsal only |
| 44 backref-cost specialization alignment v4 | `WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_EXPERIMENT` | `WEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_EXPERIMENT` | `WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_VARIANT=B/L/H` | fresh V4 identity preserving V3 transport and the exact six-body candidate while granting only conditional one-pipeline authority; 24 correctness encodes, coarse timer accounting, and serial retained direct-child ownership are fail-closed runtime admission gates before accepted timing |
| 45 backref-cost aligned null stage attribution v1 | `WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_EXPERIMENT` | `WEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_EXPERIMENT` | `WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_VARIANT=B/L` | fresh B/L-only four-body 16 KiB matched-layout null diagnostic with a complete disjoint whole-encode ledger, nested descriptive backref timers, separate untimed recorder, and fail-closed correctness/accounting/direct-child gates before any accepted timing |
| 46 backref-cost aligned null stage attribution v2 | `WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_EXPERIMENT` | `WEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_EXPERIMENT` | `WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_VARIANT=B/L` | fresh four-body identity refreeze generated from one canonical B-Push/L-Push/B-DP/L-DP list; preserves V1's B/L-only ledger design under isolated compile/runtime/schema/transport identities and requires exact frozen build receipt agreement before conditional authority |
| 47 backref-cost aligned null stage attribution v3 | `WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V3_EXPERIMENT` | `WEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V3_EXPERIMENT` | `WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V3_VARIANT=B/L` | fresh bounded request-transfer requalification preserving V2's canonical B/L four-body science and limits while adding an attribution-neutral authenticated per-chunk progress chain |
