# Frozen backref-cost specialization factorization v1 protocol

## Scope and provenance

This is a fresh successor to merged attribution V16 at exact main
`c6046dc7cde6e7ea1cc9d54a6a90c55f8a2f2edb`. V16 is complete evidence. None
of its timing samples are inputs to this cycle, and this cycle is not a rerun of
its two-variant A/B.
...
`WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_VARIANT`. An absent value
...
## Remote authority and process safety

Phase 1A freezes all source and hashes first. Then exactly one synchronous
untimed rehearsal is sent to `ssh pandocs-agent` with
`WEBP_BENCHMARK_SESSION` absent. It constructs the full plan, performs source,
host, toolchain, resource, process, two-build, timer, transport, archive, and
cleanup admission, but performs zero encoder workloads, zero timing samples,
and zero benchmark-lease attempts. There is no retry.
...
stopped on its own timeout. Remote cleanup may remove only the exact validated
cycle-owned `run.XXXXXX` child beneath
`~/.libwebp-metal-backref-cost-specialization-factorization-v1`, with a receipt
...
absence proof, process gate, archive gate, or receipt yields authority `NONE`.
