#!/usr/bin/env python3
"""Untimed attribution-v2 path, admission, receipt, and mode tests."""
...
from backref_cost_attribution_v2_paths import (
...
def path_tests() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v2-path-home-") as raw:
...
def repository_context_admission_tests() -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v2-admit-test-") as raw:
        temporary = Path(os.path.realpath(raw))
        repository = temporary / "repository"
        repository.mkdir()
        subprocess.run(["git", "init", "-q", str(repository)], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.name",
                        "attribution v2 admission test"], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.email",
                        "attribution-v2-admission@example.invalid"], check=True)
...
def main() -> int:
    ...
    print("PASS: attribution v2 canonical paths, repository-context bundle verification, "
