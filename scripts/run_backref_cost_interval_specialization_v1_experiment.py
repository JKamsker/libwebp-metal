#!/usr/bin/env python3
"""Frozen interval-specialization-v1 host operator for the backref-cost workspace interval-specialization v1 A/B."""
...
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_interval_specialization_v1_manifest.json"
...
def load_manifest() -> dict:
    ...
    if "timed binaries omit" not in \
            manifest["protocol"]["recorders"]:
        raise RuntimeError("timed recorders must remain disabled")
    return manifest
...
def toolchain_admission(manifest: dict,
    ...
    with tempfile.TemporaryDirectory(prefix="interval-specialization-v1-toolchain-probe-") as raw:
...
def configure_and_build(manifest: dict, temporary: Path, log,
    ...
    for variant in ("baseline", "candidate", "diagnostic"):
        ...
        make = [
            ...
            "WEBP_ENABLE_METAL=0",
            "WEBP_BACKREF_COST_INTERVAL_SPECIALIZATION_V1_DETERMINISTIC_BUILD=1",
            f"{manifest['candidate']['build_flag']}=" +
            ("0" if variant == "baseline" else "1"),
            "examples/cwebp", "examples/dwebp",
        ]
        if variant == "diagnostic":
            make.append("WEBP_BACKREF_COST_INTERVAL_SPECIALIZATION_V1_RECORDER=1")
        if variant != "baseline":
...
def work_metric_gate(manifest: dict, builds: dict[str, Path], corpus: Path,
                     cases: dict, output: Path) -> list[dict]:
    """Run the optional recorder only before timing and freeze direct work."""
    rows = []
    runner = (builds["diagnostic"] / "tools" /
              manifest["candidate"]["runner_target"])
    for timed in manifest["corpus"]["timed"]:
        case_id = timed["case_id"]
        source = corpus / cases[case_id]["file"]
        for method in manifest["protocol"]["methods"]:
            pair = {}
            for variant in ("baseline", "candidate"):
                result = command(
                    [str(runner), "diagnostic", str(source), case_id,
                     str(method), variant],
                    env=clean_environment(manifest, variant))
                lines = result.stdout.splitlines()
                if len(lines) != 1:
                    raise RuntimeError("work recorder row-count mismatch")
                row = json.loads(lines[0])
                if row.get("schema") != manifest["schemas"]["work"]:
                    raise RuntimeError("work recorder schema mismatch")
                row["role"] = timed["role"]
                rows.append(row)
                pair[variant] = row
            ...
            if any(baseline[name] != candidate[name] for name in immutable):
                raise RuntimeError("candidate changed non-search work metrics")
            if (baseline["bitstream_fnv1a64"] !=
                    candidate["bitstream_fnv1a64"] or
                    baseline["bitstream_bytes"] != candidate["bitstream_bytes"]):
                raise RuntimeError("work recorder bitstream mismatch")
            active = baseline["position_forward_steps"] > 0
            if active:
                baseline_steps = (baseline["position_forward_steps"] +
                                  baseline["position_backward_steps"])
                candidate_steps = (candidate["position_forward_steps"] +
                                   candidate["position_backward_steps"])
                if (candidate["runtime_activations"] <= 0 or
                        candidate["append_hint_fast_paths"] <= 0 or
                        candidate_steps * 10 > baseline_steps):
                    raise RuntimeError("frozen positioning-work reduction failed")
            elif (candidate["position_forward_steps"] != 0 or
                  candidate["append_hint_fast_paths"] != 0):
                raise RuntimeError("inactive corpus cell reported search work")
    with (output / "work-metrics.jsonl").open("w") as destination:
        for row in rows:
            destination.write(json.dumps(row, sort_keys=True) + "\n")
    write_json(output / "work-metric-summary.json", {
        "schema": manifest["schemas"]["work_summary"],
        "rows": len(rows),
        "active_candidate_rows": sum(
            row["variant"] == "candidate" and
            row["append_hint_fast_paths"] > 0
            for row in rows),
        "criterion": "candidate positioning steps <= 10% of baseline in every active cell",
        "status": "PASS",
    })
    return rows
...
def record_stage(output: Path, stage: str) -> None:
    write_json(output / "operator-stage.json", {
        "schema": "libwebp-backref-cost-interval-specialization-v1-stage-v1",
        "stage": stage,
        "captured_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    })
...
def execute(manifest: dict, output: Path) -> None:
        ...
        with lease_path.open("a+") as lease:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory(
                        prefix="backref-cost-interval-specialization-v1-build-") as raw:
                    temporary = Path(raw)
                    ...
                    record_stage(output, "work-metrics")
                    work_metrics = work_metric_gate(
                        manifest, builds, corpus, cases, output)
    ...
    if (len(commands), len(cold), len(warm), len(warmups), len(measured),
            ...
            len(correctness), len(work_metrics)) != (
    ...
    (output / "operator-report.md").write_text(
        "# Remote backref-cost workspace interval-specialization v1 A/B operator report\n\n"
        f"Source: `{commit}`  \nCorrectness cells: {len(correctness)}  \n"
        f"Untimed work-metric rows: {len(work_metrics)}  \n"
        f"Raw commands: {len(commands)}  \nRaw sample rows: {len(samples)}  \n"
        f"Frozen automated decision: **{evaluation['decision']}**  \n"
        "Only this interval-specialization v1 matrix is eligible. Timing was remote, serial, "
...
def controlled_run(output: Path) -> None:
    ...
    bootstrap_lease = {
        "schema": "libwebp-backref-cost-interval-specialization-v1-lease-v1",
    ...
    try:
        ...
        write_json(output / "refusal.json", {
            "schema": "libwebp-backref-cost-interval-specialization-v1-refusal-v1",
    ...
    write_json(output / "operator-status.json", {
        "schema": "libwebp-backref-cost-interval-specialization-v1-operator-status-v1",
        "status": status, "artifact_index_written": True,
        "controlled_exit": True,
    })
    artifact_schema = (manifest["schemas"]["artifact_hashes"] if manifest
                       else "libwebp-backref-cost-interval-specialization-v1-artifact-hashes-v1")
    ...
    if failure is not None:
        raise RuntimeError(
            "controlled interval-specialization-v1 refusal; indexed evidence is returnable") from failure
...
def rehearsal(manifest: dict, output: Path) -> None:
    ...
    fixture_results = {}
    fixtures = {
        "missing_make": {"make": "/nonexistent/interval-specialization-v1-fixture/make"},
    ...
    with tempfile.TemporaryDirectory(
            prefix="backref-cost-interval-specialization-v1-rehearsal-") as raw:
        temporary = Path(raw)
        with (output / "build.log").open("wb") as log:
            builds = configure_and_build(
                manifest, temporary, log, output, commit)
    ...
    write_json(output / "rehearsal.json", {
        ...
        "work_metric_rows": len(work_metrics),
...
def refusal_fixture(manifest: dict, output: Path, fixture: str) -> None:
    ...
    overrides = {
        "missing": {"make": "/nonexistent/interval-specialization-v1-fixture/make"},
