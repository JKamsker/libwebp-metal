#!/usr/bin/env python3
"""Framed single-session transport for backref-cost attribution v12.
...
removes exactly one validated v12 safe-root child.  It never discovers a PID for
...
MAGIC = b"LIBWEBP-BACKREF-COST-ATTRIBUTION-V12-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 1
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-attribution-v12"
RUN_PREFIX = "run."
...
class OnlineResponseValidator:
    ...
    def _complete_frame(self, header: dict, payload: bytes | None) -> None:
        ...
        if kind == "bulk-declaration":
            ...
            if not isinstance(archive_bytes, int) or archive_bytes < 0 or \
                    archive_bytes > MAX_ARCHIVE_BYTES:
                self._reject("bulk declaration exceeds frozen archive cap")
            if declaration.get("chunk_bytes") != CHUNK_BYTES or \
                    not isinstance(chunk_count, int) or chunk_count < 0 or \
                    chunk_count != ((archive_bytes + CHUNK_BYTES - 1) //
                                    CHUNK_BYTES if isinstance(
                                        archive_bytes, int) and
                                    archive_bytes >= 0 else -1) or \
                    not isinstance(declaration.get("inventory"), dict):
                self._reject("bulk declaration shape is invalid")
        ...
        if process_refusal is not None:
            ...
            if not isinstance(process_refusal, dict) or \
                    ...
                    process_refusal.get("schema") != (
                        "libwebp-backref-cost-attribution-v12-process-refusal-v1") or \
...
def parse_response_file(raw_path: Path, archive_path: Path, *,
    ...
    with raw_path.open("rb") as source, archive_path.open("xb") as archive:
        if _read_exact(source, len(MAGIC)) != MAGIC:
            raise FrameRefusal("frame-stream magic/version mismatch")
        while True:
            frame = read_frame(source, payload_cap=MAX_CONTROL_PAYLOAD_BYTES)
            if frame is None:
                break
...
def _safe_root(home: Path) -> Path:
    ...
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
        ...
        raise FrameRefusal("v12 safe root is not a canonical real HOME child")
    return root
...
def _validate_child(home: Path, root: Path, child: Path) -> None:
    if root != home / SAFE_ROOT_BASENAME or child.parent != root or \
        ...
        raise FrameRefusal("run directory is not the exact v12 safe-root child")
    info = os.lstat(child)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
...
def _admit_source(child: Path, bundle: Path, metadata: dict) -> dict:
    ...
    return {
        "schema": "libwebp-backref-cost-attribution-v12-source-admission-receipt-v1",
...
def remote_main() -> int:
    ...
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-attribution-v12-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-attribution-v12-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V12_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V12_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V12_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_attribution_v12_experiment.py", mode,
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-attribution-v12-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_attribution_v12_archive.py", "build",
    ...
    try:
        ...
        if archive is not None and declaration is not None:
            ...
            for chunk_index, offset in enumerate(range(0, len(archive), CHUNK_BYTES)):
                ...
                output.flush()
                sequence += 1
        integrity = {"schema": "libwebp-backref-cost-attribution-v12-integrity-footer-v1",
        ...
        if child is not None:
            try:
                ...
                if not absent:
                    raise FrameRefusal("exact v12 child remains after cleanup")
                cleanup = {
                    "schema": "libwebp-backref-cost-attribution-v12-cleanup-receipt-v1",
