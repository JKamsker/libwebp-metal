#!/usr/bin/env python3
"""Untimed live pandocs-agent validation of the exact v2 return protocol."""
...
import backref_cost_interval_specialization_v1_archive as protocol
import execute_backref_cost_interval_specialization_v1 as driver
...
(evidence/'lease-record.json').write_text('{"schema":"libwebp-backref-cost-interval-specialization-v1-lease-v1","path":"validation-only","acquired":false,"released":false,"acquisition_attempted":false}\n')
...
(evidence/'artifact-hashes.json').write_text(json.dumps({"schema":"libwebp-backref-cost-interval-specialization-v1-artifact-hashes-v1","artifacts":rows},indent=2,sort_keys=True)+'\n')
...
for label in ('symlink','hardlink','fifo'):
  ...
  (bad/'artifact-hashes.json').write_text(json.dumps({"schema":"libwebp-backref-cost-interval-specialization-v1-artifact-hashes-v1","artifacts":rows},indent=2,sort_keys=True)+'\n')
'''
...
def main() -> int:
    ...
    cleanup = None
    with tempfile.TemporaryDirectory(prefix="interval-specialization-v1-live-return-") as raw:
        temporary = Path(raw)
        transfers = {}
        builder_rejections = {}
        ...
        report = {
            "schema": "libwebp-backref-cost-interval-specialization-v1-live-return-validation-v1",
