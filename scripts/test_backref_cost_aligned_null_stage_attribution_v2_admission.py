#!/usr/bin/env python3
"""Untimed stage-attribution-v2 path, admission, frame, and mode tests."""
...
from backref_cost_aligned_null_stage_attribution_v2_paths import (
...
def path_tests() -> None:
    ...
    assert SAFE_ROOT_BASENAME.endswith(
        "backref-cost-aligned-null-stage-attribution-v2")
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v2-path-home-") as raw:
...
def repository_context_admission() -> None:
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v2-admit-") as raw:
        root = Path(raw)
        ...
        subprocess.run(["git", "-C", str(repository), "config", "user.name",
                        "stage-attribution-v2 admission test"], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.email",
                        "stage-attribution-v2@example.invalid"], check=True)
...
def frame_and_receipt_contracts() -> None:
    ...
    for changed in (
        ...
        rejected(executor.validate_source_admission_details, changed,
                 requested, valid)
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v2-details-") as raw:
        extracted = Path(raw)
...
def remote_refusal_cleanup_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v2-remote-home-") as raw:
...
def indexed_detail_contract() -> None:
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v2-indexed-detail-") as raw:
...
def main() -> int:
    ...
    print("PASS: specialization aligned null stage attribution v2 canonical-only paths, repository-context source "
