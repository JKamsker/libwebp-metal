#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v8."""
...
import run_backref_cost_attribution_v8_experiment as operator
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV8ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV8RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V8_EXPERIMENT=1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v8-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-attribution-v8-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v8-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v8-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "fea767272f7d9d13e5e5f19cb910e6152418dcdd"
    ...
    print("PASS: attribution v8 frozen semantics, strict single-session frames, "
