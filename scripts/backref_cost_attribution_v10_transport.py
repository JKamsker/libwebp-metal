#!/usr/bin/env python3
"""Framed single-session transport for backref-cost attribution v10.
...
removes exactly one validated v10 safe-root child.  It never discovers a PID for
...
MAGIC = b"LIBWEBP-BACKREF-COST-ATTRIBUTION-V10-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 1
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-attribution-v10"
RUN_PREFIX = "run."
...
# Every member of the transaction's self-to-PID-1 chain must match one of these
# frozen raw strings or lexical basenames.  The slash-bearing raw strings are
# process-table fixtures, not filesystem locations.  In particular,
# ``/sbin/launchd`` is the exact v9 live refusal shape.
EXPECTED_TRANSPORT_ANCESTOR_RAW_COMM_IDENTITIES = frozenset({
...
EXPECTED_TRANSPORT_ANCESTOR_LEXICAL_BASENAMES = frozenset({
    "Python", "Python3", "python", "python3",
    "ssh", "sshd", "sshd-session",
    "sh", "bash", "zsh", "launchd",
})
...
def deterministic_payload(size: int) -> bytes:
    domain = b"libwebp-backref-cost-attribution-v10-rehearsal\x00"
    output = bytearray()
    counter = 0
...
def _safe_root(home: Path) -> Path:
    ...
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
        ...
        raise FrameRefusal("v10 safe root is not a canonical real HOME child")
    return root
...
def _validate_child(home: Path, root: Path, child: Path) -> None:
    if root != home / SAFE_ROOT_BASENAME or child.parent != root or \
        ...
        raise FrameRefusal("run directory is not the exact v10 safe-root child")
    info = os.lstat(child)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
...
def classify_process_snapshot(snapshot: bytes, self_pid: int) -> dict:
    ...
    for row in rows.values():
        lexical_raws.setdefault(row["lexical_basename"], set()).add(
            row["raw_comm"])
    ancestor_pids = []
    seen = set()
    current = self_pid
    while True:
        ...
        raw_match = row["raw_comm"] in \
            EXPECTED_TRANSPORT_ANCESTOR_RAW_COMM_IDENTITIES
        lexical_match = row["lexical_basename"] in \
            EXPECTED_TRANSPORT_ANCESTOR_LEXICAL_BASENAMES
        if not raw_match and not lexical_match:
            raise FrameRefusal("transaction ancestor executable is not allowed")
        if not raw_match and len(lexical_raws[row["lexical_basename"]]) != 1:
            raise FrameRefusal("transaction ancestor lexical identity collides")
        if row["ppid"] == 0:
            if current != 1:
...
def _admit_source(child: Path, bundle: Path, metadata: dict) -> dict:
    ...
    return {
        "schema": "libwebp-backref-cost-attribution-v10-source-admission-receipt-v1",
...
def remote_main() -> int:
    ...
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-attribution-v10-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-attribution-v10-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V10_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V10_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V10_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_attribution_v10_experiment.py", mode,
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-attribution-v10-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_attribution_v10_archive.py", "build",
    ...
    try:
        ...
        if archive is not None and declaration is not None:
            ...
            for chunk_index, offset in enumerate(range(0, len(archive), CHUNK_BYTES)):
                ...
                output.flush()
                sequence += 1
        integrity = {"schema": "libwebp-backref-cost-attribution-v10-integrity-footer-v1",
        ...
        if child is not None:
            try:
                ...
                if not absent:
                    raise FrameRefusal("exact v10 child remains after cleanup")
                cleanup = {
                    "schema": "libwebp-backref-cost-attribution-v10-cleanup-receipt-v1",
