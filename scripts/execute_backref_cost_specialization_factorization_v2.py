#!/usr/bin/env python3
"""Frozen local driver for one specialization-factorization-v2 single-session transaction."""
...
import backref_cost_specialization_factorization_v2_admission as admission
import backref_cost_specialization_factorization_v2_archive as archive_protocol
...
PROTOCOL_PATH = ROOT / "doc" / "backref-cost-specialization-factorization-v2-protocol-20260819.md"
TRANSPORT_PATH = ROOT / "scripts" / "backref_cost_specialization_factorization_v2_transport.py"
PAYLOAD_GENERATOR_PATH = \
    ROOT / "scripts" / "backref_cost_specialization_factorization_v2_payload.py"
AUTHORIZATION_PATH = (ROOT / "evidence" / "backref-cost-specialization-factorization-v2-design" /
                      "phase1b-authorization.json")
REHEARSAL_HASH_INDEX_PATH = (
    ROOT / "evidence" / "backref-cost-specialization-factorization-v2-design" /
    "phase1b-rehearsal-hashes.json")
REHEARSAL_ROOT = (ROOT / "evidence" / "backref-cost-specialization-factorization-v2-design" /
                  "rehearsal")
EXPECTED_ENV = "EXPECTED_FREEZE_COMMIT"
AUTHORIZATION_SCHEMA = \
    "libwebp-backref-cost-specialization-factorization-v2-authorization-v1"
REHEARSAL_HASH_INDEX_SCHEMA = \
    "libwebp-backref-cost-specialization-factorization-v2-phase1b-hash-index-v1"
...
def _allowed_phase1b_path(path: str) -> bool:
    prefix = "evidence/backref-cost-specialization-factorization-v2-design/rehearsal/"
    return path.startswith(prefix) or path in {
        "evidence/backref-cost-specialization-factorization-v2-design/phase1b-rehearsal-hashes.json",
        "evidence/backref-cost-specialization-factorization-v2-design/phase1b-authorization.json",
    }
...
def validate_authority_values(*, envelope: dict | None, head_commit: str,
    ...
    if not envelope.get("timed_execution_authorized") or \
            ...
            envelope.get("phase_2_authority") != "FACTORIZATION_V2_DIAGNOSTIC_ONLY":
...
def validate_authorized_handoff(head: str) -> str:
    ...
    hashes = {
        "manifest": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_factorization_v2_manifest.json"),
        "protocol": _git_file_sha256(
            source_commit, "doc/backref-cost-specialization-factorization-v2-protocol-20260819.md"),
        "schemas": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_factorization_v2_schemas.json"),
        "transport": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_factorization_v2_transport.py"),
    }
...
def lease_transfer_contract(mode: str, attempted: bool,
    ...
    if not attempted:
        return ({"schema": "libwebp-backref-cost-specialization-factorization-v2-lease-transfer-v1",
...
def _transport_command(mode: str) -> list[str]:
    if mode == "rehearse":
        # Phase 1 is expressly local-only: exercise the exact framed server as
        # one directly retained child without resolving or contacting a host.
        return [sys.executable, str(TRANSPORT_PATH)]
    source = TRANSPORT_PATH.read_text(encoding="utf-8")
    payload_source = PAYLOAD_GENERATOR_PATH.read_text(encoding="utf-8")
    bootstrap = (
        "import sys,types;"
        "m=types.ModuleType('backref_cost_specialization_factorization_v2_payload');"
...
def remote_pipeline(output: Path, mode: str) -> dict:
    ...
    try:
        ...
        write_json(output / "transfer-cleanup.json", {
            "schema": "libwebp-backref-cost-specialization-factorization-v2-transfer-cleanup-v1",
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v2-return-") as raw:
        temporary = Path(raw)
        try:
