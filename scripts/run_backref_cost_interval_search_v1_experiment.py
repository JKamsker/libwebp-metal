#!/usr/bin/env python3
"""Frozen interval-search-v1 host operator for the backref-cost workspace interval-search v1 A/B."""
...
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_interval_search_v1_manifest.json"
...
def toolchain_admission(manifest: dict,
    ...
    with tempfile.TemporaryDirectory(prefix="interval-search-v1-toolchain-probe-") as raw:
...
def configure_and_build(manifest: dict, temporary: Path, log,
    ...
    for variant in ("baseline", "candidate", "diagnostic"):
        ...
        make = [
            ...
            "WEBP_ENABLE_METAL=0",
            "WEBP_BACKREF_COST_INTERVAL_SEARCH_V1_DETERMINISTIC_BUILD=1",
            f"{manifest['candidate']['build_flag']}=" +
            ("0" if variant == "baseline" else "1"),
            "examples/cwebp", "examples/dwebp",
        ]
        if variant == "diagnostic":
            make.append("WEBP_BACKREF_COST_INTERVAL_SEARCH_V1_RECORDER=1")
        if variant != "baseline":
...
def work_metric_gate(manifest: dict, builds: dict[str, Path], corpus: Path,
        ...
        for method in manifest["protocol"]["methods"]:
            ...
            if active:
                ...
                if (candidate["runtime_activations"] <= 0 or
                        candidate["tail_fast_paths"] <= 0 or
    ...
    write_json(output / "work-metric-summary.json", {
        ...
        "active_candidate_rows": sum(
            row["variant"] == "candidate" and row["tail_fast_paths"] > 0
            for row in rows),
...
def record_stage(output: Path, stage: str) -> None:
    write_json(output / "operator-stage.json", {
        "schema": "libwebp-backref-cost-interval-search-v1-stage-v1",
        "stage": stage,
        "captured_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    })
...
def execute(manifest: dict, output: Path) -> None:
        ...
        with lease_path.open("a+") as lease:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory(
                        prefix="backref-cost-interval-search-v1-build-") as raw:
                    temporary = Path(raw)
    ...
    (output / "operator-report.md").write_text(
        "# Remote backref-cost workspace interval-search v1 A/B operator report\n\n"
        ...
        "Only this interval-search v1 matrix is eligible. Timing was remote, serial, "
...
def controlled_run(output: Path) -> None:
    ...
    bootstrap_lease = {
        "schema": "libwebp-backref-cost-interval-search-v1-lease-v1",
    ...
    try:
        ...
        write_json(output / "refusal.json", {
            "schema": "libwebp-backref-cost-interval-search-v1-refusal-v1",
    ...
    write_json(output / "operator-status.json", {
        "schema": "libwebp-backref-cost-interval-search-v1-operator-status-v1",
        "status": status, "artifact_index_written": True,
        "controlled_exit": True,
    })
    artifact_schema = (manifest["schemas"]["artifact_hashes"] if manifest
                       else "libwebp-backref-cost-interval-search-v1-artifact-hashes-v1")
    ...
    if failure is not None:
        raise RuntimeError(
            "controlled interval-search-v1 refusal; indexed evidence is returnable") from failure
...
def rehearsal(manifest: dict, output: Path) -> None:
    ...
    fixture_results = {}
    fixtures = {
        "missing_make": {"make": "/nonexistent/interval-search-v1-fixture/make"},
    ...
    with tempfile.TemporaryDirectory(
            prefix="backref-cost-interval-search-v1-rehearsal-") as raw:
        temporary = Path(raw)
        with (output / "build.log").open("wb") as log:
            builds = configure_and_build(
                manifest, temporary, log, output, commit)
...
def refusal_fixture(manifest: dict, output: Path, fixture: str) -> None:
    ...
    overrides = {
        "missing": {"make": "/nonexistent/interval-search-v1-fixture/make"},
