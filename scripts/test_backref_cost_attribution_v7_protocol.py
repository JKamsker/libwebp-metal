#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v7."""
...
import run_backref_cost_attribution_v7_experiment as operator
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV7ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV7RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V7_EXPERIMENT=1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v7-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-attribution-v7-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v7-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v7-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "567e48ddfeacfaa65fb11a1611fba7aa7f5590a5"
    ...
    print("PASS: attribution v7 frozen semantics, strict single-session frames, "
