#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v13."""
...
import run_backref_cost_attribution_v13_experiment as operator


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_attribution_v13_manifest.json"
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV13ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV13RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V13_EXPERIMENT=1",
...
def v12_magic_timeout_audit() -> None:
    old_admission = (ROOT / "scripts/backref_cost_attribution_v12_admission.py").read_text()
    old_transport = (ROOT / "scripts/backref_cost_attribution_v12_transport.py").read_text()
    new_admission = (ROOT / "scripts/backref_cost_attribution_v13_admission.py").read_text()
    assert len(b"LIBWEBP-BACKREF-COST-ATTRIBUTION-V12-FRAMES\x00\x01\n") == 46
    assert 'elif stdout_count:' in old_admission
    assert 'candidates.append((response_deadline, "response-progress"))' in \
        old_admission
    assert 'output.write(MAGIC)\n    output.flush()' in old_transport
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-attribution-v13-archive-inventory-v1",
                           "status": "accepted"}}
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v13-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-attribution-v13-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v13-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v13-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "7700d05060803afcac030aa5452849f93515b0d8"
    ...
    semantic_identity()
    v12_magic_timeout_audit()
    framing_contracts()
    plan_contract(manifest)
    authority_rejections()
    strict_hash_index_order_contract()
    print("PASS: attribution v13 frozen semantics, strict single-session frames, "
