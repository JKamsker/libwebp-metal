#!/usr/bin/env python3
"""Untimed V3 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "086059436a46f1d5cd48d401fc06e2f96bbf095b"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V3_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V3_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V3_EXPERIMENT"
RUNNER = "backref_cost_attribution_v3_experiment_runner"
SOURCE = "backref_cost_attribution_v3_experiment_enc.c"
...
def run(argv: list[str], *, env: dict[str, str] | None = None,
    ...
    for name in (
        ...
        "WEBP_BACKREF_COST_ATTRIBUTION_V2_EXPERIMENT", RUNTIME,
    ):
        clean.pop(name, None)
    if env:
        clean.update(env)
...
def main() -> int:
    ...
    assert "V3 is deliberately outside the predecessor fallthrough chain" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v3-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V3_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V3_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V3_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V3_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
