#!/usr/bin/env python3
"""Deterministic Alignment V3 READY/request-ACK/full-duplex fixtures."""
...
BOOTSTRAP = ROOT / "scripts/backref_cost_specialization_alignment_v3_bootstrap.py"
NONCE = "a" * 64
...
def environment(channel_request: transport.ChannelRequest) -> dict[str, str]:
    ...
    result.update({
        "WEBP_ALIGNMENT_V3_TRANSACTION_NONCE": NONCE,
        "WEBP_ALIGNMENT_V3_REQUEST_DECLARATION_SHA256":
            channel_request.declaration_sha256,
        "WEBP_ALIGNMENT_V3_BOOTSTRAP_SHA256": BOOTSTRAP_SHA,
    })
    return result
...
def pump(command: list[str], channel_request: transport.ChannelRequest, *,
    ...
    temporary = tempfile.TemporaryDirectory(prefix="alignment-v3-channel-")
...
def timeout_fixture(command: list[str], channel_request: transport.ChannelRequest,
    ...
    with tempfile.TemporaryDirectory(prefix="alignment-v3-timeout-") as raw:
        root = Path(raw)
        try:
...
def source_refusal_is_not_success() -> None:
    ...
    source = (ROOT / "scripts/backref_cost_specialization_alignment_v3_transport.py").read_bytes()
    payload = (ROOT / "scripts/backref_cost_specialization_alignment_v3_payload.py").read_bytes()
    ...
    channel_request = transport.build_channel_request(
        ...
        bootstrap_sha256=BOOTSTRAP_SHA)
    with tempfile.TemporaryDirectory(prefix="alignment-v3-remote-home-") as remote_home:
