#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization aligned null stage attribution v4."""
...
import backref_cost_aligned_null_stage_attribution_v4_admission as admission
import backref_cost_aligned_null_stage_attribution_v4_transport as transport
import execute_backref_cost_aligned_null_stage_attribution_v4 as executor
import run_backref_cost_aligned_null_stage_attribution_v4_experiment as operator
...
import evaluate_backref_cost_aligned_null_stage_attribution_v4 as evaluator
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostAlignedNullStageAttributionV4GetVariant" in current
    ...
    dp_symbols = [entry.symbol for entry in identity.HOT_ENTRIES
                  if entry.role == "dp"]
    assert all(boundary.count(symbol + "(") == 1 for symbol in dp_symbols)
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostAlignedNullStageAttributionV4GetVariant()") == 1
    assert traceback.count("BackwardReferencesHashChainDistanceOnlySpecialized(") == 0
    assert all(traceback.count(symbol + "(") == 1 for symbol in dp_symbols)
    assert traceback.count(
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-archive-inventory-v1",
...
def framing_contracts() -> None:
    ...
    # Non-transport children have no signal authority; only the retained
    # request-channel pump owns the bounded transport-stop path.
    run_owned_source = inspect.getsource(admission.run_owned)
    assert "stop_after_timeout" not in run_owned_source
    assert "communicate(input_data)" in run_owned_source


def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-plan-") as raw:
        temporary = Path(raw)
        ...
        assert len(plan) == 32
        assert [row["ordinal"] for row in plan] == list(range(32))
        assert {row["variant"] for row in plan} == {"B", "L"}
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "pipeline_invocation_authorized": True,
    ...
    diff = [
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v4-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v4-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v4-design/phase1b-authorization.json")]
    kwargs = {
    ...
    for field, value in (
            ("pipeline_invocation_authorized", False),
            ("performance_sampling_authorized_before_dynamic_admission", True),
            ("timed_execution_authorized", True),
            ("dynamic_gates_deferred_to_single_phase2_pipeline", False)):
        changed = copy.deepcopy(kwargs)
...
def zero_sample_refusal_contract() -> None:
    def fail_before_admission(mode: str, output: Path) -> None:
        del mode, output
        raise RuntimeError("synthetic dynamic admission failure")

    original = operator.operator
    operator.operator = fail_before_admission
    try:
        with tempfile.TemporaryDirectory(
                prefix="stage-attribution-v4-refusal-") as raw:
            output = Path(raw) / "evidence"
            rejected(operator.controlled, "run", output)
            status = json.loads((output / "performance-evidence-status.json")
                                .read_text(encoding="utf-8"))
            lease = json.loads((output / "lease-record.json").read_text(
                encoding="utf-8"))
            assert status["status"] == \
                "REFUSED_ZERO_ACCEPTED_PERFORMANCE_EVIDENCE"
            assert status["accepted_performance_rows"] == 0
            assert status["retry_forbidden"] is True
            assert lease == {
...
def compact_source_control_freeze_contract(manifest: dict) -> None:
    ...
    assert freeze["positive_alignment_v4_compact_fits_cap"] is True
    assert "source_hashes" not in json.dumps(compact)


def v4_audit_and_null_contract(manifest: dict) -> None:
    audit = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v4-design/phase1-prior-evidence-audit.json"
    ).read_text())
    assert audit["status"] == "PASS_TRANSPORT_REQUALIFICATION_PREMISE_FROZEN"
    assert audit["performance_samples_generated"] == 0
    comparison = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v4-design/transport-comparison.json"
    ).read_text())
    assert comparison["deterministic_regression_found"] is False
    assert comparison["absent_data_inference_used"] is False
    assert comparison["frozen_consequence"]["classification"] == \
        "fresh-bounded-requalification"
    assert comparison["frozen_consequence"]["protocol_limit_changes"] == []
    assert comparison["variants"]["aligned-null-v2"]["request_chunks_acknowledged"] == 5
    assert comparison["variants"]["alignment-v4"]["request_chunks_acknowledged"] == 354
    assert comparison["variants"]["alignment-v4"]["request_chunks_acknowledged"] == 357
    assert audit["v4_timing_rows_loaded_into_v4_statistics"] == 0
    assert audit["v1_timing_rows_loaded_into_v4_statistics"] == 0
    assert audit["v1_authority_audit"]["failed_gates"] == [
        "alignment build frozen"]
    assert audit["v1_authority_audit"]["can_contribute_samples_to_v4"] is False
    assert audit["raw_index_audit"]["artifact-hashes.json"]["rows"] == 136
    assert audit["raw_index_audit"]["returned-evidence-hashes.json"]["rows"] == 152
    assert all(not row["mismatches"]
               for row in audit["raw_index_audit"].values())
    reconstruction = audit["evaluator_reconstruction"]
    assert (reconstruction["domains_passing"],
            reconstruction["domains_total"]) == (11, 12)
    assert reconstruction["sole_failure"] == {
        "cell": "graphic-m4", "metric": "whole_encode",
        "mean_percent": -0.6199563389094708,
        "ci95_percent": [-1.0795343104954225, -0.21412171812593112]}
    assert reconstruction["graphic_m4_dp_total"] == {
        "mean_percent": -0.10469717299024589,
        "ci95_percent": [-0.3269444151885643, 0.19313983030375964],
        "null_passed": True}
    assert reconstruction["graphic_m4_dp_steady"] == {
        "mean_percent": -0.09301206840802462,
        "ci95_percent": [-0.3168809078248977, 0.21050021137331046],
        "null_passed": True}
    assert audit["v1_authority_audit"]["stale_expected_identity"] == {
        "private_address_count": 6, "section_size_bytes": 83212,
        "body_bytes": 5624, "padding_bytes": 77588}
    assert audit["v1_authority_audit"]["valid_v1_identity"] == {
        "private_address_count": 4, "section_size_bytes": 50444,
        "body_bytes": 3600, "padding_bytes": 46844}
    assert not audit["timer_boundary_classification"][
        "attribution_suitable_v4_timers"]
    bounds = manifest["acceptance"]["mandatory_b_to_l_null_control"]
    passing = {"mean_percent": 0.1,
               "bootstrap_mean_ci_percent": [-0.2, 0.3],
               "paired_percentages": [-0.4, 0.2] * 6}
    assert evaluator.null_control(passing, bounds)["passes"]
    fixtures = [
        {**passing, "mean_percent": 0.751},
        {**passing, "bootstrap_mean_ci_percent": [0.01, 0.3]},
        {**passing, "bootstrap_mean_ci_percent": [-1.501, 0.3]},
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "92c3b59b2d178c5a03ec20b897252b358f07de81"
    assert manifest["candidate"]["build_flag"] == \
        "WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_EXPERIMENT"
    assert manifest["candidate"]["runtime_flag"] == \
        "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_VARIANT"
    assert manifest["protocol_version"] == transport.PROTOCOL_VERSION == 1
    assert manifest["protocol"]["profile_processes"] == 32
    assert manifest["protocol"]["warmups_per_profile"] == 1
    assert manifest["protocol"]["retained_encodes_per_profile"] == 4
    assert manifest["protocol"]["retained_stage_records"] == 128
    assert manifest["protocol"]["runner_output_records"] == 160
    assert manifest["protocol"]["stage_records"] == 160
    assert manifest["dynamic_admission"]["gate_a"]["count"] == 16
    assert manifest["dynamic_admission"]["gate_b"]["accounting_rows"] == 2
    assert manifest["dynamic_admission"]["gate_c"][
        "planned_runner_children"] == 59
    assert manifest["dynamic_admission"]["gate_c"]["breakdown"] == {
        "correctness": 16, "clock": 1, "timer_accounting": 2,
        "work": 8, "profile": 32}
    assert manifest["two_level_authority_contract"][
        "phase_1b_authority_if_rehearsal_passes"] == \
        "ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CONDITIONAL_DIAGNOSTIC_AUTHORITY"
    assert "16 total" in manifest["protocol"]["correctness"]
    assert "8 B/L work rows" in manifest["protocol"][
        "selected_dp_structure"]
    assert "100000-resample" in manifest["protocol"]["statistics"]
    assert "0x4e554c33" in manifest["protocol"]["statistics"]
    assert manifest["protocol"]["alignment_boundary_bytes"] == 16384
    assert manifest["protocol"]["entry_order"] == identity.names()
    assert manifest["four_body_identity"] == identity.generator_binding()
    assert evaluator.BOOTSTRAP_SEED == 0x4E554C33
    expected_receipt = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v4-design" /
        "four-body-expected-receipt.json").read_text())
    identity.validate_machine_receipt(
        expected_receipt, manifest["four_body_receipt_bindings"])
    assert "B and L" in manifest["protocol"]["pairing"]
    assert "no H interpretation" in manifest["acceptance"][
    ...
    evaluator_source = (ROOT / "scripts" /
        "evaluate_backref_cost_aligned_null_stage_attribution_v4.py").read_text()
    assert "100000" in evaluator_source
    assert "graphic-m4" in evaluator_source
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_aligned_null_stage_attribution_v4.py run '
                '"$PWD/evidence/backref-cost-aligned-null-stage-attribution-v4-execution-20260820"')
    ...
    zero_sample_refusal_contract()
    strict_hash_index_order_contract()
    compact_inventory_contract()
    compact_source_control_freeze_contract(manifest)
    print("PASS: specialization aligned null stage attribution v4 frozen semantics, strict single-session frames, "
          "all refusal fixtures, authority separation, and 32-command plan")
    return 0
