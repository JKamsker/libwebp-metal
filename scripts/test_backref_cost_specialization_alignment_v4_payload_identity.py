#!/usr/bin/env python3
"""Fail-closed alignment-v4 representative-payload triple-agreement gate."""
...
import backref_cost_specialization_alignment_v4_payload as generator
...
FIXTURE = (ROOT / "evidence/backref-cost-specialization-alignment-v4-design" /
           "representative-payload-fixture.json")
RECEIPT = (ROOT / "evidence/backref-cost-specialization-alignment-v4-design" /
           "representative-payload-agreement.json")


def independent_digest(size: int) -> str:
    domain = b"libwebp-backref-cost-specialization-alignment-v4-rehearsal\x00"
...
def main() -> int:
    ...
    stale_v3 = generator.STALE_V3_SHA256
    ...
    stale_alignment_v3 = generator.STALE_ALIGNMENT_V3_SHA256
    assert stale_v2 == fixture["stale_v2_sha256"]
    assert stale_v3 == fixture["stale_v3_sha256"]
    ...
    assert stale_alignment_v3 == fixture["stale_alignment_v3_sha256"]
    assert stale_v2 not in observed and stale_v3 not in observed and \
        stale_v4 not in observed and stale_alignment_v1 not in observed and \
        stale_alignment_v2 not in observed and stale_alignment_v3 not in observed
    assert receipt["negative_stale_v2_test"] == "PASS_REJECTED"
    assert receipt["negative_stale_v3_test"] == "PASS_REJECTED"
