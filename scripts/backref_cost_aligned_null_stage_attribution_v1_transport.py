#!/usr/bin/env python3
"""Framed single-session transport for aligned-null stage attribution v1.
...
removes exactly one validated V1 safe-root child.  It never discovers a PID for
...
import backref_cost_aligned_null_stage_attribution_v1_payload as payload_generator


MAGIC = b"LIBWEBP-BACKREF-COST-ALIGNED-NULL-STAGE-ATTRIBUTION-V1-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 1
CHANNEL_MAGIC = b"LIBWEBP-BACKREF-COST-ALIGNED-NULL-STAGE-ATTRIBUTION-V1-CHANNEL\x00\x01\n"
CHANNEL_READY_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-ready-v1"
REQUEST_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-request-declaration-v1"
REQUEST_ACK_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-request-ack-v1"
RESPONSE_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-response-declaration-v1"
RESPONSE_COMPLETE_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-response-complete-v1"
...
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-aligned-null-stage-attribution-v1"
RUN_PREFIX = "run."
...
BULK_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-archive-v1"
SOURCE_RECEIPT_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-source-admission-receipt-v1"
SOURCE_DETAILS_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v1-source-admission-details-v1"
    ...
    def _valid_bulk_declaration(self, declaration: dict) -> bool:
        ...
        valid = (
            ...
            inventory.get("schema") ==
                "libwebp-backref-cost-aligned-null-stage-attribution-v1-archive-inventory-v1" and
        ...
        if process_refusal is not None:
            ...
            if not isinstance(process_refusal, dict) or \
                    ...
                    process_refusal.get("schema") != (
                        "libwebp-backref-cost-aligned-null-stage-attribution-v1-process-refusal-v1") or \
...
def _validate_child(home: Path, root: Path, child: Path) -> None:
    if root != home / SAFE_ROOT_BASENAME or child.parent != root or \
        ...
        raise FrameRefusal("run directory is not the exact V1 safe-root child")
    info = os.lstat(child)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
...
def remote_transaction(input_stream) -> tuple[int, bytes]:
    ...
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_aligned_null_stage_attribution_v1_experiment.py", mode,
        ...
        _add_indexed_detail(
            evidence, "remote-admission-process-isolation.json", {
                "schema":
                    "libwebp-backref-cost-aligned-null-stage-attribution-v1-remote-process-isolation-v1",
                "status": "verified",
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_aligned_null_stage_attribution_v1_archive.py", "build",
    ...
    integrity = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-integrity-footer-v1",
    ...
    if child is not None:
        try:
            ...
            if not absent:
                raise FrameRefusal("exact V1 child remains after cleanup")
            cleanup = {
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-cleanup-receipt-v1",
