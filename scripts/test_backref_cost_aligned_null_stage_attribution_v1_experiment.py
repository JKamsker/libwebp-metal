...
BASE = "5de7b3848adfdb5d1058a061979535f05f976e5b"
BUILD = "WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_EXPERIMENT"
RECORDER = "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_RECORDER"
MACRO = "WEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_VARIANT"
RUNNER = "backref_cost_aligned_null_stage_attribution_v1_experiment_runner"
...
def main() -> int:
    ...
    assert "VP8LBackrefCostAlignedNullStageAttributionV1GetVariant" in source
    assert "switch (variant)" in source
    assert not run(["git", "diff", "--name-only", BASE, "--", "src/webp",
                    "sharpyuv"]).stdout.strip()

    with tempfile.TemporaryDirectory(
            prefix="backref-aligned-null-stage-v1-test-") as raw:
        temporary = Path(raw)
        ...
        assert b"SPECIALIZATION_ALIGNED_NULL_STAGE_ATTRIBUTION_V1" not in ordinary_archive
        ...
        candidate_symbols = ("PushInterval", "PushIntervalLayoutClone",
                       "BackwardReferencesHashChainDistanceOnly",
                       "BackwardReferencesHashChainDistanceOnlyLayoutClone")
        ...
        assert len(private_addresses) == 4
        assert len(set(private_addresses.values())) == 4
        assert "AlignedNullStageAttributionV1RecordPush" not in timed_nm
        assert "AlignedNullStageAttributionV1RecordPush" in recorder_nm
        ...
        for dp, push in zip(expected_order[2:], expected_order[:2]):
            assert address_values[dp] - address_values[push] == 2 * 16384
