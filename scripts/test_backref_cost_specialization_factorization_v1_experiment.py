...
BASE = "c6046dc7cde6e7ea1cc9d54a6a90c55f8a2f2edb"
BUILD = "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_EXPERIMENT"
RECORDER = "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_RECORDER"
MACRO = "WEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_VARIANT"
RUNNER = "backref_cost_specialization_factorization_v1_experiment_runner"
...
def main() -> int:
    ...
    assert "VP8LBackrefCostSpecializationFactorizationV1GetVariant" in source
    assert "switch (variant)" in source
    assert not run(["git", "diff", "--name-only", BASE, "--", "src/webp",
                    "sharpyuv"]).stdout.strip()

    with tempfile.TemporaryDirectory(
            prefix="backref-specialization-factorization-v1-test-") as raw:
        temporary = Path(raw)
        ...
        assert "FactorizationV1RecordPush" not in timed_nm
        assert "FactorizationV1RecordPush" in recorder_nm
