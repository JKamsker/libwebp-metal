#!/usr/bin/env python3
"""Framed single-session transport for backref-cost attribution v14.
...
removes exactly one validated v14 safe-root child.  It never discovers a PID for
...
MAGIC = b"LIBWEBP-BACKREF-COST-ATTRIBUTION-V14-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 6
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-attribution-v14"
RUN_PREFIX = "run."
...
BULK_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-attribution-v14-archive-v1"
...
class ProcessRefusal(FrameRefusal):
    ...
    def __init__(self, failure_code: str, *, row: dict | None = None,
        ...
        self.diagnostic = {
            "schema":
                "libwebp-backref-cost-attribution-v14-process-refusal-v1",
...
def deterministic_payload_chunks(size: int,
    ...
    # calibration and the V14 margin comparison remain non-circular.
    ...
    def _valid_bulk_declaration(self, declaration: dict) -> bool:
        ...
        valid = (
            ...
            inventory.get("schema") ==
                "libwebp-backref-cost-attribution-v14-archive-inventory-v1" and
        ...
        if process_refusal is not None:
            ...
            if not isinstance(process_refusal, dict) or \
                    ...
                    process_refusal.get("schema") != (
                        "libwebp-backref-cost-attribution-v14-process-refusal-v1") or \
...
def _safe_root(home: Path) -> Path:
    ...
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
        ...
        raise FrameRefusal("v14 safe root is not a canonical real HOME child")
    return root
...
def _validate_child(home: Path, root: Path, child: Path) -> None:
    if root != home / SAFE_ROOT_BASENAME or child.parent != root or \
        ...
        raise FrameRefusal("run directory is not the exact v14 safe-root child")
    info = os.lstat(child)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
...
def _admit_source(child: Path, bundle: Path, metadata: dict) -> dict:
    ...
    return {
        "schema": "libwebp-backref-cost-attribution-v14-source-admission-receipt-v1",
...
def _emit_encoded(stream, encoded: bytes, response_bytes: list[int]) -> None:
    if response_bytes[0] + len(encoded) > MAX_RESPONSE_BYTES:
        raise FrameRefusal("framed response exceeds frozen total cap")
    stream.write(encoded)
    stream.flush()


def _emit_json(stream, kind: str, value: dict, sequence: int,
               response_bytes: list[int]) -> int:
    payload = canonical_json(value)
    if len(payload) > MAX_CONTROL_PAYLOAD_BYTES:
        raise FrameRefusal(f"{kind} exceeds frozen control cap")
    encoded = encode_frame(kind, payload, sequence=sequence)
    _emit_encoded(stream, encoded, response_bytes)
    response_bytes[0] += len(encoded)
    return sequence + 1
...
def remote_main() -> int:
    ...
    output.write(MAGIC)
    output.flush()
    response_bytes = [len(MAGIC)]
    child = None
    home = root = None
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-attribution-v14-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-attribution-v14-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V14_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V14_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V14_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_attribution_v14_experiment.py", mode,
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-attribution-v14-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_attribution_v14_archive.py", "build",
    ...
    try:
        sequence = _emit_json(
            output, "source-admission", source_receipt, sequence, response_bytes)
        sequence = _emit_json(
            output, "operator-status", operator_status, sequence, response_bytes)
        sequence = _emit_json(
            output, "archive-build", archive_status, sequence, response_bytes)
        sequence = _emit_json(
            output, "lease-state", lease_state, sequence, response_bytes)
        if archive is not None and declaration is not None:
            sequence = _emit_json(
                output, "bulk-declaration", declaration, sequence, response_bytes)
            for chunk_index, offset in enumerate(range(0, len(archive), CHUNK_BYTES)):
                ...
                _emit_encoded(output, encoded, response_bytes)
                response_bytes[0] += len(encoded)
                sequence += 1
        integrity = {"schema": "libwebp-backref-cost-attribution-v14-integrity-footer-v1",
        ...
        sequence = _emit_json(
            output, "integrity-footer", integrity, sequence, response_bytes)
        ...
        if child is not None:
            try:
                ...
                if not absent:
                    raise FrameRefusal("exact v14 child remains after cleanup")
                cleanup = {
                    "schema": "libwebp-backref-cost-attribution-v14-cleanup-receipt-v1",
        ...
        try:
            _emit_json(
                output, "cleanup-footer", cleanup, sequence, response_bytes)
