#!/usr/bin/env python3
"""Local identity and direct-child ownership for attribution-v15 transport."""
...
IDENTITY_PATHS = (
    ...
    "src/enc/vp8l_enc.c",
    "src/enc/backref_cost_attribution_v15_experiment_enc.c",
    "src/enc/backref_cost_attribution_v15_experiment_enc.h",
    "tools/backref_cost_attribution_v15_experiment_runner.c",
...
PROTOCOL_MANIFEST = ROOT / "scripts" / "backref_cost_attribution_v15_manifest.json"
SOURCE_RECEIPT_SCHEMA = \
    "libwebp-backref-cost-attribution-v15-source-admission-receipt-v1"
...
class TransportDeadlineStateMachine:
    """Pure V15 transport deadline attribution with injected monotonic time."""
...
def pump_owned_full_duplex(
    ...
    if stdout_observer is None or not hasattr(stdout_observer,
                                               "transport_state"):
        raise ValueError("V15 transport requires an online state observer")
...
def create_source_bundle() -> tuple[bytes, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v15-bundle-") as raw:
