#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost specialization aligned null stage attribution v2."""
...
from backref_cost_aligned_null_stage_attribution_v2_admission import (
...
def clean_environment(manifest: dict, variant: str,
    ...
    if stage_output is not None:
        env.update({
            ...
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_TIMERS": "1",
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    def one(name: str, recorder: bool) -> dict:
        ...
        argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
                "WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_EXPERIMENT=1",
                f"WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_RECORDER={int(recorder)}",
                f"CFLAGS={flags}",
                f"LDFLAGS={manifest['build']['ldflags']},-map,{build_root / 'linker.map'}",
                "tools/backref_cost_aligned_null_stage_attribution_v2_experiment_runner"]
    ...
    if "AlignedNullStageAttributionV2Record" in timed["nm"] or \
            "AlignedNullStageAttributionV2Stats" in timed["nm"]:
        raise RuntimeError("timed binary contains recorder symbols")
    if "AlignedNullStageAttributionV2RecordPush" not in recorder["nm"] or \
    ...
    write_json(output / "codegen.json", {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v2-codegen-v1",
...
def timer_accounting_gate(manifest: dict, timed_runner: Path,
    ...
    if record.get("schema") != \
            "libwebp-backref-cost-aligned-null-stage-attribution-v2-timer-check-v1" or \
    ...
    for variant in ("B", "L"):
        ...
        env["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_SAMPLE_SET"] = \
            "timer-validation"
        result = ledger.run(
        ...
        if emitted[0].get("schema") != \
                "libwebp-backref-cost-aligned-null-stage-attribution-v2-output-v1" or \
                output_row.get("schema") != \
                "libwebp-backref-cost-aligned-null-stage-attribution-v2-timer-accounting-output-v1" or \
                stage.get("schema") != \
                "libwebp-backref-cost-aligned-null-stage-attribution-v2-stage-ledger-v1" or \
        ...
        accounting.append({"variant": variant, "output": output_row,
                           "stage": stage})
    summary = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v2-timer-accounting-summary-v1",
...
def work_counter_gate(manifest: dict, runner: Path, corpus: Path,
    ...
    for case in manifest["corpus"]:
        ...
        for method in manifest["protocol"]["methods"]:
            ...
            if any(row.get("schema") !=
                   "libwebp-backref-cost-aligned-null-stage-attribution-v2-work-v1"
    ...
    write_json(output / "work-metric-summary.json", {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v2-work-summary-v1",
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path,
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-aligned-null-stage-attribution-v2-output-v1"
               for item in outputs):
...
def admit_representative_payload(path: Path, expected_minimum: int) -> dict:
    ...
    source_raw = os.environ.get(
        "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_REHEARSAL_PAYLOAD")
    expected_sha = os.environ.get(
        "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_REHEARSAL_PAYLOAD_SHA256")
    expected_size_raw = os.environ.get(
        "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_REHEARSAL_PAYLOAD_BYTES")
    ...
    with tempfile.TemporaryDirectory(prefix="backref-aligned-null-stage-v2-") as raw:
        ...
        if mode == "rehearse":
            ...
            write_json(output / "performance-evidence-status.json", {
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v2-performance-evidence-status-v1",
            ...
            with lease_path.open("a+") as lease:
                ...
                try:
                    ...
                    write_json(output / "dynamic-admission.json", {
                        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v2-dynamic-admission-v1",
