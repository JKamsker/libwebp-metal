#!/usr/bin/env python3
"""Static, workload-free ownership guards for specialization aligned null stage attribution v4."""
...
import run_backref_cost_aligned_null_stage_attribution_v4_experiment as operator_module


ROOT = Path(__file__).resolve().parents[1]
INFRASTRUCTURE = (
    "scripts/backref_cost_aligned_null_stage_attribution_v4_admission.py",
    "scripts/backref_cost_aligned_null_stage_attribution_v4_archive.py",
    "scripts/backref_cost_aligned_null_stage_attribution_v4_paths.py",
    "scripts/backref_cost_aligned_null_stage_attribution_v4_bootstrap.py",
    "scripts/backref_cost_aligned_null_stage_attribution_v4_transport.py",
    "scripts/execute_backref_cost_aligned_null_stage_attribution_v4.py",
    "scripts/run_backref_cost_aligned_null_stage_attribution_v4_experiment.py",
)
...
def static_guards() -> None:
    ...
    assert signal_sites == [
        ("scripts/backref_cost_aligned_null_stage_attribution_v4_admission.py", "self.process.kill")]
    assert popen_sites == [
        "scripts/backref_cost_aligned_null_stage_attribution_v4_admission.py",
        "scripts/backref_cost_aligned_null_stage_attribution_v4_admission.py"]
...
def direct_child_contract() -> None:
    ...
    owned = admission._OwnedChild(process, process.pid, id(process), True)
    owned.stop_after_timeout()
    changed = admission._OwnedChild(process, process.pid + 1, id(process), True)
    try:
    ...
    non_transport = admission._OwnedChild(process, process.pid, id(process))
    try:
        non_transport.stop_after_timeout()
    except RuntimeError as error:
        assert "only the retained transport child" in str(error)
    else:
        raise AssertionError("non-transport child gained signal authority")


def online_runner_ownership_fixtures() -> None:
    base = {
        "schema": admission.CHILD_START_SCHEMA, "ordinal": 0,
        "pid": 501, "ppid_observed": 77,
        "creation_identity": "a" * 48, "start_token": "123456789",
        "variant": "B", "case_id": "photo-large", "method": 4,
        "lifecycle": "repeat-0", "purpose": "correctness-before-timing",
        "shell": False, "wrapper": False, "grandchild": False,
    }

    def validate(receipt: dict, prior: list[dict] | None = None) -> None:
        admission.validate_runner_start_receipt(
            receipt, ordinal=0, pid=501, parent_pid=77,
            creation_identity="a" * 48, variant="B",
            case_id="photo-large", method=4, lifecycle="repeat-0",
            purpose="correctness-before-timing", prior=prior or [])
    ...
    mutations = {
        ...
        "pid": {"pid": 777},
        "ppid": {"ppid_observed": 1},
        "creation identity": {"creation_identity": "b" * 48},
        "start token": {"start_token": "invalid"},
    }
    ...
    try:
        validate(base, [{"ordinal": 0, "pid": 400,
                         "start_token": "1"}])
    except RuntimeError as error:
        assert "duplicate" in str(error)
    else:
        raise AssertionError("duplicate ordinal was accepted")
    try:
        validate(base, [{"ordinal": 9, "pid": 501,
                         "start_token": "123456789"}])
    except RuntimeError as error:
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-ledger-") as raw:
        root = Path(raw)
        ledger = admission.DirectRunnerLedger(
            root / "ledger.jsonl", (Path(sys.executable),))
        ledger.live = Live()
        try:
            ledger.run([sys.executable], cwd=root, env={}, variant="B",
                       case_id="fixture", method=0, lifecycle="fixture",
                       purpose="overlap")
        except RuntimeError as error:
            assert "overlap" in str(error)
        else:
            raise AssertionError("overlap attempt was accepted")
        ledger.live = None
        try:
            ledger.run(["/bin/sh", "-c", "true"], cwd=root, env={},
                       variant="B", case_id="fixture", method=0,
                       lifecycle="fixture", purpose="wrapper")
        except RuntimeError as error:
            assert "wrapper mismatch" in str(error)
        else:
            raise AssertionError("shell wrapper was accepted")
        ledger.receipts = [{"ordinal": 0, "exited": True, "reaped": False}]
        try:
            ledger.finalize(1)
        except RuntimeError as error:
            assert "incomplete" in str(error)
        else:
            raise AssertionError("missing reap was accepted")

    source = inspect.getsource(admission.DirectRunnerLedger)
    assert "shell=False" in source and "start_new_session=False" in source
    assert "process.stdin.close()  # EOF" in source
    assert not any(token in source for token in (
        ".kill()", ".terminate()", "os.kill", "killpg"))
...
def identity_only_process_contract() -> None:
    ...
    ignored_ancestor_argv_and_source = (
        "python3 -c 'backref_cost_aligned_null_stage_attribution_v4_experiment_runner "
    ...
    lookalikes = (
        ...
        "prefix_backref_cost_aligned_null_stage_attribution_v4_experiment_runner",
        "backref_cost_aligned_null_stage_attribution_v4_experiment_runner_suffix",
...
def operator_refusal_control_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-refusal-") as raw:
...
def single_session_contract() -> None:
    identities = {
        ...
        "request_declaration_sha256": "b" * 64,
        "bootstrap_sha256": "c" * 64,
    }
    command = executor._transport_command(**identities)
    phase2_command = executor._transport_command(**identities)
    assert command == phase2_command
    assert command[0] == "/usr/bin/ssh"
    assert command.count("/usr/bin/ssh") == 1
    assert "BatchMode=yes" in phase2_command
    assert "ConnectionAttempts=1" in phase2_command
    assert "ConnectTimeout=30" in phase2_command
    assert "ServerAliveInterval=15" in phase2_command
    assert "ServerAliveCountMax=3" in phase2_command
    assert "xcrun" not in command
    source = (ROOT / "scripts/execute_backref_cost_aligned_null_stage_attribution_v4.py").read_text()
    ...
    remote = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_transport.py").read_text()
    ...
    assert "pump_owned_request_channel" in source
    assert "request-declaration" in source
    assert "channel_ready_reached" in source
    assert "transport_source" not in command[-1]
    assert "mode == \"rehearse\"" in source


def main() -> int:
    static_guards()
    direct_child_contract()
    online_runner_ownership_fixtures()
    identity_only_process_contract()
    refusal_frame_contract()
    operator_refusal_control_contract()
    single_session_contract()
    print("PASS: specialization aligned null stage attribution v4 proves arbitrary opaque ancestors structurally, "
          ...
          "in-session cleanup plus fail-closed serial direct-runner ownership")
    return 0
