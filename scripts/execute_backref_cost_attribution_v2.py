...
import backref_cost_attribution_v2_admission as admission
import backref_cost_attribution_v2_archive as archive_protocol


ROOT = Path(__file__).resolve().parents[1]
HOST = admission.HOST
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_attribution_v2_manifest.json"
EXPECTED_ENV = "WEBP_EXPECTED_BACKREF_COST_ATTRIBUTION_V2_COMMIT"
...
def remote_pipeline(output: Path, mode: str,
    ...
    if not MANIFEST_PATH.is_file():
        raise RuntimeError("frozen attribution-v2 manifest is absent")
    ...
    temporary_holder = tempfile.TemporaryDirectory(prefix="attribution-v2-return-")
    temporary = Path(temporary_holder.name)
    try:
        remote_q = shlex.quote(record["run_directory"])
        operator = "scripts/run_backref_cost_attribution_v2_experiment.py"
