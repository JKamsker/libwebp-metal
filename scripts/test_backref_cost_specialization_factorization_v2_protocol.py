#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization factorization v2."""
...
import run_backref_cost_specialization_factorization_v2_experiment as operator
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostSpecializationFactorizationV2GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostSpecializationFactorizationV2GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_specialization_factorization_v2_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-specialization-factorization-v2-archive-inventory-v1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v2-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority": "FACTORIZATION_V2_DIAGNOSTIC_ONLY",
    ...
    diff = [
        ("A", "evidence/backref-cost-specialization-factorization-v2-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-specialization-factorization-v2-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-specialization-factorization-v2-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "ccdd4900633064f203010cbd437d2682e1cba51e"
    ...
    assert manifest["protocol_version"] == transport.PROTOCOL_VERSION == 7
    ...
    assert manifest["return_contract"]["single_remote_session"] is False
    assert manifest["return_contract"]["remote_interactions_per_transaction"] == 0
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_specialization_factorization_v2.py run '
                '"$PWD/evidence/backref-cost-specialization-factorization-v2-execution-20260819"')
    ...
    v15_exact_refusal_audit()
    framing_contracts()
    plan_contract(manifest)
    authority_rejections()
    strict_hash_index_order_contract()
    compact_inventory_contract()
    print("PASS: specialization factorization v2 frozen semantics, strict single-session frames, "
