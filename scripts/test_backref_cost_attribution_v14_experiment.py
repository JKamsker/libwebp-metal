#!/usr/bin/env python3
"""Untimed V14 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "e690603a84aa90448e7c6d44ebfc303920b806b3"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V14_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V14_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V14_EXPERIMENT"
RUNNER = "backref_cost_attribution_v14_experiment_runner"
SOURCE = "backref_cost_attribution_v14_experiment_enc.c"
...
def main() -> int:
    ...
    assert "V14 changes transport evidence only" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v14-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V14_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V14_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V14_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V14_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
