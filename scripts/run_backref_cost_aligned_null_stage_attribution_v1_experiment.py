#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost specialization aligned null stage attribution v1."""
...
from backref_cost_aligned_null_stage_attribution_v1_admission import (
...
def clean_environment(manifest: dict, variant: str,
    ...
    if stage_output is not None:
        env.update({
            ...
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_TIMERS": "1",
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    def one(name: str, recorder: bool) -> dict:
        ...
        argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
                "WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_EXPERIMENT=1",
                f"WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_RECORDER={int(recorder)}",
                f"CFLAGS={flags}",
                f"LDFLAGS={manifest['build']['ldflags']},-map,{build_root / 'linker.map'}",
                "tools/backref_cost_aligned_null_stage_attribution_v1_experiment_runner"]
    ...
    if "AlignedNullStageAttributionV1Record" in timed["nm"] or \
            "AlignedNullStageAttributionV1Stats" in timed["nm"]:
        raise RuntimeError("timed binary contains recorder symbols")
    if "AlignedNullStageAttributionV1RecordPush" not in recorder["nm"] or \
    ...
    if sorted(address_values, key=address_values.get) != expected_order or \
            ...
            any(address_values[expected_order[i + 1]] -
                address_values[expected_order[i]] != 16384 for i in range(3)):
        raise RuntimeError("16 KiB entry alignment/order gate failed")
    for dp, push in zip(expected_order[2:], expected_order[:2]):
        if address_values[dp] - address_values[push] != 2 * 16384:
    ...
    if len(map_rows) != 4 or [row.rsplit(None, 1)[-1].removeprefix("_")
    ...
    write_json(output / "codegen.json", {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-codegen-v1",
        ...
        "section": {"segment": "__TEXT", "name": "__br_null_v1",
...
def timer_accounting_gate(manifest: dict, timed_runner: Path,
    ...
    if record.get("schema") != \
            "libwebp-backref-cost-aligned-null-stage-attribution-v1-timer-check-v1" or \
    ...
    for variant in ("B", "L"):
        ...
        env["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_SAMPLE_SET"] = \
            "timer-validation"
        result = ledger.run(
        ...
        if emitted[0].get("schema") != \
                "libwebp-backref-cost-aligned-null-stage-attribution-v1-output-v1" or \
                output_row.get("schema") != \
                "libwebp-backref-cost-aligned-null-stage-attribution-v1-timer-accounting-output-v1" or \
                stage.get("schema") != \
                "libwebp-backref-cost-aligned-null-stage-attribution-v1-stage-ledger-v1" or \
        ...
        accounting.append({"variant": variant, "output": output_row,
                           "stage": stage})
    summary = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-timer-accounting-summary-v1",
...
def work_counter_gate(manifest: dict, runner: Path, corpus: Path,
    ...
    for case in manifest["corpus"]:
        ...
        for method in manifest["protocol"]["methods"]:
            ...
            if any(row.get("schema") !=
                   "libwebp-backref-cost-aligned-null-stage-attribution-v1-work-v1"
    ...
    write_json(output / "work-metric-summary.json", {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-work-summary-v1",
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path,
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-aligned-null-stage-attribution-v1-output-v1"
               for item in outputs):
...
def admit_representative_payload(path: Path, expected_minimum: int) -> dict:
    ...
    source_raw = os.environ.get(
        "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_REHEARSAL_PAYLOAD")
    expected_sha = os.environ.get(
        "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_REHEARSAL_PAYLOAD_SHA256")
    expected_size_raw = os.environ.get(
        "WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_REHEARSAL_PAYLOAD_BYTES")
...
def operator(mode: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="backref-aligned-null-stage-v1-") as raw:
        temporary = Path(raw)
        ...
        if mode == "rehearse":
            ...
            write_json(output / "performance-evidence-status.json", {
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-performance-evidence-status-v1",
            ...
            with lease_path.open("a+") as lease:
                ...
                try:
                    ...
                    write_json(output / "dynamic-admission.json", {
                        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-dynamic-admission-v1",
