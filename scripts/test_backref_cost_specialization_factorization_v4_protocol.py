#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization factorization v4."""
...
import run_backref_cost_specialization_factorization_v4_experiment as operator
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostSpecializationFactorizationV4GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V4_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostSpecializationFactorizationV4GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_specialization_factorization_v4_admission.py").read_text()
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v4-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority": "FACTORIZATION_V4_DIAGNOSTIC_ONLY",
    ...
    diff = [
        ("A", "evidence/backref-cost-specialization-factorization-v4-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-specialization-factorization-v4-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-specialization-factorization-v4-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "f289499b37875d8166792ed104f2b26e23cf4ffd"
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_specialization_factorization_v4.py run '
                '"$PWD/evidence/backref-cost-specialization-factorization-v4-execution-20260820"')
    ...
    print("PASS: specialization factorization v4 frozen semantics, strict single-session frames, "
