#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost attribution v12."""
...
from backref_cost_attribution_v12_admission import (
...
def clean_environment(manifest: dict, variant: str,
    ...
    if stage_output is not None:
        env.update({
            ...
            "WEBP_BACKREF_COST_ATTRIBUTION_V12_TIMERS": "1",
            "WEBP_BACKREF_COST_ATTRIBUTION_V12_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_ATTRIBUTION_V12_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_ATTRIBUTION_V12_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_ATTRIBUTION_V12_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
            "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V12_EXPERIMENT=1",
            "WEBP_BACKREF_COST_ATTRIBUTION_V12_DETERMINISTIC_BUILD=1",
            f"CFLAGS={flags}",
            "tools/backref_cost_attribution_v12_experiment_runner"]
    result = run(argv, cwd=build_root, env=env, timeout=1800)
    (output / "build.log").write_text(result.stdout + result.stderr,
                                       encoding="utf-8")
    runner = build_root / "tools/backref_cost_attribution_v12_experiment_runner"
...
def timer_overhead_gate(manifest: dict, runner: Path, output: Path) -> dict:
    ...
    if record.get("schema") != \
            "libwebp-backref-cost-attribution-v12-timer-check-v1" or \
    ...
    for case in manifest["corpus"]:
        ...
        for method in manifest["protocol"]["methods"]:
            ...
            if any(row.get("schema") !=
                   "libwebp-backref-cost-attribution-v12-counter-check-v1"
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path) -> list[dict]:
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-attribution-v12-output-v1"
               for item in outputs):
...
def admit_representative_payload(path: Path, expected_minimum: int) -> dict:
    ...
    source_raw = os.environ.get(
        "WEBP_BACKREF_COST_ATTRIBUTION_V12_REHEARSAL_PAYLOAD")
    expected_sha = os.environ.get(
        "WEBP_BACKREF_COST_ATTRIBUTION_V12_REHEARSAL_PAYLOAD_SHA256")
    expected_size_raw = os.environ.get(
        "WEBP_BACKREF_COST_ATTRIBUTION_V12_REHEARSAL_PAYLOAD_BYTES")
...
def operator(mode: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="backref-attribution-v12-") as raw:
        temporary = Path(raw)
