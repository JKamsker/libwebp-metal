#!/usr/bin/env python3
"""Untimed V10 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "a027f4a4b6a61d2b4a5d9e897e6b7ca51eb6c6ca"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V10_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V10_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V10_EXPERIMENT"
RUNNER = "backref_cost_attribution_v10_experiment_runner"
SOURCE = "backref_cost_attribution_v10_experiment_enc.c"
...
def main() -> int:
    ...
    assert "V10 preserves V9's frozen control flow" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v10-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V10_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V10_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V10_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V10_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
