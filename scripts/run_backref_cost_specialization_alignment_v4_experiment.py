#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost specialization alignment v4."""
...
from backref_cost_specialization_alignment_v4_admission import (
...
MANIFEST_PATH = ROOT / "scripts/backref_cost_specialization_alignment_v4_manifest.json"
...
def clean_environment(manifest: dict, variant: str,
    ...
    for name in manifest["forbidden_environment"]:
        env.pop(name, None)
    if variant not in ("B", "L", "H"):
        raise RuntimeError("unknown variant")
    env[manifest["candidate"]["runtime_flag"]] = variant
    if stage_output is not None:
        env.update({
            "WEBP_BENCHMARK_SESSION": "exclusive",
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_TIMERS": "1",
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    def one(name: str, recorder: bool) -> dict:
        ...
        argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
                "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_EXPERIMENT=1",
                f"WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_RECORDER={int(recorder)}",
                f"CFLAGS={flags}",
                f"LDFLAGS={manifest['build']['ldflags']},-map,{build_root / 'linker.map'}",
                "tools/backref_cost_specialization_alignment_v4_experiment_runner"]
    ...
    if "AlignmentV4Record" in timed["nm"] or \
            "AlignmentV4Stats" in timed["nm"]:
        raise RuntimeError("timed binary contains recorder symbols")
    if "AlignmentV4RecordPush" not in recorder["nm"] or \
    ...
    for line in timed["nm"].splitlines():
        ...
        if match and match.group(2) in manifest["acceptance"]["codegen_symbols"]:
            private_addresses[match.group(2)] = match.group(1)
    if set(private_addresses) != set(manifest["acceptance"]["codegen_symbols"]) or \
    ...
    symbols = manifest["acceptance"]["codegen_symbols"]
    disassembly = run([
    ...
    if canonical("PushIntervalSpecialized") == \
            canonical("PushIntervalLayoutClone"):
        raise RuntimeError("hint body unexpectedly equals layout clone")
    ...
    if sorted(address_values, key=address_values.get) != expected_order or \
            any(value % 16384 for value in address_values.values()) or \
            any(address_values[expected_order[i + 1]] -
                address_values[expected_order[i]] != 16384 for i in range(5)):
        raise RuntimeError("16 KiB entry alignment/order gate failed")
    for dp, push in zip(expected_order[3:], expected_order[:3]):
        if address_values[dp] - address_values[push] != 3 * 16384:
    ...
    if len(map_rows) != 6 or [row.rsplit(None, 1)[-1].removeprefix("_")
    ...
    write_json(output / "codegen.json", {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-codegen-v1",
        ...
        "hint_body_distinct": True, "all_six_private_addresses_distinct": True,
        "recorder_symbols_in_timed_binary": False,
        "symbol_addresses": private_addresses,
        "alignment_boundary_bytes": 16384,
        "section": {"segment": "__TEXT", "name": "__br_align_v4",
    ...
    record = {
        ...
        "all_six_private_addresses_distinct": True,
...
def timer_accounting_gate(manifest: dict, timed_runner: Path,
    ...
    if record.get("schema") != \
            "libwebp-backref-cost-specialization-alignment-v4-timer-check-v1" or \
    ...
    for variant in ("B", "L", "H"):
        ...
        env["WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_SAMPLE_SET"] = \
            "timer-validation"
        result = ledger.run(
        ...
        selected = (output_row["baseline_dp_calls"] +
                    ...
                    output_row["hint_dp_calls"])
        expected = {"B": "baseline_dp_calls", "L": "layout_clone_dp_calls",
                    "H": "hint_dp_calls"}[variant]
        nested = stage.get("stages", {})
        required = manifest["acceptance"]["required_timer_stages"]
        if emitted[0].get("schema") != \
                "libwebp-backref-cost-specialization-alignment-v4-output-v1" or \
                output_row.get("schema") != \
                "libwebp-backref-cost-specialization-alignment-v4-timer-accounting-output-v1" or \
                stage.get("schema") != \
                "libwebp-backref-cost-specialization-alignment-v4-timer-accounting-stage-v1" or \
        ...
        accounting.append({"variant": variant, "output": output_row,
                           "stage": stage})
    summary = {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-timer-accounting-summary-v1",
        "status": "PASS", "clock": record, "clock_children": 1,
        "accounting_children": 3, "accounting_rows": 3,
        "stage_rows": 3, "performance_rows": 0,
    ...
    for case in manifest["corpus"]:
        ...
        for method in manifest["protocol"]["methods"]:
            ...
            if any(row.get("schema") !=
                   "libwebp-backref-cost-specialization-alignment-v4-work-v1"
            ...
            if any(values["B"][field] != values["L"][field]
                   for field in common) or \
                    any(values["B"][field] != values["H"][field]
                        for field in common):
            ...
            if any(values[v][field] != 0 for v in ("B", "L")
                   for field in zero_hint) or \
                    ...
                    values["L"]["position_forward_steps"] or \
                    values["H"]["append_hint_branch_checks"] <= 0 or \
                    values["H"]["append_hint_loads"] <= 0 or \
                    values["H"]["append_hint_fast_paths"] <= 0 or \
                    values["H"]["position_forward_steps"] != 0:
                raise RuntimeError("append-hint boundary activation gate failed")
    with (output / "work-metrics.jsonl").open("w", encoding="utf-8") as stream:
        for row in rows:
            stream.write(json.dumps(row, sort_keys=True) + "\n")
    write_json(output / "work-metric-summary.json", {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-work-summary-v1",
        "rows": len(rows), "baseline_layout_work_equal": True,
        "common_algorithm_work_equal_all_variants": True,
        "hint_boundary_active_only_in_H": True})
    return rows
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path,
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-specialization-alignment-v4-output-v1"
               for item in outputs):
...
def admit_representative_payload(path: Path, expected_minimum: int) -> dict:
    ...
    source_raw = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_REHEARSAL_PAYLOAD")
    expected_sha = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_REHEARSAL_PAYLOAD_SHA256")
    expected_size_raw = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_REHEARSAL_PAYLOAD_BYTES")
...
def operator(mode: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="backref-specialization-alignment-v4-") as raw:
        temporary = Path(raw)
        ...
        if mode == "rehearse":
            ...
            write_json(output / "performance-evidence-status.json", {
                "schema": "libwebp-backref-cost-specialization-alignment-v4-performance-evidence-status-v1",
            ...
            with lease_path.open("a+") as lease:
                ...
                try:
                    ...
                    if timer_validation.get("status") != "PASS" or \
                            len(ledger.receipts) != 28:
                        raise RuntimeError("dynamic timer/ownership admission failed")
                    write_json(output / "dynamic-admission.json", {
                        "schema": "libwebp-backref-cost-specialization-alignment-v4-dynamic-admission-v1",
                        "status": "PASS_BEFORE_FIRST_PERFORMANCE_SAMPLE",
                        "gate_a_correctness_encodes": 24,
                        "gate_a_aggregate_cells": 4,
                        "gate_b_clock_children": 1,
                        "gate_b_timer_accounting_children": 3,
                        "gate_b_timer_accounting_rows": 3,
                        "gate_c_direct_children_so_far": 28,
                    ...
                    write_json(output / "performance-evidence-status.json", {
                        ...
                        "accepted_performance_rows": 144,
                        "runner_rows": 180, "stage_rows": 180,
                        "profile_children": 36,
                        "partial_promotion_forbidden": True})
                finally:
