#!/usr/bin/env python3
"""Generate fresh alignment-v4 identity fixtures and freeze manifest hashes."""
...
import backref_cost_specialization_alignment_v4_payload as payload
import backref_cost_specialization_alignment_v4_toolchain as toolchain
import backref_cost_specialization_alignment_v4_transport as transport
import test_backref_cost_specialization_alignment_v4_experiment as experiment_test


ROOT = Path(__file__).resolve().parents[1]
MANIFEST = ROOT / "scripts/backref_cost_specialization_alignment_v4_manifest.json"
EVIDENCE = ROOT / "evidence/backref-cost-specialization-alignment-v4-design"
...
def capture_codegen(build_root: Path) -> None:
    runner = build_root / "backref_cost_specialization_alignment_v4_experiment_runner"
    ...
    section_rows = [line for line in map_text.splitlines()
                    if "__TEXT\t__br_align_v4" in line]
    ...
    write_json(EVIDENCE / "phase1-local-codegen.json", {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-local-codegen-v1",
        "status": "PASS",
...
def audit_v4(v4_map: Path) -> None:
    ...
    alignment_v3_authorization_path = ROOT / \
        "evidence/backref-cost-specialization-alignment-v3-design/phase1b-authorization.json"
    alignment_v3_phase2_path = ROOT / \
        "evidence/backref-cost-specialization-alignment-v3-design/phase2-audit.json"
    alignment_v3_phase3_path = ROOT / \
        "evidence/backref-cost-specialization-alignment-v3-phase3-evaluation-consequence-20260820.json"
    alignment_v3_authorization = json.loads(
        alignment_v3_authorization_path.read_text(encoding="utf-8"))
    alignment_v3_phase2 = json.loads(
        alignment_v3_phase2_path.read_text(encoding="utf-8"))
    alignment_v3_phase3 = json.loads(
        alignment_v3_phase3_path.read_text(encoding="utf-8"))
    expected_v3_blockers = {
        "mandatory 24 correctness encodes before timing were not run in the zero-workload rehearsal",
        "coarse timer record accounting was not run in the rehearsal",
        "serial and direct ownership of timed children was not proven because the rehearsal had no timed children",
    }
    if alignment_v3_authorization.get("phase_2_authority") != \
            "ALIGNMENT_V3_DIAGNOSTIC_ONLY" or \
            alignment_v3_phase2.get("authority", {}).get(
                "authority") != "NONE" or \
            set(alignment_v3_phase3.get("authority_and_blockers", {}).get(
                "blockers", [])) != expected_v3_blockers:
        raise RuntimeError("Alignment V3 refusal/authority audit mismatch")
    v4_symbols = [
        "BackwardReferencesHashChainDistanceOnly",
        "BackwardReferencesHashChainDistanceOnlyLayoutClone",
        "BackwardReferencesHashChainDistanceOnlySpecialized",
    ...
    write_json(EVIDENCE / "phase1-prior-evidence-audit.json", {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-prior-evidence-audit-v1",
        ...
        "provenance": {
            "factorization_v4": {
                "phase1a": "1f60c324142d3dafaeece95b060114261fff86ac",
                "phase1b": "c271d129d392c9c279722bb95e17805e1c262b9a",
                "phase2": "b06596fb6c7c1fa6331dc2aa221d1bfa8f448403",
                "phase3": "b2238c88da8ea3c4c70dd19e455e62388684fcff",
                "merge": "adf680e69b9eeeef636e62e0e0eb0c8fdace4013",
            },
            "alignment_v1": {
                "phase1a": "67b53054e327729f5e7b7d8f04d384a0ddf4e6c6",
                "phase1b": "58444ffca4676ec4537ffc26542d777cf17e018f",
                "phase2": "2999fe7e43fcf1bd88a1cabb6c458db2bb1ea672",
                "phase3": "cab7f4e8d03d59fcae3c0ae89ef5033931955d56",
                "merge": "b96909b28b3d64f000d0f98ffa76137305010808",
            },
            "alignment_v2": {
                "phase1a": "e2816e632075779b299adfc2cf8ab7d87d7c0d6f",
                "phase1b": "d653a00a642fd1b2707016848a900f94ed51cfcf",
                "phase2": "9c1d562db0e7fc79829a920762e81e16a5c80de8",
                "phase3": "d158e57c57faf3ef0b567bfe8e4b0cd3634ea872",
                "merge": "c208d6ed33b0ee5e84ba02e78696879266658e7e",
            },
            "alignment_v3": {
                "phase1a": "f6735570eb2a81faa60544b289fb0207b8cde873",
                "phase1b": "3b28d9066d94d9397b2b36a09540547adee9efa5",
                "phase2": "0dcd926216af945a1688d73103b35b329b1b2c55",
                "phase3": "0e8b3bd3be962f3f14642293f92cbe169c1fb898",
                "merge": "91ab79b70bb1f99888dce63e8b1ac512741bafb3",
            },
        },
        "alignment_v3_authority_model_audit": {
            "phase1b_authority": alignment_v3_authorization["phase_2_authority"],
            "phase2_independent_authority": "NONE",
            "phase2_host_contacts": 0,
            "transport_rehearsal_status": "PASS",
            "request_payload_bytes": 23193668,
            "request_chunks_written": 354,
            "request_chunks_acknowledged": 354,
            "source_map_entry_count": 66,
            "transport_stderr_bytes": 0,
            "exact_blockers": sorted(expected_v3_blockers),
            "v4_resolution": "Treat the three facts as online gates inside the sole conditionally authorized pipeline, not historical outputs required from a zero-workload rehearsal.",
            "authorization_sha256": sha256(alignment_v3_authorization_path),
            "phase2_audit_sha256": sha256(alignment_v3_phase2_path),
            "phase3_consequence_sha256": sha256(alignment_v3_phase3_path),
        },
        "alignment_v2_request_write_audit": {
            "phase1a_source_hashes": {
                relative: git_blob_sha(
                    "e2816e632075779b299adfc2cf8ab7d87d7c0d6f",
                    relative)
                for relative in (
                    ...
                    "scripts/backref_cost_specialization_alignment_v2_manifest.json")
            },
            "initial_state": "REQUEST_WRITE",
            "stdin_registration":
                "the directly spawned SSH stdin was registered writable immediately after process creation, before any stdout identity or readiness receipt",
            "request_shape": {
                "stream_magic": "libwebp-backref-cost-specialization-alignment-v2-stream-v1\\n",
                "ordered_frames": [
                    "request-metadata sequence=0 canonical JSON",
                    "source-bundle sequence=1 exact git bundle",
                    "representative-payload sequence=2 exact 2097152-byte deterministic payload"],
                "io_chunk_bytes": 65536,
                "per_frame_payload_cap_bytes": 67108864,
                "ready_or_request_ack_frames": 0,
                "exact_total_request_bytes_recorded_by_rehearsal": None,
                "limitation":
                    "V2 froze the serializer and component shape but its timeout refusal did not persist a request byte count, so no exact attempted byte total is inferred.",
            },
            "state_machine": {
                "REQUEST_WRITE":
                    "request progress deadline; stdout/stderr drained concurrently; stdout did not establish identity",
                "SILENT_OPERATOR":
                    "entered only after local stdin completion and tracked silent response waiting",
                "RESPONSE_READ":
                    "response progress deadline after first stdout byte",
            },
            "source_admission_compact_freeze": {
                "identity_paths": 66,
                "compact_control_payload_bytes": 554,
                "compact_encoded_frame_bytes": 710,
                "control_payload_margin_bytes": 7638,
                "alignment_v1_full_map_payload_bytes": 9285,
            },
            "sole_rehearsal": {
                "timeout_phase": "REQUEST_WRITE",
                "timeout_deadline_kind": "request-progress",
                "framed_stdout_bytes": 0,
                "transport_stderr_bytes": 0,
                "connection_attempts": 1,
                "remote_connection_established": False,
                "remote_contacts_established": 0,
                "encoder_workloads": 0,
                "performance_samples": 0,
                "lease_attempted": False,
                "transfer_cleanup_sha256": sha256(ROOT /
                    "evidence/backref-cost-specialization-alignment-v2-design/rehearsal/transfer-cleanup.json"),
            },
            "conclusion":
                "An SSH process attempt is not evidence that the host/channel was established; V2 produced no authenticated remote identity receipt and no scientific timing.",
        },
        ...
        "sources": {
            ...
            "alignment_v3_authorization": {
                "path": str(alignment_v3_authorization_path.relative_to(ROOT)),
                "sha256": sha256(alignment_v3_authorization_path)},
            "alignment_v3_phase2_refusal": {
                "path": str(alignment_v3_phase2_path.relative_to(ROOT)),
                "sha256": sha256(alignment_v3_phase2_path)},
...
def fresh_identities(manifest: dict) -> None:
    ...
    payload_fixture = {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-payload-fixture-v1",
    ...
    payload_agreement = {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-payload-agreement-v1",
        "status": "PASS",
        ...
        "generator_path": "scripts/backref_cost_specialization_alignment_v4_payload.py",
        "generator_file_sha256": sha256(ROOT / "scripts/backref_cost_specialization_alignment_v4_payload.py"),
    ...
    write_json(toolchain_fixture_path, {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-toolchain-fixture-v1",
        "identity": toolchain_identity,
    })
    identity_digest = canonical_sha256(toolchain_identity)
    toolchain_agreement = {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-toolchain-agreement-v1",
        "status": "PASS",
    ...
    manifest["build"].update({
        ...
        "identity_rule":
            "one recorder-free Mach-O serves B/L/H; -no_deduplicate preserves six bodies; __TEXT,__br_align_v4 contains Push B/L/H then DP B/L/H at exact 16 KiB entry spacing; linker map, section flags, body facts, normalized instructions, padding and binary bytes are frozen",
    })
...
def freeze_request_channel_sizes(manifest: dict) -> dict:
    ...
    bootstrap_path = ROOT / \
        "scripts/backref_cost_specialization_alignment_v4_bootstrap.py"
    bootstrap_sha = sha256(bootstrap_path)
    request = transport.build_channel_request(
        (ROOT / "scripts/backref_cost_specialization_alignment_v4_transport.py").read_bytes(),
        (ROOT / "scripts/backref_cost_specialization_alignment_v4_payload.py").read_bytes(),
    ...
    with tempfile.TemporaryDirectory(prefix="alignment-v4-base-bundle-") as raw:
    ...
    value = {
        "schema":
            "libwebp-backref-cost-specialization-alignment-v4-request-size-freeze-v1",
...
def freeze_local_validation(manifest: dict, request_sizes: dict) -> None:
    ...
    write_json(EVIDENCE / "phase1-local-validation.json", {
        "schema":
            "libwebp-backref-cost-specialization-alignment-v4-phase1-local-validation-v1",
        ...
        "remote_contacts": 0,
        "design_decision": "SIX_BODY_16K_ALIGNMENT_WITH_IN_PIPELINE_DYNAMIC_ADMISSION",
        "checks": {
            ...
            "runtime_truth_table": "PASS_ABSENT_B_EXACT_B_L_H_OTHER_INVALID",
            ...
            "sanitizer_boundary_encodes": 36,
            "correctness_fallback_determinism_immutability": "PASS",
            "six_private_symbols_distinct": "PASS_NO_DEDUPLICATION",
            "mach_o_section": "PASS___TEXT___br_align_v4_EXECUTABLE_PURE_INSTRUCTIONS",
            "alignment_modulo": "PASS_ALL_SIX_ZERO_MOD_16384",
            "b_l_push_and_dp_canonical_instruction_identity": "PASS",
            "h_intended_append_hint_boundary_difference": "PASS",
            ...
            "dynamic_gate_a_24_correctness_before_timing": "PASS_STRUCTURAL_AND_FIXTURES",
            ...
            "global_experiment_mutual_exclusion": "PASS_44_GUARDS",
            ...
            "source_admission_compact_and_full_map_equivalence": "PASS_66_PATHS",
        ...
        "transport_freeze": {
            ...
            "frozen_source_identity_paths": 66,
        },
        "identity_freeze": {
            "payload_bytes": payload.PAYLOAD_BYTES,
            "payload_sha256": payload.sha256(),
            "toolchain_identity_canonical_sha256": canonical_sha256(identity),
            "xcode": identity["xcode"]["short_version"],
            "sdk": identity["sdk"]["canonical_name"],
            "required_executables": len(identity["required_executable_names"]),
            "required_metadata": len(identity["required_metadata_names"]),
            "xcrun_required": False,
        },
        "frozen_inventory": {
            "cases": ["photo-large", "graphic-large"], "methods": [4, 6],
            "variants": ["B", "L", "H"],
            "variant_order": [["B", "L", "H"], ["H", "L", "B"],
                              ["L", "B", "H"]],
            "profile_repetitions": 3, "warmups_per_profile": 1,
            "retained_encodes_per_profile": 4, "profile_processes": 36,
            "warmups": 36, "retained_timing_rows": 144,
            "stage_rows": 180, "runner_rows": 180,
            "correctness_encodes": 24, "untimed_work_rows": 12,
            "timer_validation_clock_children": 1,
            "timer_accounting_children": 3,
            "direct_runner_children": 76,
            "bootstrap_resamples": 100000, "bootstrap_seed": "0x424c4801",
        },
        "acceptance_freeze": {
        ...
        "commands": [
            "python3 -m py_compile scripts/*alignment_v4*.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_payload_identity.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_toolchain_identity.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_process_ownership.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_archive.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_streaming.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_admission.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_experiment.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_specialization_alignment_v4_protocol.py",
...
def freeze_source_admission_control(manifest: dict) -> dict:
    ...
    value = {
        "schema":
            "libwebp-backref-cost-specialization-alignment-v4-control-freeze-v1",
        "status": "PASS",
        "construction":
            "actual backref_cost_specialization_alignment_v4_transport.canonical_json and _encoded_json",
