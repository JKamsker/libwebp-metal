#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v11."""
...
import execute_backref_cost_attribution_v11 as executor
import run_backref_cost_attribution_v11_experiment as operator
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV11ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV11RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V11_EXPERIMENT=1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v11-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "observed_transport_calibration": {
            ...
            "archive_bytes": 5 * 1024 * 1024, "chunk_count": 10,
            "transaction_elapsed_ns": 1, "finite_margin_ratio": 1.25},
        "cleanup_footer_verified": True,
        "post_run_remote_cleanup_operations": 0,
        "timed_execution_authorized": True}
    diff = [
        ("A", "evidence/backref-cost-attribution-v11-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v11-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v11-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "631ac52803ff7d74f8461917503d54b46a8fc5e4"
    ...
    print("PASS: attribution v11 frozen semantics, strict single-session frames, "
