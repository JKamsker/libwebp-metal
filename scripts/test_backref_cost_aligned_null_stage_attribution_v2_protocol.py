#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization aligned null stage attribution v2."""
...
import execute_backref_cost_aligned_null_stage_attribution_v2 as executor
import run_backref_cost_aligned_null_stage_attribution_v2_experiment as operator
...
import evaluate_backref_cost_aligned_null_stage_attribution_v2 as evaluator


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_aligned_null_stage_attribution_v2_manifest.json"
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostAlignedNullStageAttributionV2GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostAlignedNullStageAttributionV2GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v2_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v2-archive-inventory-v1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v2-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority":
            "ALIGNED_NULL_STAGE_ATTRIBUTION_V2_CONDITIONAL_DIAGNOSTIC_AUTHORITY",
    ...
    diff = [
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v2-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v2-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v2-design/phase1b-authorization.json")]
    kwargs = {
...
def zero_sample_refusal_contract() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(
                prefix="stage-attribution-v2-refusal-") as raw:
...
def v4_audit_and_null_contract(manifest: dict) -> None:
    audit = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v2-design/phase1-prior-evidence-audit.json"
    ).read_text())
    assert audit["status"] == "PASS_ALIGNMENT_V4_PREMISE_FROZEN"
    assert audit["performance_samples_generated"] == 0
    assert audit["v4_timing_rows_loaded_into_v2_statistics"] == 0
    assert audit["v1_timing_rows_loaded_into_v2_statistics"] == 0
    assert audit["v1_authority_audit"]["failed_gates"] == [
        "alignment build frozen"]
    assert audit["v1_authority_audit"]["can_contribute_samples_to_v2"] is False
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "fb297dd1097f6fc5a2f2d4d453da3c24b235d7d1"
    assert manifest["candidate"]["build_flag"] == \
        "WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2_EXPERIMENT"
    ...
    assert "0x4e554c32" in manifest["protocol"]["statistics"]
    ...
    assert evaluator.BOOTSTRAP_SEED == 0x4E554C32
    expected_receipt = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v2-design" /
    ...
    evaluator_source = (ROOT / "scripts" /
        "evaluate_backref_cost_aligned_null_stage_attribution_v2.py").read_text()
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_aligned_null_stage_attribution_v2.py run '
                '"$PWD/evidence/backref-cost-aligned-null-stage-attribution-v2-execution-20260820"')
    ...
    print("PASS: specialization aligned null stage attribution v2 frozen semantics, strict single-session frames, "
