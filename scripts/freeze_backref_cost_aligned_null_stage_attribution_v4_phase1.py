#!/usr/bin/env python3
"""Generate fresh stage-attribution-v4 identity fixtures and freeze hashes."""
...
import backref_cost_aligned_null_stage_attribution_v4_payload as payload
...
import test_backref_cost_aligned_null_stage_attribution_v4_experiment as experiment_test


ROOT = Path(__file__).resolve().parents[1]
MANIFEST = ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_manifest.json"
EVIDENCE = ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v4-design"
...
def receipt_bindings(manifest: dict) -> dict:
    ...
    return {
        ...
        "generator_file_sha256": sha256(
            ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_identity.py"),
    }
...
def capture_codegen(build_root: Path, bindings: dict) -> None:
    ...
    runner = build_root / "backref_cost_aligned_null_stage_attribution_v4_experiment_runner"
    ...
    write_json(EVIDENCE / "phase1-local-codegen.json", {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-local-codegen-v1",
        "status": "PASS",
...
def audit_v4(v4_map: Path) -> None:
    ...
    write_json(EVIDENCE / "phase1-prior-evidence-audit.json", {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-prior-evidence-audit-v1",
        "status": "PASS_TRANSPORT_REQUALIFICATION_PREMISE_FROZEN",
        ...
        "v4_timing_rows_loaded_into_v4_statistics": 0,
        ...
        "v1_authority_audit": {
            ...
            "can_contribute_samples_to_v4": False,
            "status": "PASS_SOLE_STALE_SIX_BODY_GATE_ZERO_ROWS",
        },
        "v1_timing_rows_loaded_into_v4_statistics": 0,
...
def audit_request_transport_requalification() -> None:
    """Machine-freeze successful Alignment V4/V4 and failed null V2 transfer."""
    variants = {
        "alignment-v4": {
            "prefix": "backref_cost_specialization_alignment_v4",
            "design": "backref-cost-specialization-alignment-v4-design",
        },
        "alignment-v4": {
            "prefix": "backref_cost_specialization_alignment_v4",
            "design": "backref-cost-specialization-alignment-v4-design",
        ...
        "aligned-null-v2": {
            "prefix": "backref_cost_aligned_null_stage_attribution_v2",
            "design": "backref-cost-aligned-null-stage-attribution-v2-design",
        },
    }

    def function_source(path: Path, names: set[str]) -> str:
        ...
        for node in tree.body:
            if isinstance(node, (ast.FunctionDef, ast.ClassDef)) and \
                    node.name in names:
                rows.append(ast.get_source_segment(source, node) or "")
        if len(rows) != len(names):
            raise RuntimeError(f"transport audit source extraction failed: {path}")
        return "\n\n".join(rows)
    ...
    for label, item in variants.items():
        prefix = item["prefix"]
        design = ROOT / "evidence" / item["design"]
        progress_path = design / "rehearsal/return-protocol/request-progress.json"
        transfer_path = design / "rehearsal/transfer-cleanup.json"
        progress = json.loads(progress_path.read_text(encoding="utf-8"))
        transfer = json.loads(transfer_path.read_text(encoding="utf-8"))
        admission_path = ROOT / "scripts" / f"{prefix}_admission.py"
        transport_path = ROOT / "scripts" / f"{prefix}_transport.py"
        execute_path = ROOT / "scripts" / f"execute_{prefix}.py"
        pump = function_source(admission_path, {
            "TransportDeadlineStateMachine", "pump_owned_request_channel"})
        remote = function_source(transport_path, {
            "OnlineChannelValidator", "remote_transaction"})
        normalized = pump + "\n\n" + remote
        normalized = re.sub(
            r"backref_cost_(?:specialization_alignment_v[34]|aligned_null_stage_attribution_v2)",
            "EXPERIMENT", normalized)
        normalized = re.sub(
            r"(?:BACKREF_COST_SPECIALIZATION_ALIGNMENT_V[34]|BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V2)",
            "EXPERIMENT_UP", normalized)
        normalized = re.sub(
            r"backref-cost-(?:specialization-alignment-v[34]|aligned-null-stage-attribution-v2)",
            "EXPERIMENT-HYPHEN", normalized)
        normalized = re.sub(
            r"(?:specialization-alignment-v[34]|stage-attribution-v2)",
            "EXPERIMENT-LABEL", normalized)
        normalized = re.sub(r"(?:alignment-v[34]|V2)",
                            "EXPERIMENT-SHORT", normalized)
        normalized_sources[label] = normalized
        execute_text = execute_path.read_text(encoding="utf-8")
        constants = {}
        for name in ("REMOTE_TRANSACTION_TIMEOUT_SECONDS",
                     "CONNECT_CHANNEL_READY_TIMEOUT_SECONDS",
                     "SILENT_OPERATOR_TIMEOUT_SECONDS",
                     "REQUEST_PROGRESS_TIMEOUT_SECONDS",
                     "RESPONSE_PROGRESS_TIMEOUT_SECONDS"):
            match = re.search(rf"^{name} = ([0-9]+)$", execute_text, re.MULTILINE)
            if match is None:
                raise RuntimeError(f"missing frozen transport constant: {label}:{name}")
            constants[name] = int(match.group(1))
        rows[label] = {
            "request_payload_bytes": progress["request_payload_bytes"],
            "request_chunk_bytes": 65536,
            "request_chunks_planned": progress.get(
                "request_chunks_planned", progress.get("request_chunks_written")),
            "request_chunks_acknowledged": progress[
                "request_chunks_acknowledged"],
            "request_bytes_acknowledged": progress[
                "request_bytes_acknowledged"],
            "declaration_accepted": progress["declaration_accepted"],
            "status": progress["status"],
            "timeout_phase": transfer.get("timeout_phase"),
            "timeout_deadline_kind": transfer.get("timeout_deadline_kind"),
            "channel_stdout_bytes": transfer.get("channel_stdout_bytes"),
            "transport_stderr_bytes": transfer.get("transport_stderr_bytes"),
            "remote_connection_established": transfer.get(
                "remote_connection_established"),
            "cleanup_footer_verified": transfer.get("cleanup_footer_verified"),
            "remote_cleanup_absence_proved": transfer.get(
                "remote_cleanup_absence_proved"),
            "progress_sha256": sha256(progress_path),
            "transfer_sha256": sha256(transfer_path),
            "admission_source_sha256": sha256(admission_path),
            "transport_source_sha256": sha256(transport_path),
            "execute_source_sha256": sha256(execute_path),
            "frozen_deadlines_seconds": constants,
            "scheduling": {
                "client": "one selector concurrently drains stdout/stderr and ACK-gates one nonblocking stdin frame",
                "remote": "READY then declaration ACK then exact sequential read/chunk ACK; response only after EOF and request-complete",
                "deadline_progress": "only stdin bytes written, request ACK state advancement, response payload bytes, or bounded refusal drain",
                "keepalive_is_data_progress": False,
                "cleanup": "same-transaction finally; absent cleanup footer never implies cleanup",
            },
        }
    normalized_hashes = {
    ...
    if normalized_hashes["aligned-null-v2"] != \
            normalized_hashes["alignment-v4"]:
        raise RuntimeError("V2 differs deterministically from successful V4 transport")
    comparison = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-transport-comparison-v1",
        "status": "PASS_NO_DETERMINISTIC_V2_TRANSPORT_REGRESSION_FOUND",
        "audited_main": "92c3b59b2d178c5a03ec20b897252b358f07de81",
        "variants": rows,
        "normalized_state_machine_and_pump_sha256": normalized_hashes,
        "source_differences": [{
            "variants": ["alignment-v4", "alignment-v4/aligned-null-v2"],
            "difference": "Alignment V4 did not mark its directly spawned Popen as transport_child; Alignment V4 and null V2 did.",
            "stall_plausibility": "not plausible: it changes only whether the retained child may be stopped after a deadline and does not alter writes, reads, ACKs, buffering, scheduling, or deadline extension",
        }],
        "deterministic_regression_found": False,
        "absent_data_inference_used": False,
        "frozen_consequence": {
            "classification": "fresh-bounded-requalification",
            "rationale": "V2 used the same pump, validator, ACK-gated scheduling, deadlines, chunk size, stdout/stderr draining, keepalives, and cleanup/finally protocol as two successful 23+ MB transfers; the observed five-ACK stop is real but does not identify a deterministic source/config regression.",
            "protocol_limit_changes": [],
            "adaptive_remote_decision_permitted": False,
        },
    }
    write_json(EVIDENCE / "transport-comparison.json", comparison)
    prior = json.loads((EVIDENCE / "phase1-prior-evidence-audit.json").read_text(
        encoding="utf-8"))
    prior.update({
        "status": "PASS_TRANSPORT_REQUALIFICATION_PREMISE_FROZEN",
        "audited_main": comparison["audited_main"],
        "transport_comparison_path":
            "evidence/backref-cost-aligned-null-stage-attribution-v4-design/transport-comparison.json",
        "transport_comparison_sha256": sha256(EVIDENCE / "transport-comparison.json"),
        "chosen_consequence": comparison["frozen_consequence"],
        "performance_samples_generated": 0,
    })
    write_json(EVIDENCE / "phase1-prior-evidence-audit.json", prior)
...
def capture_local_untimed_evidence() -> dict:
    ...
    value = json.loads(run([
        "python3",
        str(ROOT / "scripts/test_backref_cost_aligned_null_stage_attribution_v4_experiment.py")
    ]).strip())
...
def fresh_identities(manifest: dict) -> None:
    ...
    payload_fixture = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-payload-fixture-v1",
        ...
        "stale_alignment_v4_sha256": payload.STALE_ALIGNMENT_V4_SHA256,
    }
    write_json(payload_fixture_path, payload_fixture)
    payload_agreement = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-payload-agreement-v1",
        "status": "PASS",
        ...
        "generator_path": "scripts/backref_cost_aligned_null_stage_attribution_v4_payload.py",
        "generator_file_sha256": sha256(ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_payload.py"),
    ...
    write_json(toolchain_fixture_path, {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-toolchain-fixture-v1",
        "identity": toolchain_identity,
    })
    identity_digest = canonical_sha256(toolchain_identity)
    toolchain_agreement = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-toolchain-agreement-v1",
        "status": "PASS",
        ...
        "negative_stale_alignment_v4_identity_test": "PASS_REJECTED",
        ...
        "stale_alignment_v4_identity_canonical_sha256":
            toolchain.STALE_ALIGNMENT_V4_IDENTITY_CANONICAL_SHA256,
    ...
    manifest["payload_identity_gate"].update({
        "sha256": payload_identity["sha256"],
        "negative_test": "PASS_REJECTED_ALL_PREDECESSOR_IDENTITIES_THROUGH_ALIGNMENT_V4",
        "negative_stale_v4_sha256": payload.STALE_V4_SHA256,
        "negative_stale_alignment_v4_sha256":
            payload.STALE_ALIGNMENT_V4_SHA256,
    })
    ...
    manifest["toolchain_identity_gate"].update({
        "negative_test": "PASS_REJECTED_STALE_XCRUN_AND_ALL_PREDECESSOR_ENVELOPES_THROUGH_ALIGNMENT_V4",
    })
    manifest["build"].update({
        "ld": str(toolchain.EXECUTABLE_PATHS["ld"]),
        "otool": str(toolchain.EXECUTABLE_PATHS["otool"]),
        "identity_rule":
            "one recorder-free Mach-O serves B/L; -no_deduplicate preserves four bodies; __TEXT,__br_null_v4 contains Push B/L then DP B/L at exact 16 KiB entry spacing",
    })
...
def freeze_request_channel_sizes(manifest: dict) -> dict:
    ...
    bootstrap_path = ROOT / \
        "scripts/backref_cost_aligned_null_stage_attribution_v4_bootstrap.py"
    bootstrap_sha = sha256(bootstrap_path)
    request = transport.build_channel_request(
        (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_transport.py").read_bytes(),
        (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_payload.py").read_bytes(),
    ...
    declaration_ack_core = {
        ...
        "request_declaration_sha256": request.declaration_sha256,
        "remote_receipt_sequence": 0,
        "prior_ack_digest": "0" * 64,
    }
    declaration_ack = {**declaration_ack_core, "ack_digest":
        transport.sha256_bytes(transport.canonical_json(declaration_ack_core))}
    controls.append(("request-ack", declaration_ack,
                     {"sequence": sequence, "ack_index": 0}))
    sequence += 1
    prior_ack_digest = declaration_ack["ack_digest"]
    for index in range(request.chunk_count):
        start = index * transport.REQUEST_CHUNK_BYTES
        acked = min((index + 1) * transport.REQUEST_CHUNK_BYTES,
                    len(request.value))
        chunk = request.value[start:acked]
        ack_core = {
            "schema": transport.REQUEST_CHUNK_ACK_SCHEMA,
            "ack_kind": "chunk",
            "chunk_sha256": transport.sha256_bytes(chunk),
            "cumulative_bytes": acked,
            "remote_receipt_sequence": index + 1,
            "prior_ack_digest": prior_ack_digest,
        }
        ack = {**ack_core, "ack_digest":
            transport.sha256_bytes(transport.canonical_json(ack_core))}
        prior_ack_digest = ack["ack_digest"]
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-base-bundle-") as raw:
    ...
    value = {
        "schema":
            "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-size-freeze-v1",
        ...
        "progress_chain_row_cap": 512,
        "progress_chain_max_rows": request.chunk_count,
        "progress_chain_worst_case_json_bytes":
            len(transport.canonical_json({
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-progress-chain-v1",
                "rows": [{
                    "transaction_nonce": "f" * 64,
                    "chunk_ordinal": index + 1,
                    "byte_start": index * transport.REQUEST_CHUNK_BYTES,
                    "byte_end_exclusive": min((index + 1) * transport.REQUEST_CHUNK_BYTES, len(request.value)),
                    "chunk_bytes": min(transport.REQUEST_CHUNK_BYTES, len(request.value) - index * transport.REQUEST_CHUNK_BYTES),
                    "cumulative_bytes": min((index + 1) * transport.REQUEST_CHUNK_BYTES, len(request.value)),
                    "chunk_sha256": "f" * 64,
                    "prior_ack_digest": "f" * 64,
                    "remote_receipt_sequence": index + 1,
                    "ack_digest": "f" * 64,
                    "write_start_monotonic_ns": 9_999_999_999_999_999_999,
                    "write_complete_monotonic_ns": 9_999_999_999_999_999_999,
                    "ack_received_monotonic_ns": 9_999_999_999_999_999_999,
                    "ack_latency_ns": 9_999_999_999_999_999_999,
                } for index in range(request.chunk_count)]})),
...
def freeze_local_validation(manifest: dict, request_sizes: dict) -> None:
    ...
    write_json(EVIDENCE / "phase1-local-validation.json", {
        "schema":
            "libwebp-backref-cost-aligned-null-stage-attribution-v4-phase1-local-validation-v1",
        ...
        "checks": {
            ...
            "mach_o_section": "PASS___TEXT___br_null_v4_EXECUTABLE_PURE_INSTRUCTIONS",
        ...
        "frozen_inventory": {
            ...
            "bootstrap_resamples": 100000, "bootstrap_seed": "0x4e554c33",
        },
        "acceptance_freeze": {
        ...
        "commands": [
            "python3 -m py_compile scripts/*aligned_null_stage_attribution_v4*.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_identity.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_payload_identity.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_toolchain_identity.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_process_ownership.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_archive.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_streaming.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_admission.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_experiment.py",
            "PYTHONPATH=scripts python3 scripts/test_backref_cost_aligned_null_stage_attribution_v4_protocol.py",
...
def freeze_source_admission_control(manifest: dict) -> dict:
    ...
    value = {
        "schema":
            "libwebp-backref-cost-aligned-null-stage-attribution-v4-control-freeze-v1",
        "status": "PASS",
        "construction":
            "actual backref_cost_aligned_null_stage_attribution_v4_transport.canonical_json and _encoded_json",
...
def main() -> int:
    ...
    if args.v4_linker_map is not None:
        ...
        audit_request_transport_requalification()
