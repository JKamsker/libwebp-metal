...
EXPECTED_ENV = "WEBP_EXPECTED_BACKREF_COST_INTERVAL_SPECIALIZATION_V1_COMMIT"
...
def remote_pipeline(output: Path, mode: str,
    ...
    if not MANIFEST_PATH.is_file():
        raise RuntimeError("frozen interval-specialization v1 manifest is absent")
    ...
    temporary_holder = tempfile.TemporaryDirectory(prefix="interval-specialization-v1-return-")
    temporary = Path(temporary_holder.name)
    try:
        remote_q = shlex.quote(record["run_directory"])
        operator = "scripts/run_backref_cost_interval_specialization_v1_experiment.py"
        ...
        elif any(lease_value.get(field) for field in
                 ("acquisition_attempted", "acquired", "released")):
            raise RuntimeError("rehearsal touched the benchmark lease")
        ...
        operator_status = json.loads(
            (output / "operator-status.json").read_text(encoding="utf-8"))
    ...
    receipt = {
        ...
        "benchmark_lease_acquisition_attempted": bool(
            (temporary / "lease-record.json").is_file() and
            json.loads((temporary / "lease-record.json").read_text()).get(
