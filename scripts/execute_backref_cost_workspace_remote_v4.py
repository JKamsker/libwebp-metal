#!/usr/bin/env python3
"""Remote-v4 admission-only rehearsal and frozen pipeline executor."""
...
HOST = admission.HOST
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_workspace_remote_v4_manifest.json"
EXPECTED_ENV = "WEBP_EXPECTED_BACKREF_COST_WORKSPACE_REMOTE_V4_COMMIT"
...
def admission_only(output: Path) -> dict:
    ...
    try:
        ...
        raise
    receipt = {
        "schema": "libwebp-backref-cost-workspace-remote-v4-admission-rehearsal-v1",
...
def timed_run(output: Path) -> dict:
    ...
    if not MANIFEST_PATH.is_file():
        raise RuntimeError("frozen v4 manifest is absent")
    if not output.is_absolute() or output.exists():
        raise RuntimeError("output must be a new absolute local path")
    ...
    temporary_holder = tempfile.TemporaryDirectory(prefix="remote-v4-return-")
    temporary = Path(temporary_holder.name)
    try:
        remote_q = shlex.quote(record["run_directory"])
        operator = "scripts/run_backref_cost_workspace_remote_v4_experiment.py"
