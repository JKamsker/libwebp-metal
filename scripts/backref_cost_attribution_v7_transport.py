#!/usr/bin/env python3
"""Framed single-session transport for backref-cost attribution v7.
...
removes exactly one validated v7 safe-root child.  It never discovers a PID for
...
MAGIC = b"LIBWEBP-BACKREF-COST-ATTRIBUTION-V7-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 1
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-attribution-v7"
RUN_PREFIX = "run."
...
def deterministic_payload(size: int) -> bytes:
    domain = b"libwebp-backref-cost-attribution-v7-rehearsal\x00"
    output = bytearray()
    counter = 0
...
def _safe_root(home: Path) -> Path:
    ...
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
        ...
        raise FrameRefusal("v7 safe root is not a canonical real HOME child")
    return root
...
def _validate_child(home: Path, root: Path, child: Path) -> None:
    if root != home / SAFE_ROOT_BASENAME or child.parent != root or \
        ...
        raise FrameRefusal("run directory is not the exact v7 safe-root child")
    info = os.lstat(child)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
...
def _process_guard() -> dict:
    result = _command(["ps", "-axo", "pid=,ppid=,state=,etime=,command="])
    rows = []
    parents = {}
    for line in result.stdout.decode(errors="replace").splitlines():
        ...
        if len(parts) == 5 and parts[0].isdigit() and parts[1].isdigit():
            ...
            parents[pid] = ppid
            rows.append((pid, line))
    lineage = set()
    current = os.getpid()
    while current and current not in lineage:
        lineage.add(current)
        current = parents.get(current, 0)
    candidates = [line for pid, line in rows if pid not in lineage]
    workers = [line for line in candidates if "Runner.Worker" in line]
    patterns = ("_experiment_runner", "nvidia-smi", "nvcc", "cuda-memcheck")
    conflicts = [line for line in candidates
                 if any(pattern.lower() in line.lower() for pattern in patterns)]
    if workers:
        raise FrameRefusal("active Runner.Worker/job detected; untouched")
    if conflicts:
        raise FrameRefusal("competing benchmark or CUDA process detected; untouched")
    return {"process_table_sha256": sha256_bytes(result.stdout),
            "runner_worker_matches": [], "benchmark_or_cuda_matches": [],
            "external_process_policy": "observe-only; refuse; never signal"}
...
def _admit_source(child: Path, bundle: Path, metadata: dict) -> dict:
    ...
    return {
        "schema": "libwebp-backref-cost-attribution-v7-source-admission-receipt-v1",
...
def remote_main() -> int:
    ...
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-attribution-v7-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-attribution-v7-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V7_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V7_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V7_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_attribution_v7_experiment.py", mode,
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-attribution-v7-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_attribution_v7_archive.py", "build",
    ...
    try:
        ...
        if archive is not None and declaration is not None:
            ...
            for chunk_index, offset in enumerate(range(0, len(archive), CHUNK_BYTES)):
                ...
                output.flush()
                sequence += 1
        integrity = {"schema": "libwebp-backref-cost-attribution-v7-integrity-footer-v1",
        ...
        if child is not None:
            try:
                ...
                if not absent:
                    raise FrameRefusal("exact v7 child remains after cleanup")
                cleanup = {
                    "schema": "libwebp-backref-cost-attribution-v7-cleanup-receipt-v1",
