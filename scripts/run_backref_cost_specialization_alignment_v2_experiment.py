#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost specialization alignment v2."""
...
from backref_cost_specialization_alignment_v2_admission import (
...
MANIFEST_PATH = ROOT / "scripts/backref_cost_specialization_alignment_v2_manifest.json"
...
def clean_environment(manifest: dict, variant: str,
    ...
    if stage_output is not None:
        env.update({
            ...
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_TIMERS": "1",
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    def one(name: str, recorder: bool) -> dict:
        ...
        argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
                ...
                f"WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_RECORDER={int(recorder)}",
                f"CFLAGS={flags}",
                f"LDFLAGS={manifest['build']['ldflags']},-map,{build_root / 'linker.map'}",
                "tools/backref_cost_specialization_alignment_v2_experiment_runner"]
        result = run(argv, cwd=build_root, env=env, timeout=1800)
        runner = (build_root / "tools" /
                  "backref_cost_specialization_alignment_v2_experiment_runner")
    ...
    if "AlignmentV2Record" in timed["nm"] or \
            "AlignmentV2Stats" in timed["nm"]:
        raise RuntimeError("timed binary contains recorder symbols")
    if "AlignmentV2RecordPush" not in recorder["nm"] or \
    ...
    write_json(output / "codegen.json", {
        "schema": "libwebp-backref-cost-specialization-alignment-v2-codegen-v1",
        ...
        "section": {"segment": "__TEXT", "name": "__br_align_v2",
...
def timer_overhead_gate(manifest: dict, runner: Path, output: Path) -> dict:
    ...
    if record.get("schema") != \
            "libwebp-backref-cost-specialization-alignment-v2-timer-check-v1" or \
...
def work_counter_gate(manifest: dict, runner: Path, corpus: Path,
    ...
    for case in manifest["corpus"]:
        ...
        for method in manifest["protocol"]["methods"]:
            ...
            if any(row.get("schema") !=
                   "libwebp-backref-cost-specialization-alignment-v2-work-v1"
    ...
    write_json(output / "work-metric-summary.json", {
        "schema": "libwebp-backref-cost-specialization-alignment-v2-work-summary-v1",
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path) -> list[dict]:
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-specialization-alignment-v2-output-v1"
               for item in outputs):
...
def admit_representative_payload(path: Path, expected_minimum: int) -> dict:
    ...
    source_raw = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_REHEARSAL_PAYLOAD")
    expected_sha = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_REHEARSAL_PAYLOAD_SHA256")
    expected_size_raw = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_REHEARSAL_PAYLOAD_BYTES")
...
def operator(mode: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="backref-specialization-alignment-v2-") as raw:
        temporary = Path(raw)
