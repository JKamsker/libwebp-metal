#!/usr/bin/env python3
"""Local identity and direct-child ownership for attribution-v16 transport."""
...
IDENTITY_PATHS = (
    ...
    "src/enc/vp8l_enc.c",
    "src/enc/backref_cost_attribution_v16_experiment_enc.c",
    "src/enc/backref_cost_attribution_v16_experiment_enc.h",
    "tools/backref_cost_attribution_v16_experiment_runner.c",
...
PROTOCOL_MANIFEST = ROOT / "scripts" / "backref_cost_attribution_v16_manifest.json"
SOURCE_RECEIPT_SCHEMA = \
    "libwebp-backref-cost-attribution-v16-source-admission-receipt-v1"
...
class TransportDeadlineStateMachine:
    """Pure V16 transport deadline attribution with injected monotonic time."""
...
def pump_owned_full_duplex(
    ...
    if stdout_observer is None or not hasattr(stdout_observer,
                                               "transport_state"):
        raise ValueError("V16 transport requires an online state observer")
...
def create_source_bundle() -> tuple[bytes, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v16-bundle-") as raw:
