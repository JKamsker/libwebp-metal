#!/usr/bin/env python3
"""Untimed V7 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "567e48ddfeacfaa65fb11a1611fba7aa7f5590a5"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V7_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V7_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V7_EXPERIMENT"
RUNNER = "backref_cost_attribution_v7_experiment_runner"
SOURCE = "backref_cost_attribution_v7_experiment_enc.c"
...
def run(argv: list[str], *, env: dict[str, str] | None = None,
    ...
    for name in (
        ...
        "WEBP_BACKREF_COST_ATTRIBUTION_V6_EXPERIMENT", RUNTIME,
    ):
        clean.pop(name, None)
    if env:
        clean.update(env)
...
def main() -> int:
    ...
    assert "V7 preserves V6's frozen control flow" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v7-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V7_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V7_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V7_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V7_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
