#!/usr/bin/env python3
"""Untimed V4 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "d8e015e8395a1ae3d5d17637cacf8b99bd084901"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V4_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V4_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V4_EXPERIMENT"
RUNNER = "backref_cost_attribution_v4_experiment_runner"
SOURCE = "backref_cost_attribution_v4_experiment_enc.c"
...
def run(argv: list[str], *, env: dict[str, str] | None = None,
    ...
    for name in (
        ...
        "WEBP_BACKREF_COST_ATTRIBUTION_V3_EXPERIMENT", RUNTIME,
    ):
        clean.pop(name, None)
    if env:
        clean.update(env)
...
def main() -> int:
    ...
    assert "V4 preserves V3's frozen control flow" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v4-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V4_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V4_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V4_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V4_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
