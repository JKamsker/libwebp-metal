#!/usr/bin/env python3
"""Static, workload-free ownership guards for specialization alignment v4."""
...
import backref_cost_specialization_alignment_v4_admission as admission
import backref_cost_specialization_alignment_v4_transport as transport
import execute_backref_cost_specialization_alignment_v4 as executor
import run_backref_cost_specialization_alignment_v4_experiment as operator_module


ROOT = Path(__file__).resolve().parents[1]
INFRASTRUCTURE = (
    "scripts/backref_cost_specialization_alignment_v4_admission.py",
    "scripts/backref_cost_specialization_alignment_v4_archive.py",
    "scripts/backref_cost_specialization_alignment_v4_paths.py",
    "scripts/backref_cost_specialization_alignment_v4_bootstrap.py",
    "scripts/backref_cost_specialization_alignment_v4_transport.py",
    "scripts/execute_backref_cost_specialization_alignment_v4.py",
    "scripts/run_backref_cost_specialization_alignment_v4_experiment.py",
)
...
def static_guards() -> None:
    ...
    assert signal_sites == [
        ("scripts/backref_cost_specialization_alignment_v4_admission.py", "self.process.kill")]
    assert popen_sites == [
        "scripts/backref_cost_specialization_alignment_v4_admission.py",
        "scripts/backref_cost_specialization_alignment_v4_admission.py"]
...
def online_runner_ownership_fixtures() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="alignment-v4-ledger-") as raw:
        root = Path(raw)
...
def identity_only_process_contract() -> None:
    ...
    ignored_ancestor_argv_and_source = (
        "python3 -c 'backref_cost_specialization_alignment_v4_experiment_runner "
    ...
    exact_conflicts = (
        ...
        "backref_cost_specialization_alignment_v4_experiment_runner",
    ...
    for index, identity in enumerate(exact_conflicts, start=200):
        ...
        assert diagnostic == {
            "schema":
                "libwebp-backref-cost-specialization-alignment-v4-process-refusal-v1",
    ...
    lookalikes = (
        ...
        "prefix_backref_cost_specialization_alignment_v4_experiment_runner",
        "backref_cost_specialization_alignment_v4_experiment_runner_suffix",
...
def operator_refusal_control_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v4-refusal-") as raw:
...
def single_session_contract() -> None:
    ...
    source = (ROOT / "scripts/execute_backref_cost_specialization_alignment_v4.py").read_text()
    ...
    remote = (ROOT / "scripts/backref_cost_specialization_alignment_v4_transport.py").read_text()
...
def main() -> int:
    ...
    print("PASS: specialization alignment v4 proves arbitrary opaque ancestors structurally, "
