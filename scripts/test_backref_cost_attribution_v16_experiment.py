#!/usr/bin/env python3
"""Untimed V16 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT"
RUNNER = "backref_cost_attribution_v16_experiment_runner"
SOURCE = "backref_cost_attribution_v16_experiment_enc.c"
...
def main() -> int:
    ...
    assert "V16 preserves the frozen selector-once and one-selected-DP boundary" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v16-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V16_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V16_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V16_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V16_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
