#!/usr/bin/env python3
"""Minimal remote request-channel bootstrap for Aligned Null Stage Attribution V4.
...
READY_SCHEMA = "libwebp-backref-cost-aligned-null-stage-attribution-v4-ready-v1"
DECLARATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-declaration-v1"
ACK_SCHEMA = "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-ack-v1"
CHUNK_ACK_SCHEMA = "br-null-v4-chunk-ack-v1"
RESPONSE_SCHEMA = \
...
def chained_ack(value: dict, prior_ack_digest: str) -> dict:
    """Bind one ACK to its predecessor without a self-referential digest."""
    core = {**value, "prior_ack_digest": prior_ack_digest}
    return {**core, "ack_digest": sha256(canonical_json(core))}
...
def main() -> int:
    ...
    sequence = 0
    nonce = os.environ.get("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_TRANSACTION_NONCE", "")
    expected_declaration_sha = os.environ.get(
        "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_REQUEST_DECLARATION_SHA256", "")
    bootstrap_sha = os.environ.get("WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_BOOTSTRAP_SHA256", "")
    ...
    try:
        ...
        sequence = emit(output, "request-ack", declaration_ack,
        ...
        for chunk_index in range(declaration["request_chunk_count"]):
            ...
            digest.update(chunk)
            byte_start = len(request) - len(chunk)
            chunk_ack = chained_ack({
                "schema": CHUNK_ACK_SCHEMA, "ack_kind": "chunk",
                "chunk_sha256": sha256(chunk),
                "cumulative_bytes": len(request),
                "remote_receipt_sequence": chunk_index + 1,
            }, prior_ack_digest)
            sequence = emit(output, "request-ack", chunk_ack,
                            sequence, ack_index=chunk_index + 1)
            prior_ack_digest = chunk_ack["ack_digest"]
        ...
        payload_module = types.ModuleType(
            "backref_cost_aligned_null_stage_attribution_v4_payload")
        exec(compile(values["payload-source"].decode(), "<stage-attribution-v4-payload>",
                     "exec"), payload_module.__dict__)
        sys.modules[payload_module.__name__] = payload_module
        transport_module = types.ModuleType(
            "backref_cost_aligned_null_stage_attribution_v4_remote")
        sys.modules[transport_module.__name__] = transport_module
        exec(compile(values["transport-source"].decode(),
                     "<stage-attribution-v4-transport>", "exec"),
        ...
        try:
            emit(output, "channel-refusal", {
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-channel-refusal-v1",
