#!/usr/bin/env python3
"""Local identity and direct-child ownership for specialization-alignment-v1 transport."""
...
PROTOCOL_MANIFEST = ROOT / "scripts" / "backref_cost_specialization_alignment_v1_manifest.json"
...
class TransportDeadlineStateMachine:
    """Pure Alignment V1 transport deadline attribution with injected monotonic time."""
...
def pump_owned_full_duplex(
    ...
    if stdout_observer is None or not hasattr(stdout_observer,
                                               "transport_state"):
        raise ValueError("Alignment V1 transport requires an online state observer")
...
def create_source_bundle() -> tuple[bytes, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v1-bundle-") as raw:
