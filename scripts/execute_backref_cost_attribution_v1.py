...
import backref_cost_attribution_v1_admission as admission
import backref_cost_attribution_v1_archive as archive_protocol


ROOT = Path(__file__).resolve().parents[1]
HOST = admission.HOST
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_attribution_v1_manifest.json"
EXPECTED_ENV = "WEBP_EXPECTED_BACKREF_COST_ATTRIBUTION_V1_COMMIT"
...
def remote_pipeline(output: Path, mode: str,
    ...
    temporary_holder = tempfile.TemporaryDirectory(prefix="attribution-v1-return-")
    temporary = Path(temporary_holder.name)
    try:
        remote_q = shlex.quote(record["run_directory"])
        operator = "scripts/run_backref_cost_attribution_v1_experiment.py"
