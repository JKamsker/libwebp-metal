#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization factorization v1."""
...
import run_backref_cost_specialization_factorization_v1_experiment as operator
...
def semantic_identity() -> None:
    ...
    assert current.count(
        "VP8LBackrefCostSpecializationFactorizationV1GetVariant();") == 1
    boundary = current.split("const VP8LBackrefCostSpecializationFactorizationV1Variant variant", 1)[1]
    ...
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_EXPERIMENT=1",
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_specialization_factorization_v1_admission.py").read_text()
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v1-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority": "FACTORIZATION_V1_DIAGNOSTIC_ONLY",
    ...
    diff = [
        ("A", "evidence/backref-cost-specialization-factorization-v1-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-specialization-factorization-v1-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-specialization-factorization-v1-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "c6046dc7cde6e7ea1cc9d54a6a90c55f8a2f2edb"
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_specialization_factorization_v1.py run '
                '"$PWD/evidence/backref-cost-specialization-factorization-v1-execution-20260819"')
    ...
    print("PASS: specialization factorization v1 frozen semantics, strict single-session frames, "
