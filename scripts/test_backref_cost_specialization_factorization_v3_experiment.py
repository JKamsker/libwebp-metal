...
BASE = "96f5d1881619bd5b25404d869cbb2ac2d4b3329c"
BUILD = "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_EXPERIMENT"
RECORDER = "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_RECORDER"
MACRO = "WEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_VARIANT"
RUNNER = "backref_cost_specialization_factorization_v3_experiment_runner"
...
def main() -> int:
    ...
    assert "VP8LBackrefCostSpecializationFactorizationV3GetVariant" in source
    assert "switch (variant)" in source
    assert not run(["git", "diff", "--name-only", BASE, "--", "src/webp",
                    "sharpyuv"]).stdout.strip()

    with tempfile.TemporaryDirectory(
            prefix="backref-specialization-factorization-v3-test-") as raw:
        temporary = Path(raw)
        ...
        assert "FactorizationV3RecordPush" not in timed_nm
        assert "FactorizationV3RecordPush" in recorder_nm
