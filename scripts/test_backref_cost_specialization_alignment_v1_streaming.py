#!/usr/bin/env python3
"""Workload-free Alignment V1 duplex, deadline-state, and online-stream fixtures."""
...
def declaration(archive: bytes) -> dict:
    return {
        ...
        "inventory": {
            "schema":
                "libwebp-backref-cost-specialization-alignment-v1-archive-inventory-v1",
            "status": "accepted", "member_count": 1,
            "inventory_sha256": "0" * 64},
    }
...
def deterministic_deadline_state_contracts() -> None:
    ...
    # The 46-byte V12 magic-only response started response-progress. Alignment V1 magic
...
def pump_fixture(mode: str, *, request_bytes: int, response_bytes: int,
    ...
    temporary = tempfile.TemporaryDirectory(prefix="specialization-alignment-v1-pump-")
...
def retained_child_timeout_contract(mode: str, expected_state: str,
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v1-stall-") as raw:
        root = Path(raw)
        observer = transport.OnlineResponseValidator(require_success=True)
        try:
...
def main() -> int:
    ...
    print("PASS: Alignment V1 keeps magic/control/partial declarations SILENT_OPERATOR, "
