#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost specialization factorization v1."""
...
from backref_cost_specialization_factorization_v1_admission import (
    OBSERVE_ONLY_EXTERNAL_PROCESS_CLASSES, run_owned)
import backref_cost_specialization_factorization_v1_transport as transport


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = ROOT / "scripts/backref_cost_specialization_factorization_v1_manifest.json"
...
def clean_environment(manifest: dict, variant: str,
    ...
    if stage_output is not None:
        env.update({
            ...
            "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_TIMERS": "1",
            "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    def one(name: str, recorder: bool) -> dict:
        ...
        argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
                "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_EXPERIMENT=1",
                f"WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_RECORDER={int(recorder)}",
                f"CFLAGS={flags}",
                f"LDFLAGS={manifest['build']['ldflags']}",
                "tools/backref_cost_specialization_factorization_v1_experiment_runner"]
        result = run(argv, cwd=build_root, env=env, timeout=1800)
        runner = (build_root / "tools" /
                  "backref_cost_specialization_factorization_v1_experiment_runner")
        if not runner.is_file():
            raise RuntimeError("factorization runner missing after build")
        nm_text = run([manifest["diagnostic"]["executables"]["nm"]["path"],
    ...
    if "FactorizationV1Record" in timed["nm"] or \
            "FactorizationV1Stats" in timed["nm"]:
        raise RuntimeError("timed binary contains recorder symbols")
    if "FactorizationV1RecordPush" not in recorder["nm"] or \
    ...
    disassembly = run([
        manifest["diagnostic"]["executables"]["xcrun"]["path"],
        "llvm-objdump", "--disassemble-symbols=" + ",".join(
    ...
    for line in disassembly.splitlines():
        ...
        if label:
            ...
            continue
        instruction = re.match(r"^\s*[0-9a-f]+:\s+([0-9a-f]+)\s+(\S+)", line)
    ...
    for left, right in equivalence_pairs:
        if facts[left]["mnemonics"] != facts[right]["mnemonics"]:
            raise RuntimeError(f"matched-layout mnemonic gate failed: {left}/{right}")
    if facts["PushIntervalSpecialized"]["mnemonics"] == \
            facts["PushIntervalLayoutClone"]["mnemonics"]:
        raise RuntimeError("hint body unexpectedly equals layout clone")
    for fact in facts.values():
        del fact["mnemonics"]
    write_json(output / "codegen.json", {
        "schema": "libwebp-backref-cost-specialization-factorization-v1-codegen-v1",
...
def timer_overhead_gate(manifest: dict, runner: Path, output: Path) -> dict:
    ...
    if record.get("schema") != \
            "libwebp-backref-cost-specialization-factorization-v1-timer-check-v1" or \
...
def work_counter_gate(manifest: dict, runner: Path, corpus: Path,
    ...
    for case in manifest["corpus"]:
        ...
        for method in manifest["protocol"]["methods"]:
            values = {}
            for variant in ("B", "L", "H"):
                result = run([str(runner), "work", str(source),
            ...
            if any(row.get("schema") !=
                   "libwebp-backref-cost-specialization-factorization-v1-work-v1"
    ...
    write_json(output / "work-metric-summary.json", {
        "schema": "libwebp-backref-cost-specialization-factorization-v1-work-summary-v1",
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path) -> list[dict]:
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-specialization-factorization-v1-output-v1"
               for item in outputs):
...
def admit_representative_payload(path: Path, expected_minimum: int) -> dict:
    ...
    source_raw = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_REHEARSAL_PAYLOAD")
    expected_sha = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_REHEARSAL_PAYLOAD_SHA256")
    expected_size_raw = os.environ.get(
        "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V1_REHEARSAL_PAYLOAD_BYTES")
...
def operator(mode: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="backref-specialization-factorization-v1-") as raw:
        temporary = Path(raw)
