...
import backref_cost_attribution_v5_archive as archive_protocol


ROOT = Path(__file__).resolve().parents[1]
HOST = admission.HOST
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_attribution_v5_manifest.json"
EXPECTED_ENV = "WEBP_EXPECTED_BACKREF_COST_ATTRIBUTION_V5_COMMIT"
...
def lease_transfer_contract(mode: str, attempted: bool,
    ...
    if not attempted:
        return ({
            "schema":
                "libwebp-backref-cost-attribution-v5-lease-transfer-v1",
    ...
    return ({
        "schema": "libwebp-backref-cost-attribution-v5-lease-transfer-v1",
...
def remote_pipeline(output: Path, mode: str,
    ...
    if not MANIFEST_PATH.is_file():
        raise RuntimeError("frozen attribution-v5 manifest is absent")
    ...
    temporary_holder = tempfile.TemporaryDirectory(prefix="attribution-v5-return-")
    temporary = Path(temporary_holder.name)
    try:
        remote_q = shlex.quote(record["run_directory"])
        operator = "scripts/run_backref_cost_attribution_v5_experiment.py"
    ...
    write_json(protocol / "control-plane-order.json", {
        "schema": "libwebp-backref-cost-attribution-v5-control-plane-v1",
