#!/usr/bin/env python3
"""Untimed, workload-free contract tests for attribution v6."""
...
import run_backref_cost_attribution_v6_experiment as operator
...
def semantic_identity() -> None:
    ...
    v6_boundary = current.split(
        "// V6 preserves V5's frozen control flow and timer ownership exactly.",
        1)[1]
    v6_boundary = v6_boundary.split("#else", 1)[0]
    assert v6_boundary.count(
        "VP8LBackrefCostAttributionV6ExperimentEnabled()") == 1
    assert v6_boundary.count(
        "BackwardReferencesHashChainDistanceOnlySpecialized(") == 1
    assert v6_boundary.count("BackwardReferencesHashChainDistanceOnly(") == 1
    assert "VP8LBackrefCostAttributionV6RecordSelector();" in v6_boundary
    assert "if (use_candidate)" in v6_boundary and "} else {" in v6_boundary
    ...
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V6_EXPERIMENT=1",
...
def lease_transfer_truth_table() -> None:
    ...
    def completed_fetch():
        ...
        return ({"hashes_match": True}, {
            "schema": "libwebp-backref-cost-attribution-v6-lease-v1",
...
def authority_rejection_fixtures() -> None:
    source = "1" * 40
    source_tree = "2" * 40
    freeze = "3" * 40
    freeze_tree = "4" * 40
    ...
    receipt_paths = (
        ...
        "return-protocol/source-admission-receipt.json",
    ...
    envelope = {
        ...
        "observed_transport_calibration": {
            "archive_bytes": 4 * 1024 * 1024,
            "chunk_count": 8, "elapsed_ns": 1,
        },
        "timed_execution_authorized": True,
    }
    diff = [
        ("A", "evidence/backref-cost-attribution-v6-design/rehearsal/raw.json"),
        ("A", "evidence/backref-cost-attribution-v6-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v6-design/phase1b-authorization.json"),
    ]
    kwargs = {
        "envelope": envelope, "head_commit": freeze,
        "head_tree": freeze_tree, "parents": [source],
        "source_tree": source_tree, "diff_entries": diff,
        "source_hashes": {"manifest": digest, "protocol": digest,
                          "schemas": digest},
    ...
    fixtures = {}
    fixtures["wrong-parent"] = {**kwargs, "parents": ["9" * 40]}
    fixtures["modified-frozen-artifact"] = {
    ...
    fixtures["stale-receipt"] = stale
    fixtures["wrong-tree"] = {**kwargs, "source_tree": "7" * 40}
    fixtures["wrong-hash"] = {**kwargs, "hash_index_sha256": "6" * 64}
    fixtures["missing-envelope"] = {**kwargs, "envelope": None}
    fixtures["extra-phase1b-path"] = {
        **kwargs, "diff_entries": diff + [("A", "unexpected.txt")]}
    for label, fixture in fixtures.items():
        try:
            executor.validate_authority_values(**fixture)
        except RuntimeError:
            pass
        else:
            raise AssertionError(f"authority rejection fixture accepted: {label}")


def main() -> int:
    manifest = json.loads(MANIFEST_PATH.read_text())
    decision = manifest["phase_1_decision"]
    assert decision["status"] == "PENDING_PHASE_1B_AUTHORIZATION"
    assert decision["timed_execution_authorized"] is False
    assert manifest["base_commit"] == \
        "c9d21ac9c512c94f036fc2db7eb118da83b4f7f9"
    ...
    authority_rejection_fixtures()
    source = (ROOT / "scripts/run_backref_cost_attribution_v6_experiment.py").read_text()
    ...
    executor_source = (
        ROOT / "scripts/execute_backref_cost_attribution_v6.py").read_text()
    ...
    print("PASS: attribution v6 exact specialization identity, one selected DP "
