#!/usr/bin/env python3
"""Untimed V8 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "fea767272f7d9d13e5e5f19cb910e6152418dcdd"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V8_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V8_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V8_EXPERIMENT"
RUNNER = "backref_cost_attribution_v8_experiment_runner"
SOURCE = "backref_cost_attribution_v8_experiment_enc.c"
...
def run(argv: list[str], *, env: dict[str, str] | None = None,
    ...
    for name in (
        ...
        "WEBP_BACKREF_COST_ATTRIBUTION_V7_EXPERIMENT", RUNTIME,
    ):
        clean.pop(name, None)
    if env:
        clean.update(env)
...
def main() -> int:
    ...
    assert "V8 preserves V7's frozen control flow" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v8-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V8_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V8_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V8_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V8_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
