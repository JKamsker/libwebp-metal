#!/usr/bin/env python3
"""Untimed specialization-factorization-v4 path, admission, frame, and mode tests."""
...
from backref_cost_specialization_factorization_v4_paths import (
...
def path_tests() -> None:
    ...
    assert SAFE_ROOT_BASENAME.endswith("specialization-factorization-v4")
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v4-path-home-") as raw:
...
def repository_context_admission() -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v4-admit-") as raw:
        root = Path(raw)
        ...
        subprocess.run(["git", "-C", str(repository), "config", "user.name",
                        "factorization-v4 admission test"], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.email",
                        "factorization-v4@example.invalid"], check=True)
...
def remote_refusal_cleanup_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v4-remote-home-") as raw:
...
def main() -> int:
    ...
    mode_truth_table()
    print("PASS: specialization factorization v4 canonical-only paths, repository-context source "
