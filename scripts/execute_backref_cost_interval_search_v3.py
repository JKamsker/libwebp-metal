#!/usr/bin/env python3
"""Disjoint local validation, remote rehearsal, and frozen v3 execution."""
...
EXPECTED_ENV = "WEBP_EXPECTED_BACKREF_COST_INTERVAL_SEARCH_V3_COMMIT"
...
def remote_pipeline(output: Path, mode: str,
    ...
    if not MANIFEST_PATH.is_file():
        raise RuntimeError("frozen interval-search v3 manifest is absent")
    ...
    temporary_holder = tempfile.TemporaryDirectory(prefix="interval-search-v3-return-")
    temporary = Path(temporary_holder.name)
    try:
        remote_q = shlex.quote(record["run_directory"])
        operator = "scripts/run_backref_cost_interval_search_v3_experiment.py"
