#!/usr/bin/env python3
"""Framed single-session transport for aligned-null stage attribution v4.
...
import backref_cost_aligned_null_stage_attribution_v4_payload as payload_generator


MAGIC = b"LIBWEBP-BACKREF-COST-ALIGNED-NULL-STAGE-ATTRIBUTION-V4-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 1
CHANNEL_MAGIC = b"LIBWEBP-BACKREF-COST-ALIGNED-NULL-STAGE-ATTRIBUTION-V4-CHANNEL\x00\x01\n"
CHANNEL_READY_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-ready-v1"
REQUEST_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-declaration-v1"
REQUEST_ACK_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-ack-v1"
REQUEST_CHUNK_ACK_SCHEMA = "br-null-v4-chunk-ack-v1"
RESPONSE_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-response-declaration-v1"
RESPONSE_COMPLETE_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-response-complete-v1"
...
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-aligned-null-stage-attribution-v4"
RUN_PREFIX = "run."
...
BULK_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-archive-v1"
SOURCE_RECEIPT_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-source-admission-receipt-v1"
SOURCE_DETAILS_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-source-admission-details-v1"
...
DENIED_RAW_COMM_IDENTITIES = frozenset({
    ...
    "backref_cost_interval_search_v4_experiment_runner",
...
class ProcessRefusal(FrameRefusal):
    ...
    def __init__(self, failure_code: str, *, row: dict | None = None,
        ...
        self.diagnostic = {
            "schema":
                "libwebp-backref-cost-aligned-null-stage-attribution-v4-process-refusal-v1",
...
class ChannelRequest:
    """Exact V4 request bytes and its canonical declaration."""
...
class OnlineChannelValidator:
    ...
    def __init__(self, request: ChannelRequest, *, transaction_nonce: str,
        ...
        self.request_progress_chain: list[dict] = []
        self.declaration_ack_digest: str | None = None
        self._client_chunk_write_times: dict[int, dict[str, int]] = {}
        self.client_transport_state = {
            "request_chunks_written": 0,
            "pending_kind": None,
            "pending_chunk_ordinal": None,
    ...
    @staticmethod
    def _ack_digest(value: dict) -> str:
        core = {key: item for key, item in value.items()
                if key != "ack_digest"}
        return sha256_bytes(canonical_json(core))

    def note_chunk_write_start(self, ordinal: int, when_ns: int) -> None:
        row = self._client_chunk_write_times.setdefault(ordinal, {})
        if "write_start_monotonic_ns" in row:
            self._reject("chunk write-start was recorded twice")
        row["write_start_monotonic_ns"] = when_ns

    def note_chunk_write_complete(self, ordinal: int, when_ns: int) -> None:
        row = self._client_chunk_write_times.setdefault(ordinal, {})
        if "write_complete_monotonic_ns" in row or \
                "write_start_monotonic_ns" not in row:
            self._reject("chunk write-complete ordering is invalid")
        row["write_complete_monotonic_ns"] = when_ns

    def progress_chain_snapshot(self) -> list[dict]:
        return [dict(row) for row in self.request_progress_chain]
    ...
    def _complete_frame(self, header: dict, payload: bytes) -> None:
        ...
        if kind == "request-ack":
            ...
            if value is None or value.get("schema") not in (
                    REQUEST_ACK_SCHEMA, REQUEST_CHUNK_ACK_SCHEMA):
                self._reject("request ACK identity is invalid")
                return
            ack_kind = value.get("ack_kind")
            if ack_kind == "declaration":
                if value.get("schema") != REQUEST_ACK_SCHEMA or \
                        self.declaration_acked or self.request_chunks_acked or \
                        ...
                        value.get("remote_receipt_sequence") != 0 or \
                        value.get("prior_ack_digest") != "0" * 64 or \
                        value.get("ack_digest") != self._ack_digest(value) or \
                        header.get("ack_index") != 0:
                    self._reject("request declaration ACK is duplicate/out of order")
                else:
                    self.declaration_acked = True
                    self.declaration_ack_digest = value["ack_digest"]
                return
            if ack_kind == "chunk":
                expected_index = self.request_chunks_acked + 1
                expected_start = (expected_index - 1) * REQUEST_CHUNK_BYTES
                expected_bytes = min(
                    expected_index * REQUEST_CHUNK_BYTES,
                    len(self.request.value))
                expected_chunk = self.request.value[expected_start:expected_bytes]
                prior_digest = (self.declaration_ack_digest if expected_index == 1
                                else self.request_progress_chain[-1]["ack_digest"])
                if value.get("schema") != REQUEST_CHUNK_ACK_SCHEMA or \
                        not self.declaration_acked or self.request_complete is not None or \
                        value.get("chunk_sha256") != sha256_bytes(expected_chunk) or \
                        value.get("cumulative_bytes") != expected_bytes or \
                        value.get("prior_ack_digest") != prior_digest or \
                        value.get("remote_receipt_sequence") != expected_index or \
                        value.get("ack_digest") != self._ack_digest(value) or \
                        header.get("ack_index") != expected_index:
                    self._reject("request chunk ACK range/digest/chain is invalid")
                else:
                    times = self._client_chunk_write_times.get(expected_index, {})
                    received_ns = time.monotonic_ns()
                    if set(times) != {"write_start_monotonic_ns",
                                     "write_complete_monotonic_ns"} or \
                            times["write_complete_monotonic_ns"] > received_ns:
                        self._reject("client chunk write/ACK timestamps are invalid")
                        return
                    self.request_chunks_acked = expected_index
                    self.request_bytes_acked = expected_bytes
                    self.request_progress_chain.append({
                        "transaction_nonce": self.transaction_nonce,
                        "chunk_ordinal": expected_index,
                        "byte_start": expected_start,
                        "byte_end_exclusive": expected_bytes,
                        "chunk_bytes": len(expected_chunk),
                        "cumulative_bytes": expected_bytes,
                        "chunk_sha256": value["chunk_sha256"],
                        "prior_ack_digest": value["prior_ack_digest"],
                        "remote_receipt_sequence": value["remote_receipt_sequence"],
                        "ack_digest": value["ack_digest"],
                        **times,
                        "ack_received_monotonic_ns": received_ns,
                        "ack_latency_ns": received_ns -
                            times["write_complete_monotonic_ns"],
                    })
                return
    ...
    def finalize(self) -> dict:
        ...
        return {
            ...
            "request_progress_chain": self.progress_chain_snapshot(),
    ...
    def _valid_bulk_declaration(self, declaration: dict) -> bool:
        ...
        valid = (
            ...
            inventory.get("schema") ==
                "libwebp-backref-cost-aligned-null-stage-attribution-v4-archive-inventory-v1" and
        ...
        if process_refusal is not None:
            ...
            if not isinstance(process_refusal, dict) or \
                    ...
                    process_refusal.get("schema") != (
                        "libwebp-backref-cost-aligned-null-stage-attribution-v4-process-refusal-v1") or \
...
def remote_transaction(input_stream) -> tuple[int, bytes]:
    ...
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_aligned_null_stage_attribution_v4_experiment.py", mode,
        ...
        _add_indexed_detail(
            evidence, "remote-admission-process-isolation.json", {
                "schema":
                    "libwebp-backref-cost-aligned-null-stage-attribution-v4-remote-process-isolation-v1",
                "status": "verified",
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_aligned_null_stage_attribution_v4_archive.py", "build",
    ...
    integrity = {
        "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-integrity-footer-v1",
    ...
    if child is not None:
        try:
            ...
            cleanup = {
                "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-cleanup-receipt-v1",
