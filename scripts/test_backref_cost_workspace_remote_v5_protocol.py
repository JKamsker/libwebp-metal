#!/usr/bin/env python3
"""Untimed static validation of the frozen remote-v5 protocol."""
...
MANIFEST = ROOT / "scripts" / "backref_cost_workspace_remote_v5_manifest.json"
CATALOG = ROOT / "scripts" / "backref_cost_workspace_remote_v5_schemas.json"
EXPECTED = "WEBP_EXPECTED_BACKREF_COST_WORKSPACE_REMOTE_V5_COMMIT"
...
def main() -> int:
    ...
    assert manifest["schema"] == \
        "libwebp-backref-cost-workspace-remote-v5-frozen-protocol-v1"
    assert manifest["protocol_version"] == 5
    assert catalog["protocol_version"] == 5
    assert manifest["base_commit"] == "978e1b3e12e710c25f465fe1c8a68c359a38004e"
    ...
    assert manifest["remote_host"]["safe_root_basename"] == \
        ".libwebp-metal-backref-cost-workspace-remote-v5"
    ...
    assert design_inventory["remote_exact_build_commands"] == 7
    assert design_inventory["remote_correctness_cells"] == 26
    assert design_inventory["toolchain_refusal_fixtures"] == 2
    assert design_inventory["remote_build_archive_members"] == 7
    ...
    operator = (ROOT / "scripts" /
                "run_backref_cost_workspace_remote_v5_experiment.py").read_text()
    driver = (ROOT / "scripts" /
              "execute_backref_cost_workspace_remote_v5.py").read_text()
    path_guard = (ROOT / "scripts" /
                  "backref_cost_workspace_remote_v5_paths.py").read_text()
    admission = (ROOT / "scripts" /
                 "backref_cost_workspace_remote_v5_admission.py").read_text()
    archive = (ROOT / "scripts" /
               "backref_cost_workspace_remote_v5_archive.py").read_text()
    ...
    design = ROOT / "evidence" / "backref-cost-workspace-remote-v5-design"
    live = json.loads((design / "live-validation.json").read_text())
    required_live = catalog["schemas"][
        "libwebp-backref-cost-workspace-remote-v5-live-return-validation-v1"][
            "required"]
    ...
    with tempfile.TemporaryDirectory(prefix="remote-v5-refusal-") as raw:
        output = Path(raw) / "must-not-exist"
        refused = execute([
            sys.executable,
            "scripts/run_backref_cost_workspace_remote_v5_experiment.py",
            "run", str(output)])
    ...
    print("PASS: v5 schemas, hashes, matrix inventory, session refusals, "
