...
BASE = "91ab79b70bb1f99888dce63e8b1ac512741bafb3"
BUILD = "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_EXPERIMENT"
RECORDER = "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_RECORDER"
MACRO = "WEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_VARIANT"
RUNNER = "backref_cost_specialization_alignment_v4_experiment_runner"
...
def main() -> int:
    ...
    assert "VP8LBackrefCostSpecializationAlignmentV4GetVariant" in source
    assert "switch (variant)" in source
    assert not run(["git", "diff", "--name-only", BASE, "--", "src/webp",
                    "sharpyuv"]).stdout.strip()

    with tempfile.TemporaryDirectory(
            prefix="backref-specialization-alignment-v4-test-") as raw:
        temporary = Path(raw)
        ...
        assert "AlignmentV4RecordPush" not in timed_nm
        assert "AlignmentV4RecordPush" in recorder_nm
        ...
        for variant in ("B", "L", "H"):
            ...
            assert stages[0]["schema"].endswith(
                "timer-accounting-stage-v1")
