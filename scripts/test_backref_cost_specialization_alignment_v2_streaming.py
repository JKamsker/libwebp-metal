#!/usr/bin/env python3
"""Workload-free Alignment V2 duplex, deadline-state, and online-stream fixtures."""
...
import backref_cost_specialization_alignment_v2_admission as admission
...
def source_receipt():
    return {
        ...
        "source_tree": "2" * 40, "source_bundle_sha256": "3" * 64,
        "source_map_entry_count": 1, "source_map_sha256": "4" * 64,
...
sys.stdout.buffer.flush()
emit("source-admission", source_receipt(), 0)
...
declaration = {
    ...
    "inventory": {
        "schema": "libwebp-backref-cost-specialization-alignment-v2-archive-inventory-v1",
...
def controls() -> bytes:
    return b"".join([
        transport.encode_frame(
            "source-admission", transport.canonical_json(source_receipt()),
            sequence=0),
        transport.encode_frame(
...
def declaration(archive: bytes) -> dict:
    return {
        ...
        "inventory": {
            "schema":
                "libwebp-backref-cost-specialization-alignment-v2-archive-inventory-v1",
            "status": "accepted", "member_count": 1,
            "inventory_sha256": "0" * 64},
    }
...
def deterministic_deadline_state_contracts() -> None:
    ...
    # The 46-byte V12 magic-only response started response-progress. Alignment V2 magic
...
def pump_fixture(mode: str, *, request_bytes: int, response_bytes: int,
    ...
    temporary = tempfile.TemporaryDirectory(prefix="specialization-alignment-v2-pump-")
...
def simultaneous_duplex_contract() -> None:
    ...
    try:
        ...
        reparsed = transport.parse_response_file(
            ...
            pre_archive_validator=lambda receipt: pre_archive_checks.append(
                receipt["source_map_sha256"])
                if not archive.exists() else
                (_ for _ in ()).throw(AssertionError(
                    "archive existed before compact source validation")))
        assert pre_archive_checks == ["4" * 64]
...
def retained_child_timeout_contract(mode: str, expected_state: str,
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v2-stall-") as raw:
        root = Path(raw)
        observer = transport.OnlineResponseValidator(require_success=True)
        try:
...
def complete_response_preflight_contract() -> None:
    archive = b"A" * transport.MAX_ARCHIVE_BYTES
    source = source_receipt()
    operator = {"status": "complete",
                "benchmark_lease_acquisition_attempted": False}
    archive_status = {"status": "accepted", "builder_exit_code": 0}
    lease = {"state": "not-attempted", "acquisition_attempted": False}
    integrity = {"status": "complete",
                 "archive_sha256": transport.sha256_bytes(archive)}
    cleanup = {"status": "complete",
               "removed_exact_validated_child": True,
               "absence_proved": True}
    value = transport.preflight_response(
        source, operator, archive_status, lease, archive,
        declaration(archive), integrity, cleanup)
    assert len(value) < transport.MAX_RESPONSE_BYTES
    assert transport.parse_response(value, require_success=True)["archive"] == archive
    try:
        transport.preflight_response(
            source, operator, archive_status, lease, archive + b"X",
            declaration(archive + b"X"), integrity, cleanup)
    except transport.FrameRefusal:
        pass
    else:
        raise AssertionError("oversized archive passed response preflight")
    original_cap = transport.MAX_RESPONSE_BYTES
    try:
        transport.MAX_RESPONSE_BYTES = len(value)
        try:
            transport.preflight_response(
                source, operator, archive_status, lease, archive,
                declaration(archive), integrity, cleanup)
            ...
            raise AssertionError("non-strict response cap passed preflight")
    finally:
        transport.MAX_RESPONSE_BYTES = original_cap
...
def main() -> int:
    ...
    complete_response_preflight_contract()
    print("PASS: Alignment V2 keeps magic/control/partial declarations SILENT_OPERATOR, "
          ...
          "persists duplex hashes/counts, preflights the complete response, "
          "and times out only its retained child")
    return 0
