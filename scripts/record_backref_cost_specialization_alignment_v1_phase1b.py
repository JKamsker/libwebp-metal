#!/usr/bin/env python3
"""Record the sole alignment-v1 rehearsal as additions-only Phase 1B evidence."""
...
DESIGN = ROOT / "evidence/backref-cost-specialization-alignment-v1-design"
...
PROTOCOL = ROOT / "doc/backref-cost-specialization-alignment-v1-protocol-20260820.md"
SCHEMAS = ROOT / "scripts/backref_cost_specialization_alignment_v1_schemas.json"
TRANSPORT = ROOT / "scripts/backref_cost_specialization_alignment_v1_transport.py"
...
def success_gate(source_commit: str, source_tree: str,
    ...
    checks = {
        ...
        "source admission verified": source.get("status") == "verified",
        ...
        "observe-only process isolation":
            ...
            not resources.get("process_isolation", {}).get("exact_identity_conflicts"),
...
def main() -> int:
    ...
    rows = artifact_rows()
    index = {
        "schema": "libwebp-backref-cost-specialization-alignment-v1-phase1b-hash-index-v1",
    ...
    authorization = {
        "schema": "libwebp-backref-cost-specialization-alignment-v1-authorization-v1",
        "authorization_status": "AUTHORIZED" if passed else "REFUSED",
        "status": "AUTHORIZED" if passed else "REFUSED",
        "phase_2_authority": "ALIGNMENT_V1_DIAGNOSTIC_ONLY" if passed else "NONE",
        "timed_execution_authorized": passed,
        "authorization_scope": (
            "Phase 2 may make exactly one frozen alignment-v1 diagnostic transaction; no production promotion, Phase 3, retry, or other workload is authorized."
            if passed else "Phase 2 is unauthorized."),
        "base_commit": "adf680e69b9eeeef636e62e0e0eb0c8fdace4013",
        "branch": "codex/backref-cost-specialization-alignment-v1",
        "source_commit": source_commit, "source_tree": source_tree,
