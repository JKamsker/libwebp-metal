#!/usr/bin/env python3
"""Frozen interval-search-v2 host operator for the backref-cost workspace interval-search v2 A/B."""
...
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_interval_search_v2_manifest.json"
...
def toolchain_admission(manifest: dict,
    ...
    with tempfile.TemporaryDirectory(prefix="interval-search-v2-toolchain-probe-") as raw:
...
def configure_and_build(manifest: dict, temporary: Path, log,
    ...
    for variant in ("baseline", "candidate", "diagnostic"):
        ...
        make = [
            ...
            "WEBP_ENABLE_METAL=0",
            "WEBP_BACKREF_COST_INTERVAL_SEARCH_V2_DETERMINISTIC_BUILD=1",
            f"{manifest['candidate']['build_flag']}=" +
            ("0" if variant == "baseline" else "1"),
            "examples/cwebp", "examples/dwebp",
        ]
        if variant == "diagnostic":
            make.append("WEBP_BACKREF_COST_INTERVAL_SEARCH_V2_RECORDER=1")
        if variant != "baseline":
...
def record_stage(output: Path, stage: str) -> None:
    write_json(output / "operator-stage.json", {
        "schema": "libwebp-backref-cost-interval-search-v2-stage-v1",
        "stage": stage,
        "captured_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    })
...
def execute(manifest: dict, output: Path) -> None:
        ...
        with lease_path.open("a+") as lease:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory(
                        prefix="backref-cost-interval-search-v2-build-") as raw:
                    temporary = Path(raw)
    ...
    (output / "operator-report.md").write_text(
        "# Remote backref-cost workspace interval-search v2 A/B operator report\n\n"
        ...
        "Only this interval-search v2 matrix is eligible. Timing was remote, serial, "
...
def controlled_run(output: Path) -> None:
    ...
    bootstrap_lease = {
        "schema": "libwebp-backref-cost-interval-search-v2-lease-v1",
    ...
    try:
        ...
        write_json(output / "refusal.json", {
            "schema": "libwebp-backref-cost-interval-search-v2-refusal-v1",
    ...
    write_json(output / "operator-status.json", {
        "schema": "libwebp-backref-cost-interval-search-v2-operator-status-v1",
        "status": status, "artifact_index_written": True,
        "controlled_exit": True,
    })
    artifact_schema = (manifest["schemas"]["artifact_hashes"] if manifest
                       else "libwebp-backref-cost-interval-search-v2-artifact-hashes-v1")
    ...
    if failure is not None:
        raise RuntimeError(
            "controlled interval-search-v2 refusal; indexed evidence is returnable") from failure
...
def rehearsal(manifest: dict, output: Path) -> None:
    ...
    fixture_results = {}
    fixtures = {
        "missing_make": {"make": "/nonexistent/interval-search-v2-fixture/make"},
    ...
    with tempfile.TemporaryDirectory(
            prefix="backref-cost-interval-search-v2-rehearsal-") as raw:
        temporary = Path(raw)
        with (output / "build.log").open("wb") as log:
            builds = configure_and_build(
                manifest, temporary, log, output, commit)
...
def refusal_fixture(manifest: dict, output: Path, fixture: str) -> None:
    ...
    overrides = {
        "missing": {"make": "/nonexistent/interval-search-v2-fixture/make"},
