#!/usr/bin/env python3
"""Static, workload-free ownership guards for specialization alignment v2."""
...
import run_backref_cost_specialization_alignment_v2_experiment as operator_module


ROOT = Path(__file__).resolve().parents[1]
INFRASTRUCTURE = (
    "scripts/backref_cost_specialization_alignment_v2_admission.py",
    "scripts/backref_cost_specialization_alignment_v2_archive.py",
    ...
    "scripts/run_backref_cost_specialization_alignment_v2_experiment.py",
)
...
def static_guards() -> None:
    ...
    assert signal_sites == [
        ("scripts/backref_cost_specialization_alignment_v2_admission.py", "self.process.kill")]
    assert popen_sites == ["scripts/backref_cost_specialization_alignment_v2_admission.py"]
...
def identity_only_process_contract() -> None:
    ...
    ignored_ancestor_argv_and_source = (
        "python3 -c 'backref_cost_specialization_alignment_v2_experiment_runner "
    ...
    lookalikes = (
        ...
        "prefix_backref_cost_specialization_alignment_v2_experiment_runner",
        "backref_cost_specialization_alignment_v2_experiment_runner_suffix",
...
def operator_refusal_control_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v2-refusal-") as raw:
...
def single_session_contract() -> None:
    ...
    source = (ROOT / "scripts/execute_backref_cost_specialization_alignment_v2.py").read_text()
    ...
    remote = (ROOT / "scripts/backref_cost_specialization_alignment_v2_transport.py").read_text()
...
def main() -> int:
    ...
    print("PASS: specialization alignment v2 proves arbitrary opaque ancestors structurally, "
