#!/usr/bin/env python3
"""Frozen independent evaluator for specialization alignment v4 evidence."""
...
def classify_effect(row: dict) -> str:
    if abs(row["mean_percent"]) < 0.10:
        return "indeterminate-small"
    low, high = row["bootstrap_mean_ci95_percent"]
    if high < 0:
...
def null_control(row: dict, bounds: dict) -> dict:
    low, high = row["bootstrap_mean_ci95_percent"]
    checks = {
        "absolute_mean_within_bound":
            abs(row["mean_percent"]) <=
            bounds["per_cell_absolute_mean_percent_max"],
        "bootstrap_ci_contains_zero": low <= 0 <= high,
        "bootstrap_ci_endpoints_within_bound":
            max(abs(low), abs(high)) <=
            bounds["per_cell_bootstrap_95_ci_endpoint_absolute_percent_max"],
        "all_paired_rows_within_bound":
            max(abs(value) for value in row["paired_percentages"]) <=
            bounds["per_paired_row_absolute_percent_max"],
    }
...
def evaluate(evidence: Path) -> dict:
    ...
    if len(work) != 12 or summary.get("rows") != 12 or \
            ...
            codegen.get("alignment_boundary_bytes") != 16384 or \
            codegen.get("inter_entry_padding_bytes") != 77588 or \
            codegen.get("dp_to_push_displacement_bytes") != 49152 or \
            any(int(address, 16) % 16384 for address in
                codegen.get("symbol_addresses", {}).values()) or \
    ...
    null_bounds = manifest["acceptance"]["mandatory_b_to_l_null_control"]
    null_checks = []
    for cell, metrics in statistics_rows.items():
        for metric in METRICS:
            result = null_control(metrics[metric]["B_to_L"], null_bounds)
            null_checks.append({"cell": cell, "metric": metric, **result})
    null_passes = all(row["passes"] for row in null_checks)
    if null_passes:
        for cell in classifications:
            for metric in METRICS:
                classifications[cell][metric] = {
                    "L_to_H": classify_effect(
                        statistics_rows[cell][metric]["L_to_H"]),
                    "B_to_H": classify_effect(
                        statistics_rows[cell][metric]["B_to_H"]),
                }
    else:
        for cell in classifications:
            for metric in METRICS:
                classifications[cell][metric] = "invalid-b-to-l-null-control"
        ...
        for cell, metrics in statistics_rows.items():
            for metric in ("dp_steady", "whole_encode"):
                ...
                promotion_checks.append({
                    ...
                    "upper_ci_nonpositive":
                        row["bootstrap_mean_ci95_percent"][1] <= 0,
                    "no_material_paired_regression":
                        row["maximum_percent"] <= material,
                })
    production = null_passes and all(all(value for key, value in row.items()
    ...
    return {
        "schema": "libwebp-backref-cost-specialization-alignment-v4-evaluation-v1",
        "status": "PASS_DIAGNOSTIC" if null_passes else
                  "FAIL_B_TO_L_NULL_CONTROL",
        "integrity": integrity,
        ...
        "mandatory_b_to_l_null_control": {
            "passes": null_passes, "checks": null_checks,
            "failure_consequence": null_bounds["failure_consequence"],
        },
