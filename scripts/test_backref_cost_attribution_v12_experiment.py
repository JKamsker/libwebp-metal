#!/usr/bin/env python3
"""Untimed V12 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "6e6fa6905b67c65a10b3a0e80111fd93d9a82552"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V12_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V12_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V12_EXPERIMENT"
RUNNER = "backref_cost_attribution_v12_experiment_runner"
SOURCE = "backref_cost_attribution_v12_experiment_enc.c"
...
def main() -> int:
    ...
    assert "V12 preserves V11's frozen control flow" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v12-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V12_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V12_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V12_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V12_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
