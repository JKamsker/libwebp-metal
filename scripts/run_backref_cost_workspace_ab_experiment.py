#!/usr/bin/env python3
"""Frozen v1 remote-host operator for the backref-cost workspace A/B."""
...
def disk_available(path: Path) -> int:
    rows = capture(["df", "-Pk", str(path)]).splitlines()
    if len(rows) != 2:
...
def host_preflight(manifest: dict, output: Path | None = None,
    ...
    return {
        ...
        "available_disk_bytes": disk, "compiler_path": compiler_path,
...
def execute(manifest: dict, output: Path) -> None:
    ...
    try:
        with lease_path.open("a+") as lease:
            ...
            with tempfile.TemporaryDirectory(
                    prefix="backref-cost-workspace-ab-build-") as raw:
                temporary = Path(raw)
    ...
    (output / "operator-report.md").write_text(
        "# Remote backref-cost workspace A/B operator report\n\n"
        ...
        "No old refusal row was counted. Timing was remote, serial, recorder-free, "
        "and end-to-end only.\n")
