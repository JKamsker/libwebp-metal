#!/usr/bin/env python3
"""Frozen local driver for one specialization-alignment-v4 single-session transaction."""
...
import backref_cost_specialization_alignment_v4_archive as archive_protocol
...
PROTOCOL_PATH = ROOT / "doc" / "backref-cost-specialization-alignment-v4-protocol-20260820.md"
TRANSPORT_PATH = ROOT / "scripts" / "backref_cost_specialization_alignment_v4_transport.py"
BOOTSTRAP_PATH = ROOT / "scripts" / "backref_cost_specialization_alignment_v4_bootstrap.py"
PAYLOAD_GENERATOR_PATH = \
    ROOT / "scripts" / "backref_cost_specialization_alignment_v4_payload.py"
AUTHORIZATION_PATH = (ROOT / "evidence" / "backref-cost-specialization-alignment-v4-design" /
                      "phase1b-authorization.json")
REHEARSAL_HASH_INDEX_PATH = (
    ROOT / "evidence" / "backref-cost-specialization-alignment-v4-design" /
    "phase1b-rehearsal-hashes.json")
REHEARSAL_ROOT = (ROOT / "evidence" / "backref-cost-specialization-alignment-v4-design" /
                  "rehearsal")
EXPECTED_ENV = "EXPECTED_FREEZE_COMMIT"
AUTHORIZATION_SCHEMA = \
    "libwebp-backref-cost-specialization-alignment-v4-authorization-v1"
REHEARSAL_HASH_INDEX_SCHEMA = \
    "libwebp-backref-cost-specialization-alignment-v4-phase1b-hash-index-v1"
...
def _allowed_phase1b_path(path: str) -> bool:
    prefix = "evidence/backref-cost-specialization-alignment-v4-design/rehearsal/"
    return path.startswith(prefix) or path in {
        "evidence/backref-cost-specialization-alignment-v4-design/phase1b-rehearsal-hashes.json",
        "evidence/backref-cost-specialization-alignment-v4-design/phase1b-authorization.json",
    }
...
def validate_authority_values(*, envelope: dict | None, head_commit: str,
    ...
    if envelope.get("authorization_status") != "AUTHORIZED" or \
            ...
            "ALIGNMENT_V4_CONDITIONAL_DIAGNOSTIC_AUTHORITY" or \
...
def validate_authorized_handoff(head: str) -> str:
    ...
    hashes = {
        "manifest": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_alignment_v4_manifest.json"),
        "protocol": _git_file_sha256(
            source_commit, "doc/backref-cost-specialization-alignment-v4-protocol-20260820.md"),
        "schemas": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_alignment_v4_schemas.json"),
        "transport": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_alignment_v4_transport.py"),
    }
...
def lease_transfer_contract(mode: str, attempted: bool,
    ...
    if not attempted:
        return ({"schema": "libwebp-backref-cost-specialization-alignment-v4-lease-transfer-v1",
...
def _transport_command(*, transaction_nonce: str,
    ...
    remote = (
        "exec /usr/bin/env "
        f"WEBP_ALIGNMENT_V4_TRANSACTION_NONCE={transaction_nonce} "
        f"WEBP_ALIGNMENT_V4_REQUEST_DECLARATION_SHA256={request_declaration_sha256} "
        f"WEBP_ALIGNMENT_V4_BOOTSTRAP_SHA256={bootstrap_sha256} "
...
def remote_pipeline(output: Path, mode: str) -> dict:
    ...
    def persist_transport_refusal(*, failure: str, timeout_phase: str | None,
        ...
        write_json(protocol / "request-progress.json", {
            "schema": "libwebp-backref-cost-specialization-alignment-v4-request-progress-v1",
        ...
        write_json(output / "transfer-cleanup.json", {
            "schema": "libwebp-backref-cost-specialization-alignment-v4-transfer-cleanup-v1",
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v4-return-") as raw:
        temporary = Path(raw)
        try:
