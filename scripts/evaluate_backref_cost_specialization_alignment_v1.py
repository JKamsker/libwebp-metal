#!/usr/bin/env python3
"""Frozen independent evaluator for specialization alignment v1 evidence."""
...
def evaluate(evidence: Path) -> dict:
    ...
    for profile in plan:
        ...
        for retained_ordinal, (stage, output) in enumerate(zip(warm, kept_outputs)):
            if output.get("schema") != \
                    "libwebp-backref-cost-specialization-alignment-v1-output-v1" or \
    ...
    if not null_passes:
        for cell in classifications:
            for metric in classifications[cell]:
    ...
    return {
        "schema": "libwebp-backref-cost-specialization-alignment-v1-evaluation-v1",
