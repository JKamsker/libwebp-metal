#!/usr/bin/env python3
"""Interval-search-v1 admission-only rehearsal and frozen pipeline executor."""
...
EXPECTED_ENV = "WEBP_EXPECTED_BACKREF_COST_INTERVAL_SEARCH_V1_COMMIT"
...
def admission_only(output: Path) -> dict:
    ...
    try:
        ...
        raise
    receipt = {
        "schema": "libwebp-backref-cost-interval-search-v1-admission-rehearsal-v1",
...
def timed_run(output: Path) -> dict:
    ...
    if not MANIFEST_PATH.is_file():
        raise RuntimeError("frozen interval-search v1 manifest is absent")
    if not output.is_absolute() or output.exists():
        raise RuntimeError("output must be a new absolute local path")
    ...
    temporary_holder = tempfile.TemporaryDirectory(prefix="interval-search-v1-return-")
    temporary = Path(temporary_holder.name)
    try:
        remote_q = shlex.quote(record["run_directory"])
        operator = "scripts/run_backref_cost_interval_search_v1_experiment.py"
