#!/usr/bin/env python3
"""Shared fail-closed source admission for attribution-v6 rehearsal and execution."""
...
HOST = "pandocs-agent"
PATH_GUARD = ROOT / "scripts" / "backref_cost_attribution_v6_paths.py"
IDENTITY_PATHS = (
    "CMakeLists.txt",
    "makefile.unix",
    ...
    "src/enc/vp8l_enc.c",
    "src/enc/backref_cost_attribution_v6_experiment_enc.c",
    "src/enc/backref_cost_attribution_v6_experiment_enc.h",
    "tools/backref_cost_attribution_v6_experiment_runner.c",
...
PROTOCOL_MANIFEST = ROOT / "scripts" / "backref_cost_attribution_v6_manifest.json"
SOURCE_RECEIPT_SCHEMA = \
    "libwebp-backref-cost-attribution-v6-source-admission-receipt-v1"
...
def local_identity(source_commit: str | None = None) -> tuple[str, str, dict[str, str]]:
    """Hash the exact committed 1A source, even when authority HEAD is 1B."""
    ...
    for relative in dict.fromkeys(identity_paths):
        ...
        hashes[relative] = hashlib.sha256(
            git_bytes("show", f"{commit}:{relative}")).hexdigest()
...
def admit_source(*, exercise_malformed: bool,
                 source_commit: str | None = None) -> dict:
    commit, tree, source_hashes = local_identity(source_commit)
    temporary = tempfile.TemporaryDirectory(prefix="attribution-v6-admission-")
    ...
    # HEAD is 1A during rehearsal and 1B during execution. The exact 1A source
    # commit is reachable in both bundles and is the only commit checked out.
