#!/usr/bin/env python3
"""Untimed attribution-v5 path, admission, receipt, and mode tests."""
...
from backref_cost_attribution_v5_paths import (
...
def path_tests() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v5-path-home-") as raw:
...
def repository_context_admission_tests() -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v5-admit-test-") as raw:
        temporary = Path(os.path.realpath(raw))
        repository = temporary / "repository"
        repository.mkdir()
        subprocess.run(["git", "init", "-q", str(repository)], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.name",
                        "attribution v5 admission test"], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.email",
                        "attribution-v5-admission@example.invalid"], check=True)
...
def shared_path_test() -> None:
    ...
    with mock.patch.object(executor.admission, "admit_source",
        ...
        assert executor.prepare_remote("rehearse") is sentinel
...
def main() -> int:
    ...
    print("PASS: attribution v5 canonical paths, repository-context bundle verification, "
