#!/usr/bin/env python3
"""Untimed static validation of the frozen interval-search-v2 protocol."""
...
import backref_cost_interval_search_v2_archive as archive_protocol


ROOT = Path(__file__).resolve().parents[1]
MANIFEST = ROOT / "scripts" / "backref_cost_interval_search_v2_manifest.json"
CATALOG = ROOT / "scripts" / "backref_cost_interval_search_v2_schemas.json"
EXPECTED = "WEBP_EXPECTED_BACKREF_COST_INTERVAL_SEARCH_V2_COMMIT"
...
def main() -> int:
    ...
    assert manifest["schema"] == \
        "libwebp-backref-cost-interval-search-v2-frozen-protocol-v1"
    assert manifest["protocol_version"] == 1
    assert catalog["protocol_version"] == 1
    assert manifest["base_commit"] == "22456df935cb3e9e834c0d06a3240db708ace2d7"
    assert "required environment" in manifest["design_commit"]
    assert manifest["independence"]["performance_inputs"] == []
    assert manifest["phase_1_decision"]["status"] == "REJECT_TIMING"
    assert not manifest["phase_1_decision"]["timed_execution_authorized"]
    assert manifest["remote_host"]["ssh_alias"] == "pandocs-agent"
    assert manifest["remote_host"]["safe_root_basename"] == \
        ".libwebp-metal-backref-cost-interval-search-v2"
    ...
    operator = (ROOT / "scripts" /
                "run_backref_cost_interval_search_v2_experiment.py").read_text()
    driver = (ROOT / "scripts" /
              "execute_backref_cost_interval_search_v2.py").read_text()
    path_guard = (ROOT / "scripts" /
                  "backref_cost_interval_search_v2_paths.py").read_text()
    admission = (ROOT / "scripts" /
                 "backref_cost_interval_search_v2_admission.py").read_text()
    archive = (ROOT / "scripts" /
               "backref_cost_interval_search_v2_archive.py").read_text()
    ...
    assert "WEBP_BACKREF_COST_INTERVAL_SEARCH_V2_RECORDER=1" in operator
    ...
    assert "phase-1 handoff rejected timing" in driver
    ...
    design = ROOT / "evidence" / "backref-cost-interval-search-v2-design"
    live = json.loads((design / "live-validation.json").read_text())
    required_live = catalog["schemas"][
        "libwebp-backref-cost-interval-search-v2-live-return-validation-v1"][
            "required"]
    ...
    assert not remote["cleanup_receipt"]["exact_run_exists"]
    assert remote["failure"]["error_type"] == "KeyError"
    assert remote["failure"]["field"] == "bundle_sha256"
    assert not remote["failure"]["retry_attempted"]
    ...
    assert early["status"] == "NOT_RUN"
    assert early["prior_failure"] == "remote-rehearsal.json"
    stalled = json.loads(
        (design / "stalled-malformed-transfer-cleanup.json").read_text())
    assert stalled["status"] == "NOT_RUN"
    assert stalled["prior_failure"] == "remote-rehearsal.json"

    with tempfile.TemporaryDirectory(prefix="interval-search-v2-refusal-") as raw:
        output = Path(raw) / "must-not-exist"
        refused = execute([
            sys.executable,
            "scripts/run_backref_cost_interval_search_v2_experiment.py",
            "run", str(output)])
        ...
        remote_refused = execute([
            sys.executable,
            "scripts/execute_backref_cost_interval_search_v2.py",
            "run", str(output)])
    ...
    print("PASS: interval-search v2 frozen hashes and matrix; local/archive/admission "
          "gates pass; remote untimed workload evidence is intact; terminal "
          "return-wrapper blocker is fail-closed and timing is unauthorized")
    return 0
