#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization aligned null stage attribution v1."""
...
import backref_cost_aligned_null_stage_attribution_v1_admission as admission
import backref_cost_aligned_null_stage_attribution_v1_transport as transport
import execute_backref_cost_aligned_null_stage_attribution_v1 as executor
import run_backref_cost_aligned_null_stage_attribution_v1_experiment as operator
import backref_cost_aligned_null_stage_attribution_v1_toolchain as toolchain
import evaluate_backref_cost_aligned_null_stage_attribution_v1 as evaluator


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_aligned_null_stage_attribution_v1_manifest.json"
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostAlignedNullStageAttributionV1GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostAlignedNullStageAttributionV1GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v1_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v1-archive-inventory-v1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v1-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v1-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v1-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v1-design/phase1b-authorization.json")]
    kwargs = {
...
def zero_sample_refusal_contract() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(
                prefix="stage-attribution-v1-refusal-") as raw:
...
def v4_audit_and_null_contract(manifest: dict) -> None:
    audit = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v1-design/phase1-prior-evidence-audit.json"
    ).read_text())
    assert audit["status"] == "PASS_ALIGNMENT_V4_PREMISE_FROZEN"
    assert audit["performance_samples_generated"] == 0
    assert audit["v4_timing_rows_loaded_into_v1_statistics"] == 0
    ...
    assert reconstruction["graphic_m4_dp_total_null_passed"] is True
    assert reconstruction["graphic_m4_dp_steady_null_passed"] is True
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "5de7b3848adfdb5d1058a061979535f05f976e5b"
    assert manifest["candidate"]["build_flag"] == \
        "WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V1_EXPERIMENT"
    ...
    assert "0x4e554c31" in manifest["protocol"]["statistics"]
    ...
    evaluator_source = (ROOT / "scripts" /
        "evaluate_backref_cost_aligned_null_stage_attribution_v1.py").read_text()
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_aligned_null_stage_attribution_v1.py run '
                '"$PWD/evidence/backref-cost-aligned-null-stage-attribution-v1-execution-20260820"')
    ...
    print("PASS: specialization aligned null stage attribution v1 frozen semantics, strict single-session frames, "
