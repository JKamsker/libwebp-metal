#!/usr/bin/env python3
"""Frozen recorder-free operator for the cache-size single-pass slab A/B."""
...
def load_manifest() -> dict:
    ...
    checks = (
        ...
        (manifest["candidate"]["protocol_document"],
         manifest["candidate"]["protocol_document_sha256"]),
        (manifest["candidate"]["design_report"],
         manifest["candidate"]["design_report_sha256"]),
        (manifest["schemas"]["catalog"],
         manifest["schemas"]["catalog_sha256"]),
...
def configure_and_build(manifest: dict, temporary: Path, log) -> dict[str, Path]:
    ...
    for variant in ("baseline", "candidate"):
        ...
        runner = build / "cache_size_single_pass_slab_runner"
...
def encode(binary: Path, source: Path, output: Path, method: int,
    ...
    run([str(binary), "-quiet", "-lossless", "-exact", "-q", "75", "-m",
...
def correctness_gate(manifest: dict, builds: dict[str, Path], corpus: Path,
    ...
    rows: list[dict] = []
    with tempfile.TemporaryDirectory(prefix="slab-correctness-") as raw:
        check = Path(raw)
        for case_id in manifest["corpus"]["correctness_cases"]:
            ...
            for method in manifest["protocol"]["methods"]:
                ...
                for name, path in paths.items():
                    ppm = check / f"{case_id}-m{method}-{name}.ppm"
                ...
                for lifecycle in ("cold", "warm"):
                    ...
                    for pair in range(life["process_pairs"]):
                        ...
                        for order, variant in enumerate(variants):
                            runner = builds[variant] / "cache_size_single_pass_slab_runner"
...
def execute(manifest: dict, output: Path) -> None:
    ...
    with lease_path.open("a+") as lease:
        ...
        with tempfile.TemporaryDirectory(prefix="cache-size-slab-build-") as raw:
            temporary = Path(raw)
    ...
    (output / "operator-report.md").write_text(
        "# Cache-size single-pass slab operator report\n\n"
