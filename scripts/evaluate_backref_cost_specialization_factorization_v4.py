#!/usr/bin/env python3
"""Frozen independent evaluator for specialization factorization v4 evidence."""
...
def evaluate(evidence: Path) -> dict:
    ...
    for profile in plan:
        ...
        for retained_ordinal, (stage, output) in enumerate(zip(warm, kept_outputs)):
            if output.get("schema") != \
                    "libwebp-backref-cost-specialization-factorization-v4-output-v1" or \
    ...
    return {
        "schema": "libwebp-backref-cost-specialization-factorization-v4-evaluation-v1",
