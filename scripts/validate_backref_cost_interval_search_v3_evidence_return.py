...
import backref_cost_interval_search_v3_archive as protocol
import execute_backref_cost_interval_search_v3 as driver
...
(evidence/'lease-record.json').write_text('{"schema":"libwebp-backref-cost-interval-search-v3-lease-v1","path":"validation-only","acquired":false,"released":false,"acquisition_attempted":false}\n')
...
(evidence/'artifact-hashes.json').write_text(json.dumps({"schema":"libwebp-backref-cost-interval-search-v3-artifact-hashes-v1","artifacts":rows},indent=2,sort_keys=True)+'\n')
...
for label in ('symlink','hardlink','fifo'):
  ...
  (bad/'artifact-hashes.json').write_text(json.dumps({"schema":"libwebp-backref-cost-interval-search-v3-artifact-hashes-v1","artifacts":rows},indent=2,sort_keys=True)+'\n')
'''
...
def main() -> int:
    ...
    cleanup = None
    with tempfile.TemporaryDirectory(prefix="interval-search-v3-live-return-") as raw:
        temporary = Path(raw)
        transfers = {}
        builder_rejections = {}
        ...
        report = {
            "schema": "libwebp-backref-cost-interval-search-v3-live-return-validation-v1",
