#!/usr/bin/env python3
"""Framed single-session transport for backref-cost specialization alignment v2.
...
removes exactly one validated alignment-v2 safe-root child.  It never discovers a PID for
...
import backref_cost_specialization_alignment_v2_payload as payload_generator


MAGIC = b"LIBWEBP-BACKREF-COST-SPECIALIZATION-ALIGNMENT-V1-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 9
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-specialization-alignment-v2"
RUN_PREFIX = "run."
...
BULK_DECLARATION_SCHEMA = \
    "libwebp-backref-cost-specialization-alignment-v2-archive-v1"
SOURCE_RECEIPT_SCHEMA = \
    "libwebp-backref-cost-specialization-alignment-v2-source-admission-receipt-v1"
SOURCE_DETAILS_SCHEMA = \
    "libwebp-backref-cost-specialization-alignment-v2-source-admission-details-v1"
...
class ProcessRefusal(FrameRefusal):
    ...
    def __init__(self, failure_code: str, *, row: dict | None = None,
        ...
        self.diagnostic = {
            "schema":
                "libwebp-backref-cost-specialization-alignment-v2-process-refusal-v1",
    ...
    def _valid_bulk_declaration(self, declaration: dict) -> bool:
        ...
        valid = (
            ...
            inventory.get("schema") ==
                "libwebp-backref-cost-specialization-alignment-v2-archive-inventory-v1" and
        ...
        if process_refusal is not None:
            ...
            if not isinstance(process_refusal, dict) or \
                    ...
                    process_refusal.get("schema") != (
                        "libwebp-backref-cost-specialization-alignment-v2-process-refusal-v1") or \
...
def _safe_root(home: Path) -> Path:
    ...
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
        ...
        raise FrameRefusal("alignment-v2 safe root is not a canonical real HOME child")
    return root
...
def _validate_child(home: Path, root: Path, child: Path) -> None:
    if root != home / SAFE_ROOT_BASENAME or child.parent != root or \
        ...
        raise FrameRefusal("run directory is not the exact alignment-v2 safe-root child")
    info = os.lstat(child)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
...
def remote_main() -> int:
    ...
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-specialization-alignment-v2-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-specialization-alignment-v2-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_specialization_alignment_v2_experiment.py", mode,
        ...
        _add_indexed_detail(
            evidence, "remote-admission-process-isolation.json", {
                "schema":
                    "libwebp-backref-cost-specialization-alignment-v2-remote-process-isolation-v1",
                "status": "verified",
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-specialization-alignment-v2-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_specialization_alignment_v2_archive.py", "build",
    ...
    integrity = {
        "schema": "libwebp-backref-cost-specialization-alignment-v2-integrity-footer-v1",
    ...
    if child is not None:
        try:
            ...
            if not absent:
                raise FrameRefusal("exact alignment-v2 child remains after cleanup")
            cleanup = {
                "schema": "libwebp-backref-cost-specialization-alignment-v2-cleanup-receipt-v1",
