#!/usr/bin/env python3
"""Deterministic Aligned Null Stage Attribution V4 READY/request-ACK/full-duplex fixtures."""
...
BOOTSTRAP = ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_bootstrap.py"
NONCE = "a" * 64
...
def environment(channel_request: transport.ChannelRequest) -> dict[str, str]:
    ...
    result.update({
        "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_TRANSACTION_NONCE": NONCE,
        "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_REQUEST_DECLARATION_SHA256":
            channel_request.declaration_sha256,
        "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_BOOTSTRAP_SHA256": BOOTSTRAP_SHA,
    })
    return result
...
def pump(command: list[str], channel_request: transport.ChannelRequest, *,
    ...
    temporary = tempfile.TemporaryDirectory(prefix="stage-attribution-v4-channel-")
...
def timeout_fixture(command: list[str], channel_request: transport.ChannelRequest,
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-timeout-") as raw:
        root = Path(raw)
        try:
...
import backref_cost_aligned_null_stage_attribution_v4_transport as t
...
def chunk_stall_matrix() -> None:
    ...
    for acknowledged in points:
        with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-stall-") as raw:
            root = Path(raw)
            watched = observer(channel_request)
            try:
                admission.pump_owned_request_channel(
...
def early_exit_absolute_timeout_and_cleanup_footer() -> None:
    ...
    ready = transport.CHANNEL_MAGIC + transport.encode_frame(
        "ready", transport.canonical_json(ready_value(channel_request)),
        sequence=0)
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-early-exit-") as raw:
        root = Path(raw)
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-absolute-") as raw:
        root = Path(raw)
        try:
            admission.pump_owned_request_channel(
...
def source_refusal_is_not_success() -> None:
    ...
    source = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_transport.py").read_bytes()
    payload = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v4_payload.py").read_bytes()
    ...
    channel_request = transport.build_channel_request(
        ...
        bootstrap_sha256=BOOTSTRAP_SHA)
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-remote-home-") as remote_home:
