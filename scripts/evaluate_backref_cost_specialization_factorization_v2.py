#!/usr/bin/env python3
"""Frozen independent evaluator for specialization factorization v2 evidence."""
...
def evaluate(evidence: Path) -> dict:
    ...
    for profile in plan:
        ...
        for retained_ordinal, (stage, output) in enumerate(zip(warm, kept_outputs)):
            if output.get("schema") != \
                    "libwebp-backref-cost-specialization-factorization-v2-output-v1" or \
    ...
    return {
        "schema": "libwebp-backref-cost-specialization-factorization-v2-evaluation-v1",
