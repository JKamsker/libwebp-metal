#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v9."""
...
import run_backref_cost_attribution_v9_experiment as operator
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV9ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV9RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V9_EXPERIMENT=1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v9-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-attribution-v9-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v9-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v9-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "2940104e909e41f973aea7f4cc525792ac088ccb"
    ...
    print("PASS: attribution v9 frozen semantics, strict single-session frames, "
