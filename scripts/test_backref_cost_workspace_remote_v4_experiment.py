#!/usr/bin/env python3
"""Untimed v4 candidate identity, correctness, fallback, and omission checks."""
...
BASELINE = "c66e0264cd7af4df7678a5b9af60c6fecbd8329d"
BUILD = "WEBP_BUILD_BACKREF_COST_WORKSPACE_REMOTE_V4_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_WORKSPACE_REMOTE_V4_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_WORKSPACE_REMOTE_V4_EXPERIMENT"
FAULT = "WEBP_BACKREF_COST_WORKSPACE_REMOTE_V4_EXPERIMENT_INJECT_FAILURE"
V3_MACRO = "WEBP_USE_BACKREF_COST_WORKSPACE_REMOTE_V3_EXPERIMENT"
V3_RUNTIME = "WEBP_BACKREF_COST_WORKSPACE_REMOTE_V3_EXPERIMENT"
RUNNER = "backref_cost_workspace_remote_v4_experiment_runner"
SOURCE = "backref_cost_workspace_remote_v4_experiment_enc.c"
MATRIX = (
...
def run(argv: list[str], *, env: dict[str, str] | None = None,
    ...
    for name in (RUNTIME, FAULT, V3_RUNTIME, "WEBP_BENCHMARK_SESSION",
...
def equivalence() -> dict[str, str]:
    ...
    actual = actual.replace("VP8LBackrefCostWorkspaceRemoteV4Experiment",
    ...
    if actual != expected:
        raise AssertionError("v4 boundary differs from the frozen v3 candidate")
    v3 = (ROOT / "src/enc/backref_cost_workspace_remote_v3_experiment_enc.c").read_text()
    v4 = (ROOT / "src/enc/backref_cost_workspace_remote_v4_experiment_enc.c").read_text()
    v3 = v3[v3.index("static int EnvironmentIsOne"):]
    v4 = v4[v4.index("static int EnvironmentIsOne"):]
    v4 = v4.replace("RemoteV4", "RemoteV3").replace("REMOTE_V4", "REMOTE_V3")
    if v4 != v3:
        raise AssertionError("v4 helper behavior differs from frozen v3")
    return {
...
def main() -> int:
    ...
    runner_source = (ROOT / "tools" /
                     "backref_cost_workspace_remote_v4_experiment_runner.c").read_text()
    assert runner_source.index("session == NULL") < runner_source.index(
        "ReadPPM(input_path")
    assert "WEBP_BENCHMARK_SESSION=exclusive" in runner_source
    with tempfile.TemporaryDirectory(prefix="remote-v4-candidate-test-") as raw:
        temporary = Path(raw)
        ...
        assert "BackrefCostWorkspaceRemoteV4" not in symbols
        for forbidden in (RUNTIME, FAULT, RUNNER, SOURCE, "remote v4"):
