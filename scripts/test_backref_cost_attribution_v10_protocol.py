#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v10."""
...
import run_backref_cost_attribution_v10_experiment as operator
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV10ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV10RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V10_EXPERIMENT=1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v10-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-attribution-v10-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v10-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v10-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "a027f4a4b6a61d2b4a5d9e897e6b7ca51eb6c6ca"
    ...
    print("PASS: attribution v10 frozen semantics, strict single-session frames, "
