#!/usr/bin/env python3
"""Frozen local driver for one attribution-v10 single-session transaction."""
...
import backref_cost_attribution_v10_archive as archive_protocol
import backref_cost_attribution_v10_transport as transport


ROOT = Path(__file__).resolve().parents[1]
HOST = admission.HOST
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_attribution_v10_manifest.json"
PROTOCOL_PATH = ROOT / "doc" / "backref-cost-attribution-v10-protocol-20260819.md"
TRANSPORT_PATH = ROOT / "scripts" / "backref_cost_attribution_v10_transport.py"
AUTHORIZATION_PATH = (ROOT / "evidence" / "backref-cost-attribution-v10-design" /
                      "phase1b-authorization.json")
REHEARSAL_HASH_INDEX_PATH = (
    ROOT / "evidence" / "backref-cost-attribution-v10-design" /
    "phase1b-rehearsal-hashes.json")
REHEARSAL_ROOT = (ROOT / "evidence" / "backref-cost-attribution-v10-design" /
                  "rehearsal")
EXPECTED_ENV = "EXPECTED_FREEZE_COMMIT"
AUTHORIZATION_SCHEMA = \
    "libwebp-backref-cost-attribution-v10-authorization-v1"
REHEARSAL_HASH_INDEX_SCHEMA = \
    "libwebp-backref-cost-attribution-v10-phase1b-hash-index-v1"
...
def _allowed_phase1b_path(path: str) -> bool:
    prefix = "evidence/backref-cost-attribution-v10-design/rehearsal/"
    return path.startswith(prefix) or path in {
        "evidence/backref-cost-attribution-v10-design/phase1b-rehearsal-hashes.json",
...
def validate_authorized_handoff(head: str) -> str:
    ...
    hashes = {
        "manifest": _git_file_sha256(
            source_commit, "scripts/backref_cost_attribution_v10_manifest.json"),
        "protocol": _git_file_sha256(
            source_commit, "doc/backref-cost-attribution-v10-protocol-20260819.md"),
        "schemas": _git_file_sha256(
            source_commit, "scripts/backref_cost_attribution_v10_schemas.json"),
        "transport": _git_file_sha256(
            source_commit, "scripts/backref_cost_attribution_v10_transport.py"),
    }
...
def lease_transfer_contract(mode: str, attempted: bool,
    ...
    if not attempted:
        return ({"schema": "libwebp-backref-cost-attribution-v10-lease-transfer-v1",
...
def remote_pipeline(output: Path, mode: str) -> dict:
    ...
    try:
        ...
        write_json(output / "transfer-cleanup.json", {
            "schema": "libwebp-backref-cost-attribution-v10-transfer-cleanup-v1",
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v10-return-") as raw:
        temporary = Path(raw)
