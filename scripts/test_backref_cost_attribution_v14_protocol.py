#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v14."""
...
import run_backref_cost_attribution_v14_experiment as operator
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV14ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV14RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V14_EXPERIMENT=1",
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_attribution_v14_admission.py").read_text()
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v14-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-attribution-v14-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v14-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v14-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "e690603a84aa90448e7c6d44ebfc303920b806b3"
    ...
    print("PASS: attribution v14 frozen semantics, strict single-session frames, "
