#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost attribution v7."""
...
from backref_cost_attribution_v7_admission import (
...
def clean_environment(manifest: dict, variant: str,
    ...
    if stage_output is not None:
        env.update({
            ...
            "WEBP_BACKREF_COST_ATTRIBUTION_V7_TIMERS": "1",
            "WEBP_BACKREF_COST_ATTRIBUTION_V7_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_ATTRIBUTION_V7_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_ATTRIBUTION_V7_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_ATTRIBUTION_V7_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def process_guard(output: Path, label: str) -> dict:
    ...
    conflicts = [line for line in conflicts
                 if "run_backref_cost_attribution_v7_experiment.py" not in line
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
            ...
            "WEBP_BACKREF_COST_ATTRIBUTION_V7_DETERMINISTIC_BUILD=1",
            f"CFLAGS={flags}",
    ...
    (output / "build.log").write_text(result.stdout + result.stderr,
                                       encoding="utf-8")
    runner = build_root / "tools/backref_cost_attribution_v7_experiment_runner"
...
def timer_overhead_gate(manifest: dict, runner: Path, output: Path) -> dict:
    ...
    if record.get("schema") != \
            "libwebp-backref-cost-attribution-v7-timer-check-v1" or \
    ...
    for case in manifest["corpus"]:
        ...
        for method in manifest["protocol"]["methods"]:
            ...
            if any(row.get("schema") !=
                   "libwebp-backref-cost-attribution-v7-counter-check-v1"
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path) -> list[dict]:
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-attribution-v7-output-v1"
               for item in outputs):
...
def admit_representative_payload(path: Path, expected_minimum: int) -> dict:
    ...
    source_raw = os.environ.get(
        "WEBP_BACKREF_COST_ATTRIBUTION_V7_REHEARSAL_PAYLOAD")
    expected_sha = os.environ.get(
        "WEBP_BACKREF_COST_ATTRIBUTION_V7_REHEARSAL_PAYLOAD_SHA256")
    expected_size_raw = os.environ.get(
        "WEBP_BACKREF_COST_ATTRIBUTION_V7_REHEARSAL_PAYLOAD_BYTES")
...
def operator(mode: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="backref-attribution-v7-") as raw:
        temporary = Path(raw)
