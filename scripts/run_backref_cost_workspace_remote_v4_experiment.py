#!/usr/bin/env python3
"""Frozen remote-v4 host operator for the backref-cost workspace remote v4 A/B."""
...
def execute(manifest: dict, output: Path) -> None:
        ...
        with lease_path.open("a+") as lease:
            ...
            try:
                with tempfile.TemporaryDirectory(
                        prefix="backref-cost-workspace-remote-v4-build-") as raw:
                    temporary = Path(raw)
    ...
    (output / "operator-report.md").write_text(
        "# Remote backref-cost workspace remote v4 A/B operator report\n\n"
        ...
        "Only this v4 matrix is eligible. Timing was remote, serial, "
