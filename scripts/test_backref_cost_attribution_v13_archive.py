#!/usr/bin/env python3
"""Untimed tests for the frozen interval-attribution v13 indexed evidence archive protocol."""
...
import backref_cost_attribution_v13_archive as protocol
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="attribution-v13-archive-test-") as raw:
        ...
        payload = representative / "representative-input.bin"
        with payload.open("xb") as output:
            for chunk in transport.deterministic_payload_chunks(
                    16 * 1024 * 1024):
                output.write(chunk)
        assert protocol.sha256(payload) == \
        ...
        assert archive_hashes[0] == \
            "9d7ee691c01af8e968ff00d0cb98c51cedd76c8bce205e09ff3cc1cd00927ad8"
        assert archive_sizes[0] == 16782794
        assert 16 * 1024 * 1024 < archive_sizes[0] < 20 * 1024 * 1024
        assert transport.MAX_ARCHIVE_BYTES / archive_sizes[0] > 1.9
