#!/usr/bin/env python3
"""Untimed attribution-v15 path, admission, frame, and mode tests."""
...
from backref_cost_attribution_v15_paths import (
...
def path_tests() -> None:
    ...
    assert SAFE_ROOT_BASENAME.endswith("attribution-v15")
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v15-path-home-") as raw:
...
def repository_context_admission() -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v15-admit-") as raw:
        root = Path(raw)
        ...
        subprocess.run(["git", "-C", str(repository), "config", "user.name",
                        "v15 admission test"], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.email",
                        "v15@example.invalid"], check=True)
...
def remote_refusal_cleanup_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v15-remote-home-") as raw:
...
def main() -> int:
    ...
    mode_truth_table()
    print("PASS: attribution v15 canonical-only paths, repository-context source "
