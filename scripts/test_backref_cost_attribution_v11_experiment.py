#!/usr/bin/env python3
"""Untimed V11 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "631ac52803ff7d74f8461917503d54b46a8fc5e4"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V11_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V11_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V11_EXPERIMENT"
RUNNER = "backref_cost_attribution_v11_experiment_runner"
SOURCE = "backref_cost_attribution_v11_experiment_enc.c"
...
def main() -> int:
    ...
    assert "V11 preserves V10's frozen control flow" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v11-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V11_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V11_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V11_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V11_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
