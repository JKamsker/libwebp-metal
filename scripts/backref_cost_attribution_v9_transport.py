#!/usr/bin/env python3
"""Framed single-session transport for backref-cost attribution v9.
...
removes exactly one validated v9 safe-root child.  It never discovers a PID for
...
MAGIC = b"LIBWEBP-BACKREF-COST-ATTRIBUTION-V9-FRAMES\x00\x01\n"
PROTOCOL_VERSION = 1
SAFE_ROOT_BASENAME = ".libwebp-metal-backref-cost-attribution-v9"
RUN_PREFIX = "run."
...
# Kernel ``comm`` is an exact opaque executable-name token, not a path.  This
# frozen set intentionally contains exact tokens: neither argv nor a
# substring/prefix/suffix match can create a conflict.
DENIED_EXECUTABLE_COMM_TOKENS = frozenset({
...
# Every member of this transaction's self-to-PID-1 chain must have one of these
# exact opaque comm tokens or a separately supplied exact verified path below.
# A path is not inferred from comm, and unrelated non-denied comm tokens never
# require path resolution.
EXPECTED_TRANSPORT_ANCESTOR_COMM_TOKENS = frozenset({
...
def deterministic_payload(size: int) -> bytes:
    domain = b"libwebp-backref-cost-attribution-v9-rehearsal\x00"
    output = bytearray()
    counter = 0
...
def _safe_root(home: Path) -> Path:
    ...
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
        ...
        raise FrameRefusal("v9 safe root is not a canonical real HOME child")
    return root
...
def _validate_child(home: Path, root: Path, child: Path) -> None:
    if root != home / SAFE_ROOT_BASENAME or child.parent != root or \
        ...
        raise FrameRefusal("run directory is not the exact v9 safe-root child")
    info = os.lstat(child)
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode) or \
...
def parse_process_snapshot(snapshot: bytes) -> dict[int, dict]:
    """Parse exact PID/PPID/opaque-comm rows and fail closed on ambiguity."""
    try:
    ...
    for line in text.splitlines():
        ...
        if "/" in comm or comm in (".", ".."):
            raise FrameRefusal("comm is not an opaque executable-name token")
        rows[pid] = {"pid": pid, "ppid": ppid, "comm": comm,
                     "comm_identity_kind": "opaque-executable-name-token"}
    return rows


def _verified_resolved_paths(rows: dict[int, dict], records: dict | None) -> dict:
    """Validate optional separately acquired read-only executable paths."""
    if records is None:
        return {}
    if not isinstance(records, dict):
        raise FrameRefusal("resolved executable path evidence is malformed")
    verified = {}
    for pid, record in records.items():
        if not isinstance(pid, int) or pid not in rows or \
                not isinstance(record, dict):
            raise FrameRefusal("resolved executable path evidence has unknown PID")
        path = record.get("path")
        provenance = record.get("provenance")
        if not isinstance(path, str) or not os.path.isabs(path) or path == "/" or \
                os.path.normpath(path) != path or "//" in path or \
                os.path.realpath(path) != path or \
                not isinstance(provenance, str) or not provenance.strip() or \
                record.get("read_only") is not True:
            raise FrameRefusal(
                "resolved executable path is not absolute/canonical/provenanced")
        verified[pid] = {
            "path": path, "provenance": provenance, "read_only": True,
            "identity_kind": "separately-resolved-absolute-executable-path"}
    return verified


def classify_process_snapshot(snapshot: bytes, self_pid: int,
                              resolved_executable_paths: dict | None = None) -> dict:
    """Classify exact comm/path identities from one frozen snapshot."""
    rows = parse_process_snapshot(snapshot)
    resolved = _verified_resolved_paths(rows, resolved_executable_paths)
    ...
    while True:
        ...
        path = resolved.get(current, {}).get("path")
        if row["comm"] not in EXPECTED_TRANSPORT_ANCESTOR_COMM_TOKENS and \
                path not in EXPECTED_TRANSPORT_ANCESTOR_PATHS:
    ...
    conflicts = [row for row in rows.values()
                 if row["comm"] in DENIED_EXECUTABLE_COMM_TOKENS]
    conflicts.extend(
        {**rows[pid], "resolved_executable_path": record}
        for pid, record in resolved.items()
        if Path(record["path"]).name in DENIED_EXECUTABLE_COMM_TOKENS and
        rows[pid] not in conflicts)
    if conflicts:
        raise FrameRefusal(
    ...
    return {
        ...
        "comm_semantics": "exact opaque executable-name token; never a path",
        "argument_text_collected": False,
        "resolved_executable_paths": [
            {"pid": pid, **resolved[pid]} for pid in sorted(resolved)],
        "resolved_path_absence_policy":
            "allowed for non-denied non-ancestor exact comm tokens",
...
def _admit_source(child: Path, bundle: Path, metadata: dict) -> dict:
    ...
    return {
        "schema": "libwebp-backref-cost-attribution-v9-source-admission-receipt-v1",
...
def remote_main() -> int:
    ...
    sequence = 0
    success = False
    cleanup = {"schema": "libwebp-backref-cost-attribution-v9-cleanup-receipt-v1",
    ...
    lease_state = {"schema": "libwebp-backref-cost-attribution-v9-lease-transfer-v1",
    ...
    try:
        ...
        if mode == "run":
            ...
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V9_REHEARSAL_PAYLOAD"] = str(payload_path)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V9_REHEARSAL_PAYLOAD_SHA256"] = sha256_bytes(payload)
            environment["WEBP_BACKREF_COST_ATTRIBUTION_V9_REHEARSAL_PAYLOAD_BYTES"] = str(len(payload))
        operator = _run([
            sys.executable,
            "scripts/run_backref_cost_attribution_v9_experiment.py", mode,
        ...
        if attempted is False:
            lease_state = {
                "schema": "libwebp-backref-cost-attribution-v9-lease-transfer-v1",
        ...
        builder = _run([
            sys.executable, "scripts/backref_cost_attribution_v9_archive.py", "build",
    ...
    try:
        ...
        if archive is not None and declaration is not None:
            ...
            for chunk_index, offset in enumerate(range(0, len(archive), CHUNK_BYTES)):
                ...
                output.flush()
                sequence += 1
        integrity = {"schema": "libwebp-backref-cost-attribution-v9-integrity-footer-v1",
        ...
        if child is not None:
            try:
                ...
                if not absent:
                    raise FrameRefusal("exact v9 child remains after cleanup")
                cleanup = {
                    "schema": "libwebp-backref-cost-attribution-v9-cleanup-receipt-v1",
