#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization factorization v3."""
...
import run_backref_cost_specialization_factorization_v3_experiment as operator
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostSpecializationFactorizationV3GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V3_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostSpecializationFactorizationV3GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_specialization_factorization_v3_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-specialization-factorization-v3-archive-inventory-v1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v3-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority": "FACTORIZATION_V3_DIAGNOSTIC_ONLY",
    ...
    diff = [
        ("A", "evidence/backref-cost-specialization-factorization-v3-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-specialization-factorization-v3-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-specialization-factorization-v3-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "96f5d1881619bd5b25404d869cbb2ac2d4b3329c"
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_specialization_factorization_v3.py run '
                '"$PWD/evidence/backref-cost-specialization-factorization-v3-execution-20260819"')
    ...
    print("PASS: specialization factorization v3 frozen semantics, strict single-session frames, "
