#!/usr/bin/env python3
"""Fail-closed stage-attribution-v4 toolchain agreement gate."""
...
def main() -> int:
    ...
    assert hashlib.sha256(canonical).hexdigest() != \
        toolchain.STALE_ALIGNMENT_V2_IDENTITY_CANONICAL_SHA256
    assert frozen["stale_alignment_v4_identity_canonical_sha256"] == \
    ...
    assert receipt["negative_stale_alignment_v2_identity_test"] == \
        "PASS_REJECTED"
    assert receipt["negative_stale_alignment_v4_identity_test"] == \
        "PASS_REJECTED"
    ...
    print(json.dumps({
        ...
        "stale_alignment_v4_identity_negative_test": "PASS_REJECTED",
    }, sort_keys=True))
    return 0
