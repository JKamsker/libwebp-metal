#!/usr/bin/env python3
"""Local identity and direct-child ownership for stage-attribution-v4."""
...
HOST = "pandocs-agent"
PROTOCOL_MANIFEST = ROOT / "scripts" / "backref_cost_aligned_null_stage_attribution_v4_manifest.json"
# The manifest owns the one ordered admission domain.  Loading it here avoids
# a second handwritten path set drifting to a predecessor clone or count.
IDENTITY_PATHS = tuple(json.loads(PROTOCOL_MANIFEST.read_text(
    encoding="utf-8"))["admission_identity_paths"])
...
class ChannelPumpResult:
    ...
    request_progress_chain: list[dict]
...
class TransportDeadlineStateMachine:
    """Pure V4 phase deadlines; no request byte may precede READY."""
...
def pump_owned_request_channel(
    ...
    pending_chunk_ordinal: int | None = None
    ...
    def queue(value: bytes, label: str, *, chunk_ordinal: int | None = None) -> None:
        nonlocal pending, pending_label, pending_chunk_ordinal
        nonlocal pending_write_started
        if pending or stdin_half_closed:
            raise RuntimeError("request channel attempted overlapping writes")
        pending = memoryview(value)
        pending_label = label
        pending_chunk_ordinal = chunk_ordinal
        pending_write_started = False
        observer.client_transport_state.update({
            "pending_kind": label,
            "pending_chunk_ordinal": chunk_ordinal,
            "pending_wire_bytes": len(value),
        })
        try:
    ...
    def adopt_observer(now: float) -> None:
        ...
        if observer.request_chunks_acked == chunks_queued and \
            ...
            queue(transport.encode_frame(
                ...
                chunk_index=index), "chunk", chunk_ordinal=index + 1)
            return
                ...
                for key, _ in events:
                    ...
                    if key.data == "stdin":
                        ...
                        if written:
                            ...
                            if pending_label == "chunk" and \
                                    not pending_write_started:
                                observer.note_chunk_write_start(
                                    pending_chunk_ordinal or 0, now_ns)
                                pending_write_started = True
                            piece = pending[:written]
                            wire_digest.update(piece)
                            wire_count += written
                            pending = pending[written:]
                            observer.client_transport_state["pending_wire_bytes"] = \
                                len(pending)
                            ...
                            if not pending:
                                ...
                                if pending_label == "chunk":
                                    chunks_written += 1
                                    observer.note_chunk_write_complete(
                                        pending_chunk_ordinal or 0,
                                        time.monotonic_ns())
                                    observer.client_transport_state[
    ...
    return ChannelPumpResult(
        ...
        request_progress_chain=channel["request_progress_chain"],
...
CHILD_START_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-child-start-v1"
CHILD_RECEIPT_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-child-receipt-v1"
...
class DirectRunnerLedger:
    ...
    def run(self, argv: list[str], *, cwd: Path, env: dict[str, str],
        ...
        child_env = dict(env)
        child_env.update({
            "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_DIRECT_CHILD": "1",
            "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_ORDINAL": str(ordinal),
            "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_CREATION_TOKEN": creation_identity,
            "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_PURPOSE": purpose,
            "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_LIFECYCLE": lifecycle,
            "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_CASE_ID": case_id,
            "WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CHILD_METHOD": str(method),
        })
    ...
    def finalize(self, expected_count: int) -> dict:
        ...
        return {"schema":
                "libwebp-backref-cost-aligned-null-stage-attribution-v4-child-summary-v1",
...
def create_source_bundle() -> tuple[bytes, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-bundle-") as raw:
