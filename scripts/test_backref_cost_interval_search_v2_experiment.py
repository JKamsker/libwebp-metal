#!/usr/bin/env python3
"""Untimed interval-search v2 correctness, fallback, and omission checks."""
...
BASELINE = "22456df935cb3e9e834c0d06a3240db708ace2d7"
BUILD = "WEBP_BUILD_BACKREF_COST_INTERVAL_SEARCH_V2_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_INTERVAL_SEARCH_V2_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_INTERVAL_SEARCH_V2_EXPERIMENT"
FAULT = "WEBP_BACKREF_COST_INTERVAL_SEARCH_V2_EXPERIMENT_INJECT_FAILURE"
...
RUNNER = "backref_cost_interval_search_v2_experiment_runner"
SOURCE = "backref_cost_interval_search_v2_experiment_enc.c"
MATRIX = (
...
def equivalence() -> dict[str, str]:
    ...
    baseline = preprocess("WEBP_INTERVAL_SEARCH_V2_UNUSED_BASELINE_MACRO")
    actual = preprocess(MACRO)
    assert "use_interval_search_v2" not in baseline
    ...
    assert "use_interval_search_v2;" not in actual
    ...
    helper = (ROOT / "src/enc/backref_cost_interval_search_v2_experiment_enc.c").read_text()
    return {
...
def main() -> int:
    ...
    runner_source = (ROOT / "tools" /
                     "backref_cost_interval_search_v2_experiment_runner.c").read_text()
    assert "session == NULL" in runner_source
    assert "recorder build refuses timed samples" in runner_source
    with tempfile.TemporaryDirectory(prefix="interval-search-v2-candidate-test-") as raw:
        temporary = Path(raw)
        ...
        assert "BackrefCostIntervalSearchV2" not in symbols
        for forbidden in (RUNTIME, FAULT, RUNNER, SOURCE, "interval-search v2"):
