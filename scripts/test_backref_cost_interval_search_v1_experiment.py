#!/usr/bin/env python3
"""Untimed interval-search v1 correctness, fallback, and omission checks."""
...
BASELINE = "568d2476afef3465294bec6a4e099d1b598135f7"
BUILD = "WEBP_BUILD_BACKREF_COST_INTERVAL_SEARCH_V1_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_INTERVAL_SEARCH_V1_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_INTERVAL_SEARCH_V1_EXPERIMENT"
FAULT = "WEBP_BACKREF_COST_INTERVAL_SEARCH_V1_EXPERIMENT_INJECT_FAILURE"
...
RUNNER = "backref_cost_interval_search_v1_experiment_runner"
SOURCE = "backref_cost_interval_search_v1_experiment_enc.c"
MATRIX = (
...
def run(argv: list[str], *, env: dict[str, str] | None = None,
    ...
    for name in (RUNTIME, FAULT, V3_RUNTIME, V4_RUNTIME, V5_RUNTIME,
...
def equivalence() -> dict[str, str]:
    ...
    baseline = preprocess("WEBP_INTERVAL_SEARCH_V1_UNUSED_BASELINE_MACRO")
    actual = preprocess(MACRO)
    assert "use_interval_search_v1" not in baseline
    assert "current->start > manager->tail->start" in current
    assert "current->start < previous->start" in current
    assert "previous->next->start < current->start" in current
    assert current.index("current->start > manager->tail->start") < \
        current.index("previous = manager->tail")
    helper = (ROOT / "src/enc/backref_cost_interval_search_v1_experiment_enc.c").read_text()
    return {
...
def main() -> int:
    ...
    runner_source = (ROOT / "tools" /
                     "backref_cost_interval_search_v1_experiment_runner.c").read_text()
    assert "session == NULL" in runner_source
    assert "recorder build refuses timed samples" in runner_source
    with tempfile.TemporaryDirectory(prefix="interval-search-v1-candidate-test-") as raw:
        temporary = Path(raw)
        ...
        assert "BackrefCostIntervalSearchV1" not in symbols
        for forbidden in (RUNTIME, FAULT, RUNNER, SOURCE, "interval-search v1"):
