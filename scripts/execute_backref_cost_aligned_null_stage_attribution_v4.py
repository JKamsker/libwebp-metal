#!/usr/bin/env python3
"""Frozen local driver for one stage-attribution-v4 single-session transaction."""
...
import backref_cost_aligned_null_stage_attribution_v4_archive as archive_protocol
...
PROTOCOL_PATH = ROOT / "doc" / "backref-cost-aligned-null-stage-attribution-v4-protocol-20260820.md"
TRANSPORT_PATH = ROOT / "scripts" / "backref_cost_aligned_null_stage_attribution_v4_transport.py"
BOOTSTRAP_PATH = ROOT / "scripts" / "backref_cost_aligned_null_stage_attribution_v4_bootstrap.py"
PAYLOAD_GENERATOR_PATH = \
    ROOT / "scripts" / "backref_cost_aligned_null_stage_attribution_v4_payload.py"
AUTHORIZATION_PATH = (ROOT / "evidence" / "backref-cost-aligned-null-stage-attribution-v4-design" /
...
REHEARSAL_ROOT = (ROOT / "evidence" / "backref-cost-aligned-null-stage-attribution-v4-design" /
                  "rehearsal")
EXPECTED_ENV = "EXPECTED_FREEZE_COMMIT"
AUTHORIZATION_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-authorization-v1"
REHEARSAL_HASH_INDEX_SCHEMA = \
    "libwebp-backref-cost-aligned-null-stage-attribution-v4-phase1b-hash-index-v1"
...
def _allowed_phase1b_path(path: str) -> bool:
    prefix = "evidence/backref-cost-aligned-null-stage-attribution-v4-design/rehearsal/"
    return path.startswith(prefix) or path in {
        "evidence/backref-cost-aligned-null-stage-attribution-v4-design/phase1b-rehearsal-hashes.json",
        "evidence/backref-cost-aligned-null-stage-attribution-v4-design/phase1b-authorization.json",
    }
...
def validate_authority_values(*, envelope: dict | None, head_commit: str,
    ...
    if envelope.get("authorization_status") != "AUTHORIZED" or \
            envelope.get("phase_2_authority") != \
            "ALIGNED_NULL_STAGE_ATTRIBUTION_V4_CONDITIONAL_DIAGNOSTIC_AUTHORITY" or \
            envelope.get("pipeline_invocation_authorized") is not True or \
            envelope.get(
                "performance_sampling_authorized_before_dynamic_admission") \
            is not False or \
            envelope.get("timed_execution_authorized") is not False or \
            envelope.get(
                "dynamic_gates_deferred_to_single_phase2_pipeline") is not True:
        raise RuntimeError("Phase-1B conditional pipeline authority is absent")
...
def validate_authorized_handoff(head: str) -> str:
    ...
    hashes = {
        "manifest": _git_file_sha256(
            source_commit, "scripts/backref_cost_aligned_null_stage_attribution_v4_manifest.json"),
        "protocol": _git_file_sha256(
            source_commit, "doc/backref-cost-aligned-null-stage-attribution-v4-protocol-20260820.md"),
        "schemas": _git_file_sha256(
            source_commit, "scripts/backref_cost_aligned_null_stage_attribution_v4_schemas.json"),
        "transport": _git_file_sha256(
            source_commit, "scripts/backref_cost_aligned_null_stage_attribution_v4_transport.py"),
    }
...
def lease_transfer_contract(mode: str, attempted: bool,
    ...
    if not attempted:
        return ({"schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-lease-transfer-v1",
...
def _transport_command(*, transaction_nonce: str,
    ...
    remote = (
        "exec /usr/bin/env "
        f"WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_TRANSACTION_NONCE={transaction_nonce} "
        f"WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_REQUEST_DECLARATION_SHA256={request_declaration_sha256} "
        f"WEBP_ALIGNED_NULL_STAGE_ATTRIBUTION_V4_BOOTSTRAP_SHA256={bootstrap_sha256} "
...
def remote_pipeline(output: Path, mode: str) -> dict:
    ...
    def persist_transport_refusal(*, failure: str, timeout_phase: str | None,
        ...
        write_json(protocol / "request-progress.json", {
            "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-progress-v1",
            ...
            "last_fully_acknowledged_chunk": observer.request_chunks_acked,
        })
        chain = observer.progress_chain_snapshot()
        write_json(protocol / "request-progress-chain.json", {
            "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-request-progress-chain-v1",
            "status": "partial" if len(chain) < request.chunk_count else "complete",
            "transaction_nonce": transaction_nonce,
            "row_cap": 512,
            "rows": chain,
            "rows_recorded": len(chain),
            "chunks_planned": request.chunk_count,
            "last_fully_acknowledged_chunk": len(chain),
            "client_transport_state": observer.client_transport_state,
            "failure_classification": failure_classification,
        })
        ...
        write_json(output / "transfer-cleanup.json", {
            "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v4-transfer-cleanup-v1",
            "status": "refused", "mode": mode,
            "source_commit": commit, "source_tree": tree,
            "failure": failure, "timeout_phase": timeout_phase,
            "timeout_deadline_kind": timeout_kind,
            "transport_failure_classification": failure_classification,
    ...
    try:
        ...
        state = observer.client_transport_state
        if error.deadline_kind == admission.ABSOLUTE_TRANSACTION:
            classification = "absolute-timeout"
        elif state.get("pending_kind") == "chunk" and \
                state.get("pending_wire_bytes", 0) > 0:
            classification = "client-write-stall"
        elif state.get("request_chunks_written", 0) > \
                observer.request_chunks_acked:
            classification = "remote-ack-stall"
        elif error.phase in ("RESPONSE_STREAM", "COMPLETE"):
            classification = "response-read-progress-stall"
        else:
            classification = "phase-progress-timeout"
        ...
        persist_transport_refusal(
            ...
            failure_classification=(
                "protocol-corruption" if observer.error is not None
                else "process-exit-or-local-refusal"))
        raise RuntimeError(
            "single remote transaction was refused; cleanup state is unknown "
            "and no second remote cleanup is permitted") from error
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v4-return-") as raw:
        temporary = Path(raw)
        try:
    ...
    write_json(protocol / "request-progress-chain.json", {
        ...
        "row_cap": 512, "rows": result.request_progress_chain,
        "rows_recorded": len(result.request_progress_chain),
        "chunks_planned": request.chunk_count,
        "last_fully_acknowledged_chunk": request.chunk_count,
        "client_transport_state": observer.client_transport_state,
    ...
    receipt = {
        ...
        "encoder_workloads": 0 if mode == "rehearse" else 184,
        "profile_encoder_workloads": 0 if mode == "rehearse" else 160,
        "untimed_correctness_workloads": 0 if mode == "rehearse" else 16,
        "untimed_counter_workloads": 0 if mode == "rehearse" else 8,
