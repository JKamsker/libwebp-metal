#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization alignment v2."""
...
import execute_backref_cost_specialization_alignment_v2 as executor
import run_backref_cost_specialization_alignment_v2_experiment as operator
import backref_cost_specialization_alignment_v2_toolchain as toolchain
import evaluate_backref_cost_specialization_alignment_v2 as evaluator
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostSpecializationAlignmentV2GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostSpecializationAlignmentV2GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_specialization_alignment_v2_admission.py").read_text()
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v2-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority": "ALIGNMENT_V2_DIAGNOSTIC_ONLY",
    ...
    diff = [
        ("A", "evidence/backref-cost-specialization-alignment-v2-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-specialization-alignment-v2-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-specialization-alignment-v2-design/phase1b-authorization.json")]
    kwargs = {
...
def compact_source_control_freeze_contract(manifest: dict) -> None:
    ...
    assert freeze["positive_alignment_v2_compact_fits_cap"] is True
    assert "source_hashes" not in json.dumps(compact)


def v4_audit_and_null_contract(manifest: dict) -> None:
    audit = json.loads((
        ROOT / "evidence/backref-cost-specialization-alignment-v2-design/phase1-prior-evidence-audit.json"
    ).read_text())
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "b96909b28b3d64f000d0f98ffa76137305010808"
    ...
    assert manifest["protocol_version"] == transport.PROTOCOL_VERSION == 9
    ...
    evaluator_source = (ROOT / "scripts" /
        "evaluate_backref_cost_specialization_alignment_v2.py").read_text()
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_specialization_alignment_v2.py run '
                '"$PWD/evidence/backref-cost-specialization-alignment-v2-execution-20260820"')
    ...
    print("PASS: specialization alignment v2 frozen semantics, strict single-session frames, "
