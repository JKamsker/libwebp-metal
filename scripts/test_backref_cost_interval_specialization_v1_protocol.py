#!/usr/bin/env python3
"""Untimed contract validation for the frozen interval-specialization-v1 handoff."""
...
import backref_cost_interval_specialization_v1_archive as archive_protocol
import backref_cost_interval_specialization_v1_admission as admission
import execute_backref_cost_interval_specialization_v1 as executor


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = ROOT / "scripts/backref_cost_interval_specialization_v1_manifest.json"
SCHEMAS_PATH = ROOT / "scripts/backref_cost_interval_specialization_v1_schemas.json"
DESIGN = ROOT / "evidence/backref-cost-interval-specialization-v1-design"
...
def main() -> int:
    ...
    assert manifest["schema"] == \
        "libwebp-backref-cost-interval-specialization-v1-frozen-protocol-v1"
    assert manifest["protocol_version"] == catalog["protocol_version"] == 1
    assert manifest["base_commit"] == \
        "b2c71348c21371877328639efb5a929f2130bd09"
    ...
    frozen_run = (
        ...
        "WEBP_EXPECTED_BACKREF_COST_INTERVAL_SPECIALIZATION_V1_COMMIT=\"$(git rev-parse HEAD)\" "
        "python3 scripts/execute_backref_cost_interval_specialization_v1.py run "
        "/Users/jonaskamsker/.codex/worktrees/c038/libwebp-metal/evidence/"
        "backref-cost-interval-specialization-v1-execution-20260819")
    ...
    assert manifest["remote_host"]["safe_root_basename"] == \
        ".libwebp-metal-backref-cost-interval-specialization-v1"
    ...
    driver = (ROOT / "scripts/execute_backref_cost_interval_specialization_v1.py").read_text()
    runner = (ROOT / "tools/backref_cost_interval_specialization_v1_experiment_runner.c").read_text()
    ...
    assert local["premise_valid"]
    assert local["tests"]["exact_v3_normalized_semantic_identity"] == "PASS"
    assert local["tests"]["cost_manager_layout_33224_bytes"] == "PASS"
    identity = json.loads((ROOT /
        "evidence/backref-cost-interval-specialization-v1-design-work/identity.json").read_text())
    assert identity["normalized_v3_specialized_candidate_identical"]
    assert identity["cost_manager_baseline_bytes"] == \
        identity["cost_manager_v3_bytes"] == \
        identity["cost_manager_specialization_v1_bytes"]
    assert not identity["candidate_push_has_use_append_hint_argument"]
    assert not identity["candidate_hot_path_has_experiment_control"]
    summary = json.loads((ROOT /
        "evidence/backref-cost-interval-specialization-v1-design-work/summary.json").read_text())
    assert summary["status"] == "PASS"
    ...
    print("PASS: specialization v1 frozen hashes, receipt/mode contracts, local gates, exact "
