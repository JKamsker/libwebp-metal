#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization alignment v1."""
...
import run_backref_cost_specialization_alignment_v1_experiment as operator
import backref_cost_specialization_alignment_v1_toolchain as toolchain
import evaluate_backref_cost_specialization_alignment_v1 as evaluator
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostSpecializationAlignmentV1GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostSpecializationAlignmentV1GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_specialization_alignment_v1_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-specialization-alignment-v1-archive-inventory-v1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v1-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority": "ALIGNMENT_V1_DIAGNOSTIC_ONLY",
    ...
    diff = [
        ("A", "evidence/backref-cost-specialization-alignment-v1-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-specialization-alignment-v1-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-specialization-alignment-v1-design/phase1b-authorization.json")]
    kwargs = {
...
def v4_audit_and_null_contract(manifest: dict) -> None:
    audit = json.loads((
        ROOT / "evidence/backref-cost-specialization-alignment-v1-design/phase1-prior-evidence-audit.json"
    ).read_text())
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "adf680e69b9eeeef636e62e0e0eb0c8fdace4013"
    ...
    assert manifest["protocol_version"] == transport.PROTOCOL_VERSION == 8
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_specialization_alignment_v1.py run '
                '"$PWD/evidence/backref-cost-specialization-alignment-v1-execution-20260820"')
    ...
    print("PASS: specialization alignment v1 frozen semantics, strict single-session frames, "
