#!/usr/bin/env python3
"""Frozen local driver for one specialization-factorization-v1 single-session transaction."""
...
import backref_cost_specialization_factorization_v1_admission as admission
import backref_cost_specialization_factorization_v1_archive as archive_protocol
...
PROTOCOL_PATH = ROOT / "doc" / "backref-cost-specialization-factorization-v1-protocol-20260819.md"
TRANSPORT_PATH = ROOT / "scripts" / "backref_cost_specialization_factorization_v1_transport.py"
AUTHORIZATION_PATH = (ROOT / "evidence" / "backref-cost-specialization-factorization-v1-design" /
                      "phase1b-authorization.json")
REHEARSAL_HASH_INDEX_PATH = (
    ROOT / "evidence" / "backref-cost-specialization-factorization-v1-design" /
    "phase1b-rehearsal-hashes.json")
REHEARSAL_ROOT = (ROOT / "evidence" / "backref-cost-specialization-factorization-v1-design" /
                  "rehearsal")
EXPECTED_ENV = "EXPECTED_FREEZE_COMMIT"
AUTHORIZATION_SCHEMA = \
    "libwebp-backref-cost-specialization-factorization-v1-authorization-v1"
REHEARSAL_HASH_INDEX_SCHEMA = \
    "libwebp-backref-cost-specialization-factorization-v1-phase1b-hash-index-v1"
...
def _allowed_phase1b_path(path: str) -> bool:
    prefix = "evidence/backref-cost-specialization-factorization-v1-design/rehearsal/"
    return path.startswith(prefix) or path in {
        "evidence/backref-cost-specialization-factorization-v1-design/phase1b-rehearsal-hashes.json",
        "evidence/backref-cost-specialization-factorization-v1-design/phase1b-authorization.json",
    }
...
def validate_authority_values(*, envelope: dict | None, head_commit: str,
    ...
    if not envelope.get("timed_execution_authorized") or \
            ...
            envelope.get("phase_2_authority") != "FACTORIZATION_V1_DIAGNOSTIC_ONLY":
...
def validate_authorized_handoff(head: str) -> str:
    ...
    hashes = {
        "manifest": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_factorization_v1_manifest.json"),
        "protocol": _git_file_sha256(
            source_commit, "doc/backref-cost-specialization-factorization-v1-protocol-20260819.md"),
        "schemas": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_factorization_v1_schemas.json"),
        "transport": _git_file_sha256(
            source_commit, "scripts/backref_cost_specialization_factorization_v1_transport.py"),
    }
...
def lease_transfer_contract(mode: str, attempted: bool,
    ...
    if not attempted:
        return ({"schema": "libwebp-backref-cost-specialization-factorization-v1-lease-transfer-v1",
...
def remote_pipeline(output: Path, mode: str) -> dict:
    ...
    try:
        result = admission.pump_owned_full_duplex(
            _transport_command(), request_chunks,
        ...
        write_json(output / "transfer-cleanup.json", {
            "schema": "libwebp-backref-cost-specialization-factorization-v1-transfer-cleanup-v1",
    ...
    with tempfile.TemporaryDirectory(prefix="specialization-factorization-v1-return-") as raw:
        temporary = Path(raw)
        try:
