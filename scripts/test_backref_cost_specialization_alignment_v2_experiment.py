...
BASE = "b96909b28b3d64f000d0f98ffa76137305010808"
BUILD = "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_EXPERIMENT"
RECORDER = "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_RECORDER"
MACRO = "WEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V2_VARIANT"
RUNNER = "backref_cost_specialization_alignment_v2_experiment_runner"
...
def main() -> int:
    ...
    assert "VP8LBackrefCostSpecializationAlignmentV2GetVariant" in source
    assert "switch (variant)" in source
    assert not run(["git", "diff", "--name-only", BASE, "--", "src/webp",
                    "sharpyuv"]).stdout.strip()

    with tempfile.TemporaryDirectory(
            prefix="backref-specialization-alignment-v2-test-") as raw:
        temporary = Path(raw)
        ...
        assert b"SPECIALIZATION_ALIGNMENT_V2" not in ordinary_archive
        assert b"specialization_alignment_v2" not in ordinary_archive
        ...
        assert "AlignmentV2RecordPush" not in timed_nm
        assert "AlignmentV2RecordPush" in recorder_nm
