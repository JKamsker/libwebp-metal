#!/usr/bin/env python3
"""Static, workload-free ownership guards for specialization aligned null stage attribution v1."""
...
import run_backref_cost_aligned_null_stage_attribution_v1_experiment as operator_module


ROOT = Path(__file__).resolve().parents[1]
INFRASTRUCTURE = (
    "scripts/backref_cost_aligned_null_stage_attribution_v1_admission.py",
    "scripts/backref_cost_aligned_null_stage_attribution_v1_archive.py",
    ...
    "scripts/run_backref_cost_aligned_null_stage_attribution_v1_experiment.py",
)
...
def static_guards() -> None:
    ...
    assert signal_sites == [
        ("scripts/backref_cost_aligned_null_stage_attribution_v1_admission.py", "self.process.kill")]
    assert popen_sites == [
        "scripts/backref_cost_aligned_null_stage_attribution_v1_admission.py",
        "scripts/backref_cost_aligned_null_stage_attribution_v1_admission.py"]
...
def online_runner_ownership_fixtures() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v1-ledger-") as raw:
        root = Path(raw)
...
def identity_only_process_contract() -> None:
    ...
    ignored_ancestor_argv_and_source = (
        "python3 -c 'backref_cost_aligned_null_stage_attribution_v1_experiment_runner "
    ...
    exact_conflicts = (
        ...
        "backref_cost_aligned_null_stage_attribution_v1_experiment_runner",
    ...
    for index, identity in enumerate(exact_conflicts, start=200):
        ...
        assert diagnostic == {
            "schema":
                "libwebp-backref-cost-aligned-null-stage-attribution-v1-process-refusal-v1",
    ...
    lookalikes = (
        ...
        "prefix_backref_cost_aligned_null_stage_attribution_v1_experiment_runner",
        "backref_cost_aligned_null_stage_attribution_v1_experiment_runner_suffix",
...
def operator_refusal_control_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v1-refusal-") as raw:
...
def single_session_contract() -> None:
    ...
    source = (ROOT / "scripts/execute_backref_cost_aligned_null_stage_attribution_v1.py").read_text()
    ...
    remote = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v1_transport.py").read_text()
...
def main() -> int:
    ...
    print("PASS: specialization aligned null stage attribution v1 proves arbitrary opaque ancestors structurally, "
