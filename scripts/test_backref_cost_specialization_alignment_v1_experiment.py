...
BASE = "adf680e69b9eeeef636e62e0e0eb0c8fdace4013"
BUILD = "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_EXPERIMENT"
RECORDER = "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_RECORDER"
MACRO = "WEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V1_VARIANT"
RUNNER = "backref_cost_specialization_alignment_v1_experiment_runner"
...
def main() -> int:
    ...
    assert "VP8LBackrefCostSpecializationAlignmentV1GetVariant" in source
    assert "switch (variant)" in source
    assert not run(["git", "diff", "--name-only", BASE, "--", "src/webp",
                    "sharpyuv"]).stdout.strip()

    with tempfile.TemporaryDirectory(
            prefix="backref-specialization-alignment-v1-test-") as raw:
        temporary = Path(raw)
        ...
        assert "AlignmentV1RecordPush" not in timed_nm
        assert "AlignmentV1RecordPush" in recorder_nm
