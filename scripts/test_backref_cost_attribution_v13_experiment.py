#!/usr/bin/env python3
"""Untimed V13 correctness, single-DP, sanitizer, ABI, and omission checks."""
...
BASE = "7700d05060803afcac030aa5452849f93515b0d8"
BUILD = "WEBP_BUILD_BACKREF_COST_ATTRIBUTION_V13_EXPERIMENT"
MACRO = "WEBP_USE_BACKREF_COST_ATTRIBUTION_V13_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_ATTRIBUTION_V13_EXPERIMENT"
RUNNER = "backref_cost_attribution_v13_experiment_runner"
SOURCE = "backref_cost_attribution_v13_experiment_enc.c"
...
def main() -> int:
    ...
    assert "V13 preserves V12's frozen control flow" in \
        source_text
    assert "BACKREF_DP_RECORD_CALL();" in (
        ROOT / "src/enc/backward_references_cost_distance_only_enc.inc"
    ).read_text()

    with tempfile.TemporaryDirectory(prefix="backref-attribution-v13-test-") as raw:
        temporary = Path(raw)
        ...
        for variant in ("baseline", "candidate"):
            ...
            environment = {
                ...
                "WEBP_BACKREF_COST_ATTRIBUTION_V13_STAGE_OUTPUT":
                    str(stage_output),
                "WEBP_BACKREF_COST_ATTRIBUTION_V13_RUN_ID": "local-smoke",
                "WEBP_BACKREF_COST_ATTRIBUTION_V13_CASE_ID":
                    "synthetic-no-encoder",
                "WEBP_BACKREF_COST_ATTRIBUTION_V13_SAMPLE_SET": "cold",
            }
            if variant == "candidate":
                environment[RUNTIME] = "1"
