#!/usr/bin/env python3
"""Frozen remote operator for diagnostic-only backref-cost attribution v2."""
...
from backref_cost_attribution_v2_admission import (
...
def clean_environment(manifest: dict, variant: str,
    ...
    if stage_output is not None:
        env.update({
            ...
            "WEBP_BACKREF_COST_ATTRIBUTION_V2_TIMERS": "1",
            "WEBP_BACKREF_COST_ATTRIBUTION_V2_STAGE_OUTPUT": str(stage_output),
            "WEBP_BACKREF_COST_ATTRIBUTION_V2_RUN_ID": run_id or "",
            "WEBP_BACKREF_COST_ATTRIBUTION_V2_CASE_ID": case_id or "",
            "WEBP_BACKREF_COST_ATTRIBUTION_V2_SAMPLE_SET": "warm-dominant",
        })
    return env
...
def process_guard(output: Path, label: str) -> dict:
    ...
    conflicts = [line for line in conflicts
                 if "run_backref_cost_attribution_v2_experiment.py" not in line
...
def build(manifest: dict, temporary: Path, output: Path) -> dict:
    ...
    argv = ["/usr/bin/make", "-f", "makefile.unix", "WEBP_ENABLE_METAL=0",
            ...
            "WEBP_BACKREF_COST_ATTRIBUTION_V2_DETERMINISTIC_BUILD=1",
            f"CFLAGS={flags}",
    ...
    (output / "build.log").write_text(result.stdout + result.stderr,
                                       encoding="utf-8")
    runner = build_root / "tools/backref_cost_attribution_v2_experiment_runner"
...
def execute_profiles(manifest: dict, plan: list[dict], output: Path) -> list[dict]:
    ...
    for row in plan:
        ...
        if any(item.get("schema") != "libwebp-backref-cost-attribution-v2-output-v1"
               for item in outputs):
...
def operator(mode: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="backref-attribution-v2-") as raw:
        temporary = Path(raw)
