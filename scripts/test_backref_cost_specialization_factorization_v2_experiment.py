...
BASE = "ccdd4900633064f203010cbd437d2682e1cba51e"
BUILD = "WEBP_BUILD_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_EXPERIMENT"
RECORDER = "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_RECORDER"
MACRO = "WEBP_USE_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_EXPERIMENT"
RUNTIME = "WEBP_BACKREF_COST_SPECIALIZATION_FACTORIZATION_V2_VARIANT"
RUNNER = "backref_cost_specialization_factorization_v2_experiment_runner"
...
def main() -> int:
    ...
    assert "VP8LBackrefCostSpecializationFactorizationV2GetVariant" in source
    assert "switch (variant)" in source
    assert not run(["git", "diff", "--name-only", BASE, "--", "src/webp",
                    "sharpyuv"]).stdout.strip()

    with tempfile.TemporaryDirectory(
            prefix="backref-specialization-factorization-v2-test-") as raw:
        temporary = Path(raw)
        ...
        timed_nm = run(["nm", "-nm", str(timed_runner)]).stdout
        recorder_nm = run(["nm", "-nm", str(recorder_runner)]).stdout
        ...
        assert "FactorizationV2RecordPush" not in timed_nm
        assert "FactorizationV2RecordPush" in recorder_nm
        ...
        for symbol, expected in frozen_codegen["facts"].items():
            assert symbol_facts(timed_runner, symbol) == expected
