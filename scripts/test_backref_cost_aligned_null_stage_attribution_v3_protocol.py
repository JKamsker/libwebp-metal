#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization aligned null stage attribution v3."""
...
import run_backref_cost_aligned_null_stage_attribution_v3_experiment as operator
...
import evaluate_backref_cost_aligned_null_stage_attribution_v3 as evaluator


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = ROOT / "scripts" / "backref_cost_aligned_null_stage_attribution_v3_manifest.json"
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostAlignedNullStageAttributionV3GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V3_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostAlignedNullStageAttributionV3GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_aligned_null_stage_attribution_v3_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-aligned-null-stage-attribution-v3-archive-inventory-v1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="stage-attribution-v3-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    diff = [
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v3-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v3-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-aligned-null-stage-attribution-v3-design/phase1b-authorization.json")]
    kwargs = {
...
def zero_sample_refusal_contract() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(
                prefix="stage-attribution-v3-refusal-") as raw:
...
def v4_audit_and_null_contract(manifest: dict) -> None:
    audit = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v3-design/phase1-prior-evidence-audit.json"
    ).read_text())
    assert audit["status"] == "PASS_TRANSPORT_REQUALIFICATION_PREMISE_FROZEN"
    assert audit["performance_samples_generated"] == 0
    comparison = json.loads((
        ROOT / "evidence/backref-cost-aligned-null-stage-attribution-v3-design/transport-comparison.json"
    ).read_text())
    ...
    assert comparison["variants"]["alignment-v3"]["request_chunks_acknowledged"] == 354
    assert comparison["variants"]["alignment-v4"]["request_chunks_acknowledged"] == 357
    assert audit["v4_timing_rows_loaded_into_v3_statistics"] == 0
    assert audit["v1_timing_rows_loaded_into_v3_statistics"] == 0
    assert audit["v1_authority_audit"]["failed_gates"] == [
        "alignment build frozen"]
    assert audit["v1_authority_audit"]["can_contribute_samples_to_v3"] is False
...
def main() -> int:
    ...
    assert manifest["candidate"]["build_flag"] == \
        "WEBP_BUILD_BACKREF_COST_ALIGNED_NULL_STAGE_ATTRIBUTION_V3_EXPERIMENT"
    ...
    evaluator_source = (ROOT / "scripts" /
        "evaluate_backref_cost_aligned_null_stage_attribution_v3.py").read_text()
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_aligned_null_stage_attribution_v3.py run '
                '"$PWD/evidence/backref-cost-aligned-null-stage-attribution-v3-execution-20260820"')
    ...
    print("PASS: specialization aligned null stage attribution v3 frozen semantics, strict single-session frames, "
