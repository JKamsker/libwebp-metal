#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for attribution v16."""
...
import run_backref_cost_attribution_v16_experiment as operator
...
def semantic_identity() -> None:
    ...
    boundary = current.split(
        ...
        1)[1].split("#else", 1)[0]
    assert boundary.count("VP8LBackrefCostAttributionV16ExperimentEnabled()") == 1
    ...
    assert "VP8LBackrefCostAttributionV16RecordSelector();" in boundary
    preprocessed = subprocess.check_output(
        [os.environ.get("CC", "cc"), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_ATTRIBUTION_V16_EXPERIMENT=1",
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_attribution_v16_admission.py").read_text()
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v16-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority": "V16_DIAGNOSTIC_ONLY",
    ...
    diff = [
        ("A", "evidence/backref-cost-attribution-v16-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-attribution-v16-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-attribution-v16-design/phase1b-authorization.json")]
    kwargs = {
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "8ac198e565ab36eedd6e124202b78c09e5180ec1"
    ...
    print("PASS: attribution v16 frozen semantics, strict single-session frames, "
