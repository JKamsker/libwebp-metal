#!/usr/bin/env python3
"""Untimed, workload-free protocol and freeze tests for specialization alignment v4."""
...
import run_backref_cost_specialization_alignment_v4_experiment as operator
import backref_cost_specialization_alignment_v4_toolchain as toolchain
import evaluate_backref_cost_specialization_alignment_v4 as evaluator
...
def semantic_identity() -> None:
    ...
    assert "VP8LBackrefCostSpecializationAlignmentV4GetVariant" in current
    ...
    preprocessed = subprocess.check_output(
        [str(toolchain.EXECUTABLE_PATHS["clang"]), "-E", "-P", f"-I{ROOT}",
         "-DWEBP_USE_BACKREF_COST_SPECIALIZATION_ALIGNMENT_V4_EXPERIMENT=1",
    ...
    assert traceback.count(
        "VP8LBackrefCostSpecializationAlignmentV4GetVariant()") == 1
...
def v11_v13_boundary_audit() -> None:
    ...
    new_admission = (ROOT / "scripts/backref_cost_specialization_alignment_v4_admission.py").read_text()
...
def response_fixture(*, success: bool = True, cleanup: bool = True,
    ...
    if include_bulk:
        declaration = {"schema": transport.BULK_DECLARATION_SCHEMA,
                       ...
                       "inventory": {
                           "schema": "libwebp-backref-cost-specialization-alignment-v4-archive-inventory-v1",
...
def plan_contract(manifest: dict) -> None:
    with tempfile.TemporaryDirectory(prefix="specialization-alignment-v4-plan-") as raw:
        temporary = Path(raw)
...
def authority_rejections() -> None:
    ...
    envelope = {
        ...
        "phase_2_authority":
            "ALIGNMENT_V4_CONDITIONAL_DIAGNOSTIC_AUTHORITY",
    ...
    diff = [
        ("A", "evidence/backref-cost-specialization-alignment-v4-design/rehearsal/raw.bin"),
        ("A", "evidence/backref-cost-specialization-alignment-v4-design/phase1b-rehearsal-hashes.json"),
        ("A", "evidence/backref-cost-specialization-alignment-v4-design/phase1b-authorization.json")]
    kwargs = {
...
def zero_sample_refusal_contract() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(
                prefix="specialization-alignment-v4-refusal-") as raw:
...
def v4_audit_and_null_contract(manifest: dict) -> None:
    audit = json.loads((
        ROOT / "evidence/backref-cost-specialization-alignment-v4-design/phase1-prior-evidence-audit.json"
    ).read_text())
...
def main() -> int:
    ...
    assert manifest["base_commit"] == \
        "91ab79b70bb1f99888dce63e8b1ac512741bafb3"
    ...
    assert manifest["protocol_version"] == transport.PROTOCOL_VERSION == 11
    ...
    assert manifest["dynamic_admission"]["gate_a"]["count"] == 24
    assert manifest["dynamic_admission"]["gate_b"]["accounting_rows"] == 3
    assert manifest["dynamic_admission"]["gate_c"][
        "planned_runner_children"] == 76
    assert manifest["dynamic_admission"]["gate_c"]["breakdown"] == {
        "correctness": 24, "clock": 1, "timer_accounting": 3,
        "work": 12, "profile": 36}
    ...
    evaluator_source = (ROOT / "scripts" /
        "evaluate_backref_cost_specialization_alignment_v4.py").read_text()
    ...
    expected = ('env WEBP_BENCHMARK_SESSION=exclusive '
                ...
                'python3 scripts/execute_backref_cost_specialization_alignment_v4.py run '
                '"$PWD/evidence/backref-cost-specialization-alignment-v4-execution-20260820"')
    ...
    print("PASS: specialization alignment v4 frozen semantics, strict single-session frames, "
