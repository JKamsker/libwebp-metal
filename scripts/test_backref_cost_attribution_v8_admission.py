#!/usr/bin/env python3
"""Untimed attribution-v8 path, admission, frame, and mode tests."""
...
from backref_cost_attribution_v8_paths import (
...
def path_tests() -> None:
    ...
    assert SAFE_ROOT_BASENAME.endswith("attribution-v8")
    ...
    rejected(validate_shape, home, root,
             f"{home}/.libwebp-metal-backref-cost-attribution-v6/run.A21KcK")
    with tempfile.TemporaryDirectory(prefix="attribution-v8-path-home-") as raw:
...
def repository_context_admission() -> None:
    with tempfile.TemporaryDirectory(prefix="attribution-v8-admit-") as raw:
        root = Path(raw)
        ...
        subprocess.run(["git", "-C", str(repository), "config", "user.name",
                        "v8 admission test"], check=True)
        subprocess.run(["git", "-C", str(repository), "config", "user.email",
                        "v8@example.invalid"], check=True)
...
def remote_refusal_cleanup_contract() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="attribution-v8-remote-home-") as raw:
...
def main() -> int:
    ...
    mode_truth_table()
    print("PASS: attribution v8 canonical-only paths, repository-context source "
